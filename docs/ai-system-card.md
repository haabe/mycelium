...
- **Version:** 0.39.19 (canonical source: the `*Version X.Y.Z` line in `CLAUDE.md`; mechanical tokens here — version, skill count — are kept in sync by `scripts/sync_derived.py`, not hand-edited)
...
- **Skills:** 50 skills — markdown SKILL.md files in `plugins/mycelium/skills/` (installed to `.claude/skills/`) — define structured procedures the agent invokes by name (`/interview`, `/diamond-progress`, `/xai-check`, etc.).
...
- **Last full audit:** 2026-06-05 via `/mycelium:xai-check` against `svc-mycelium` (fourth full audit; canonical tier confirmed by `/regulatory-review` 2026-05-04 and unchanged). Result: tier holds **Limited**; Stage 2 matrix unchanged from baseline (8 of 10 cells pass; `end_user.output` and `deployer_developer.why` remain partial pending Juniors.dev real-flow data); Stage 3 fidelity verdict reframed (eval closed 2026-05-12 INSTRUMENT FAILED; replaced by C1 mechanical capture per §4); Stage 4 system card pass (this refresh); Stage 5 recourse pass. The re-audit also surfaced two meta-findings: a `/canvas-health` spot-check on 2026-06-04 missed the Stage 3 staleness because it used duration-based "no shipped feature" heuristic and didn't reach into the fidelity sub-block; and this very card had drifted six framework patches behind on its mechanical version token until the operator asked "why is the system card stale" mid-audit. Both findings drove structural fixes (Check 40 pre-push gate wraps `sync_derived.py --check`; `/canvas-health` 9b sub-check compares system card to `services.yml :: xai.*`) in v0.39.14. Full re-audit record at `.claude/canvas/services.yml :: svc-mycelium.xai.remediation_history[2026-06-05]`.
