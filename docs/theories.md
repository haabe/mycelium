...
The L0 Purpose canvas (`canvas/purpose.yml`) follows the Why → How → What ordering Sinek argues for, and `/interview` **Q1 leads with the change** ("What are you trying to change, and for whom?") — Sinek's Why, the difference you want true in the world — populating purpose from the change/why answer, **not** the build and **not** a bare problem/job. (Corrected v0.55.1: previously Q1 asked "what are you trying to build, and for whom" and purpose was back-labeled from the What. A first draft reframed to "what problem are you solving" — but that is Torres/Christensen's *job*, not Sinek's *belief*, so it would have back-labeled one layer down; the shipped wording asks for the **change in the world**, with a light follow-up — "what becomes true if it works?" — to reach the belief, and another to capture the build for the brief. Open stem: solution-first/curiosity-first users proceed without a forced problem-frame.) Honest scope: Sinek is **not hard-gated** — the L0→L1 transition uses the generic **Evidence gate**, not a `why`-specific check, and the schema treats `why` as optional by design; "start with Why" is modeled in canvas structure + interview elicitation-order, which is the faithful representation for a communication principle rather than a gate. Implemented as: `canvas/purpose.yml`, `/interview` Q1 + Phase-1 depth, gate 1 (Evidence) at L0→L1.
...
Strategic landscape with evolution stages (Genesis → Custom → Product → Commodity). Mycelium's L1 strategic_frame articulates "where Mycelium plays" using Wardley's vocabulary. Implemented as: `canvas/landscape.yml`, `/wardley-map`, and the **L1 Engineering-trio prompt** in `theory-gates.md` (the trio table's L1 Strategy row adds "Technology landscape, Wardley evolution stages"). There is no hard Wardley gate — the skill and canvas carry the discipline. (Corrected v0.71.0: this line also claimed "a NUDGE at Develop→Deliver". No such nudge exists anywhere in the plugin tree; the claim is withdrawn rather than implemented, because nothing establishes that a Wardley check belongs at that transition.)
