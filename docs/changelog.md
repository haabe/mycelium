...
- **`hooks/absence_claim_guard.py`**: `_SENTENCE` split on every newline (`(?<=[.!?])\s+|\n`). A correction entry wrapped at 100 columns fired on the sentence written to satisfy the guard, because its scoping clause sat on the previous line (candidate 2026-08-30; two of the four fires on 2026-09-14 were the same shape). A bare newline is no longer a break. What still is: a sentence end, a blank line, a line that starts a bullet or numbered item, and a line that starts a YAML key — the three shapes where the next line is a new thought. Five tests pin each, including that a wrapped unscoped claim still fires once. The 88-test suite is unchanged otherwise.
- **`engine/surface-registry.yml`**: the `upstream-candidate` row read `authoritative_surface: null`, "17 known items sit as prose in a chronological log", from 2026-07-26 until today, while the dogfood register `.claude/harness/upstream-candidates.yml` had existed since 2026-07-27 and `check_upstream_candidates.py` had read it since 0.109.0 (candidate 2026-08-27). The registry built to catch right-content-wrong-surface carried a stale row about its own most important class. Closed: surface, reader and point-check declared; `check_surface_registry` verifies the reader references the surface.
- **`docs/ai-system-card.md`** §1 named `scripts/sync_derived.py`; the script is `plugins/mycelium/scripts/sync_derived.py`, and DL-1199 records an agent running the wrong path and shipping an unsynced card (candidate 2026-09-08). Fixed. The sibling candidate, "the pre-push hook skips Check 40", is declined on the record: `tests/validate-template.sh` has run Check 40 (`sync_derived.py --check`) since 0.39.14 and the installed pre-push hook runs that validator before the gate set; the 2026-09-08 miss was a push the hook did not run, not a gate the hook lacks. Re-open if a stale card reaches origin from a push whose pre-push log shows the validator ran.

## v0.200.0 - the disclosure moves when the hooks do

Dogfood 2026-09-14, sixth `/xai-check` on the framework as the product. `docs/ai-system-card.md` is the published account of what the AI layer does to the person using it, and it was a quarter behind the hooks for the second time. 2026-06-11 it was silent on autonomous mode for six patches. This time it said nothing about five behaviours shipped between 0.172.0 and 0.197.0: the Stop hook that will not end a framework turn without a `Next:` line, the one proposal sent to the human as a `systemMessage` on resume, the advisory ledger that mutes a warning after seven ignored sessions, ruling slots on the diamond, and canvas prose delimited as untrusted before it reaches the agent. Two required sections also described retired mechanisms: §5 located the citation rule in `CLAUDE.md` (plugin form: the injected operating contract, rule 4) and §6 promised graduation "on the next L4 cleanup cycle", a cadence nothing enforced since count-keyed graduation with `review_by` dates replaced it. Both times an audit found it; both times every release had passed, because the only thing the release path reads on the card is its version and skill-count tokens (`sync_derived.py`).

- **`docs/ai-system-card.md`** refreshed: §2 gains "what happens at the session's boundaries" (the four behaviours and the untrusted delimiting, each with its script and version); §3 describes the hook layer as it is (24 shell hooks in three manifests, six blocking at 30 s and failing closed, the 0.189.0 delivery SLI); §5 locates the rule where it lives and **states the measurement** — three `(per ...)` citations across the twelve most recent dogfood transcripts — so the reader is told the inline form is advisory and the decision log is the durable rationale surface; §6 describes the live graduation commitment and the two in-place recourses for the framework's own nudges (ledger rulings, not committed; proposal rulings on the diamond, committed).
- **`docs/context-surface.md`** refreshed (it read 2026-06-05 and listed four hooks of twenty-four): the session-start injection tiers, the NEXT ITEM channel, the state files the September hooks write, and the boundary hooks table.
- **`scripts/check_system_card_hooks.py`** (new, in the gate set and `validate.yml`): a 12-hex digest over every file under `plugins/mycelium/hooks/` plus `manifest.yml`, stamped in the card's identity block as `- **Hook surface reviewed:** YYYY-MM-DD (digest ...)`. Digest differs: exit 1, naming the file count and both digests, with the remedy (read the diff against §2/§3/§5/§6, fix what it made untrue, `--write`). No review line, no card, no hooks: exit 2, not a pass. **What it cannot do, stated in its header**: it cannot tell a typo from a new blocking hook, so both cost one read and one `--write`; and it cannot see whether the prose is right, which stays with `/xai-check` on cadence. It uses no git history, so it runs the same in CI's shallow clone as locally.
- Tests: `test_check_system_card_hooks.py` (9): a changed hook, a new hook file and a manifest change are each found; a missing review line and a missing card are preconditions; `--write` replaces its own line, anchors below `Last updated`, and fails loud without an anchor; the digest is stable and path-sensitive.

## v0.199.0 - the record is checked against itself

Dogfood 2026-09-14, `/framework-health`. Two defects in `cycle-history.yml`, neither new. `calibration_summary` is a hand-kept aggregate of the `cycles` list in the same file and had drifted from it for the fourth time (2026-06-05, 08-28, 09-02, 09-14): 22 against 23 rows, and an `outcome_distribution` of 11/3/0 against 17/4/2. On 09-02 the stale figure was handed to two blind adversarial readers who built their central argument on it. Every instance was found by an audit run and none at write time, because nothing compared the two halves. And the first record ever to carry `gates_fired` (cycle-017) coded six catches as `result: pass` beside a paragraph each of what was caught; the three records after it coded catches as `fail`, which is what `engine/cycle-learning.md` defines. Framework-health read a 50% gate failure rate from those four records as written and a different number by the definition, and nothing related the two fields.

- **`cycle_integrity_findings(cycle_file)`** (new, `check_cycle_recording.py`): for every key `calibration_summary` carries that is a pure count over the list (`total_cycles`, `launched`, `killed`, `archived`, `in_flight_observation`, `reconstructed_post_hoc_count`, and each sub-key of `cycle_class_distribution` and `outcome_distribution`), recomputes it from the list and names each disagreement with both numbers. A sub-key the summary claims and the list never produces is compared against 0, not skipped. Keys the summary does not carry are not demanded. Second finding: every `gates_fired` row with `result: pass` and a non-empty `caught`, listed by cycle and gate. **Reported, never re-coded**: the author rules; the check says only that the two fields disagree.
- **`validate_canvas.py`** calls it beside `cycle_field_coverage` through the same delegation, same WARN tier, same never-raise contract.
- Run on the dogfood canvas before release: the summary (recomputed that morning) is consistent; **10 pass-with-caught rows, not the 6 the morning's assessment counted** — four more sit in the three newer records, where advisory guards that fired and caught something were also written as `pass`. Whether an advisory guard that fired "passed" or "failed" is a definition the engine doc settles one way and the records settle the other; the check surfaces the question and leaves it on the author's desk.
- Tests: `test_cycle_field_coverage.py` (+5): a consistent summary is silent; a stale one names each field with both numbers; keys not carried are not demanded; a pass-with-caught row is reported and the file is untouched; malformed and empty files are silent.
