...
**Drew Hoskins** — Staff PM at Temporal, author of *The Product-Minded Engineer*. Over two rounds of LinkedIn feedback (2026-04-14, 2026-04-17), Hoskins sharpened two key critiques: (1) L0-L2 scales make sense, but L3-L5 "starts to look like another way to think of the product lifecycle" — the upstream is differentiated, the downstream needs more. (2) Scenarios should be a first-class primitive, not an implicit byproduct of JTBD mapping. The scenario structure Mycelium adopted — **Motivation + Persona + Simulation** — traces to *The Product-Minded Engineer* ch. 1. This directly led to `canvas/scenarios.yml` as a new canvas artifact, scenario wiring through the leaf lifecycle (phases 1, 5, 8, 10), and scenario extraction in `/user-interview`. Hoskins' "User Knowledge Repository" concept also validated Mycelium's canvas evidence system as architecturally aligned.

> **Correction, 2026-07-30.** This entry previously credited the scenario structure to a *"SAP talk ('Attention to Users Is All You Need', April 2026)"* and described it as four elements including "Means". **That talk does not exist — the citation was fabricated, and "Means" is not a Hoskins element.** Both were identified and corrected in [`docs/theories.md`](docs/theories.md) on 2026-07-01, but the correction was never propagated here, to `docs/philosophy.md`, or to `docs/glossary.md`, so a fabricated source with an invented title and date stayed attributed to a named real person for a further four weeks. Found 2026-07-30 by an adversarial documentation sweep. The LinkedIn critiques above are genuine and unchanged; only the fabricated source and the distorted element list are removed.
