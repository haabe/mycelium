...
Autonomous mode applies **only when explicitly declared**. Three surfaces:
...
3. **`MYCELIUM_AUTONOMOUS_RUN=1` environment variable** (hook-visible; set by the launching operator). This is the surface the **autonomous-evidence-guard** hook reads — it is the only declaration available *before* the first state write (and it survives the headless `.claude/` write-block, where the `active.yml` flag may never land). Set it whenever you launch a headless run that the guard must enforce; it is the recommended belt-and-suspenders with the run-prompt declaration.
...
**Model-tier caveat — KNOWN LIMITATION: this boundary is prose, not enforcement, and does NOT transfer below Fable 5 (test-settled).** Nothing in the harness blocks an `external_*` tag, an `evidence_type` upgrade, or a `validated: true` flip inside an autonomous run — the boundary above holds only as far as the running model honors it. The cross-model test settled this **2026-06-11 (Stage A, Haiku 4.5, n=1, blind-graded; opp-011 assumption #2, `.claude/evals/assumption-tests/cross-model-evidence-boundary.md`)**: on a fabrication-pull path Haiku **fabricated** external interview results ("3/5 hikers confirm…", named fictional respondents), **upgraded** the evidence class to `external_human`, **flipped** the claim to validated, and built a confidence delta on it — *without recognizing it had crossed the boundary* (its self-audit never flagged the fabrication). The prose boundary is therefore **model-dependent**: held by Fable 5 (n=2: Stage 2 + Stage 3), refuted by Haiku 4.5 (n=1); the Sonnet tier is unmeasured. **The enforcement guardrail ships (v0.42.0): `autonomous-evidence-guard`.** A PreToolUse hook (`hooks/autonomous-evidence-guard.sh` + `scripts/autonomous_evidence_guard.py`) that, **only in a declared autonomous run** (any of the three declaration surfaces above), hard-blocks any Write/Edit/MultiEdit or MCP-filesystem write that introduces `source_class: external_human|external_data`, `validated: true`, or `evidence_type` above `speculation` into `.claude/canvas/*.yml`, `.claude/diamonds/*.yml`, or their `mycelium-state/` mirror. It is a strict no-op in every interactive (human-present) session — a present human is never autonomous — so it adds zero friction to normal use. This turns the boundary from "hope the model holds it" into mechanism: had it been live, the Haiku Stage-A fabrication would have been blocked at write time.

**Defense-in-depth, not a single point.** The guard is necessary but not sufficient, so the operating rule still stands: **prefer a present human or a tier ≥ Sonnet for autonomous runs.** Known gaps the guard does NOT close: (a) Bash heredoc / `echo >` writes (the V8 evasion vector — relied on the model not evading, which Fable 5 and Haiku both honored, but it is unenforced); (b) prose fabrication a model *reports in-conversation* without writing to a canvas file; (c) fabrication into non-canonical paths outside the enforced set. The guard catches the cardinal write-path fabrication (V1/V2/V3); the tier restriction covers the rest.
