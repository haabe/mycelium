...
| P5 | `engine/mutation-log.md` specified `canvas/mutation_log.jsonl` in full — format, required fields, worked example — claiming replay, audit, regression support and drift detection | *(doc removed v0.124.0)* | **CLOSED v0.124.0 — THE DOC STOPPED PROMISING, which this registry has always allowed and which the analysis says is right.** Retired on the founder's decision after a dogfood analysis that inverted the question. THREE OF THE FOUR CLAIMS WERE ALREADY SERVED: replay and regression support by git (canvas files are committed), audit by the decision log in prose. The fourth, drift detection, is served by nothing — but is better reached from the dependency direction, not from a log. **AND THE EXPERIMENT HAD ALREADY RUN**: `.claude/state/change-log.jsonl` on the dogfood project held 5,802 entries over four months with exactly ONE reader — `/mycelium:eval-runner`, a skill that must be remembered to be invoked, which is the framework's own named contradiction of `mechanically-enforced`. A richer-fielded version of an unread log is still an unread log. **WHAT THE BUILDER ACTUALLY WANTED, asked directly**: *"if there are fields that depend on other fields, I expect them to auto-update and maybe with a notification."* That is a dependency graph with propagation, not an append-only trail — and the distinction is load-bearing: a mutation log would have faithfully recorded a parent diamond's re-specification and changed nothing about its child carrying a stale link for eight days. | — |
| P6 | **G-V14 is tagged `BLOCK`** and asserts integrity checks run in the per-change verification command, *"not only in CI"*, naming its own failure condition: *"Not satisfied by a CI step alone"* | `harness/guardrails-delivery.md` | **CLOSED v0.125.0 — AND THE ROW'S OWN FINDING WAS WRONG, which is the more useful half.** The census reported the three as appearing *"only in `.github/workflows/validate.yml`"*. **They were never CI-only.** All four fitness functions are listed in `scripts/local-gate-set.txt`, and the pre-push hook is **data-driven off that file** — it loops the lines and runs each script, naming none of them. So a grep for a script name finds only `validate.yml` and reads as CI-only, while the gate has been blocking every push all along. **A name-grep answered a question about behaviour**, which is the false-absence class this project logs most. **WHAT WAS ACTUALLY UNMET**: `scripts/gates.sh` — the command a builder runs after a change, and the closest thing to G-V14's *"project's verification command"* — ran **four** gates against **fifteen** in the set, and read a different list from CI and pre-push. v0.125.0 drives it off the same single source. `check_coverage_floor` is DEFERRED there with its reason printed, because its input is a fresh `coverage.json` that only the pytest-with-coverage step produces; it still blocks at pre-push and in CI. Found by the 2026-08-23 rule census, corrected 2026-08-24 by reading the hook instead of grepping for names | The three run in the pre-push/verification command, OR G-V14 is re-tiered to `REVIEW` with the CI-only reality stated. **`BLOCK` is the only tier claiming mechanical prevention, so an unmet `BLOCK` is worse than an honest `REVIEW`** |

**P5's SUCCESSOR IS NOT A REGISTRY ROW, DELIBERATELY.** Closing P5 removes a promise; it does not
create one. The replacement idea — **declared field-level dependencies with staleness detection that
fires unasked** — is NOT entered here, because this registry tracks prose that claims a mechanism
exists, and no framework prose claims that one does. Recording it here would manufacture the exact
defect the registry exists to catch. **The framework already has one working edge of that graph**:
`derived_from_hash` on `purpose_properties` declares a dependency, detects staleness mechanically,
marks every stance below superseded, and fires at the next transition. It appears in three files and
covers one relationship; there is no general `depends_on` anywhere in the plugin. **Generalising it is
a design question with an owner and no deadline, not an unmet promise.**
