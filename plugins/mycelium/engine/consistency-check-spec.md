...
| P2 | WIP limits are enforced (hard ceiling per scale) | `engine/diamond-rules.md` § WIP Limits | OPEN — prose-only; nothing blocks an over-ceiling spawn | Either a spawn-time gate in `/diamond-progress` step 9 (count active diamonds per scale vs ceiling, block + suggest park candidates) OR re-word the doc to "advisory limits" with a `Gated by:` on enforcement |
| P3 | `/ice-score` checks corrections.md for DORA calibration data before scoring ease | `engine/feedback-loops.md` § DORA → Feasibility | OPEN — ice-score has no such step | Add the step to ice-score (read corrections for "DORA calibration:" entries matching the solution shape) OR mark the loop aspirational in feedback-loops.md |
| P4 | Canvas-sync conflict resolution: "the person with more evidence wins" | `canvas-sync/SKILL.md` § Conflict Resolution | OPEN — no tooling evaluates evidence weight; humans resolve manually | Either a `/canvas-merge` procedure (read both versions, compare provenance depth on overlapping entries, recommend) OR re-word as a manual-resolution heuristic, which is what it operationally is |
