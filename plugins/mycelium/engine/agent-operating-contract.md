...
12. **End a framework turn on one next action (Downe P10, "have no dead ends").** Any turn in which a Mycelium skill ran ends with exactly one line beginning `Next:` — a skill to run, a question to answer, or `Next: nothing until <event>`. One line, last, then stop. Enforced on Stop by `${CLAUDE_PLUGIN_ROOT}/hooks/next-action-check.sh`, which blocks the turn end until the line exists (person override: `MYCELIUM_NEXT_ACTION_CHECK=off`). Ordinary answers with no skill run need no such line: a framework state is what must not dead-end, not every reply.

13. **What the user will act on later goes in a file; what you cannot see, you do not deny (v0.244.0).** A session ends and its chat is gone; the next session starts blind to it. So: (a) anything you hand the user to take away and use after this session — a call or interview script, a message to send, a capture template, a checklist, an answer they asked you to record — is **written to a file in the project when you produce it** (the file a skill names, else `.claude/handoffs/<YYYY-MM-DD>-<slug>.md`), and you say where. Showing it only in the reply loses it. (b) When the user refers to earlier work you cannot find on disk, **say that you cannot see earlier sessions' conversations and ask them to paste it**. Never state that the work was not done or never existed: that is an absence claim about a conversation you have no access to. Found in end-to-end dogfood runs on the installed plugin: a call script drafted in one session was asked for in the next, and the new session answered "I never wrote one"; later, "I have no record of your earlier backup question".

14. **Nothing meets real people before its cycle is in Deliver (v0.246.0).** Before you deploy, publish, send to real people, or tell the user to ("pull and restart", "send the links", "go live"), the diamond carrying the work must be in **Deliver** with **Security, Privacy and Service Quality passed** (`scripts/scale_locks.py --exposure-state`). If it is not, say so in one line and run the gates first (`/mycelium:diamond-progress`, which calls `/mycelium:threat-model`, `/mycelium:security-review`, `/mycelium:privacy-check`). A pilot or prototype is not exempt: anything user-facing that holds data can let strangers in or leak it out. The exposure gate blocks the deploys you run; this rule is the only guard on the ones the user runs.
