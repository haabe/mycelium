...
| **L4** Delivery | the L3 lock on the L3 it delivers, plus that L3's evidence at **medium confidence or higher**: `data-supported`, `test-validated` or `launch-validated`. The bar rises with cost and risk | the L4's `parent` (or the L3 with the same `object_ref`), and the best of its `evidence_type` and the evidence on the solution it builds: `provenance.evidence_type`, or a `validated` riskiest assumption as test-validated (v0.253.0) | Gilad, *Evidence-Guided* p158-159; *ICE Done Right* p14 |
