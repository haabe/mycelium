...
| **L2** Opportunity | the L1 lock, plus **a strategy** (v0.247.0): a live L1 diamond on its strategic decision, a North Star, and the landscape the choice is made against; and a desired outcome, derived from that strategy, naming the North Star it serves. A learning goal is enough to start | a live L1 with `object_ref`; `north-star.yml` `metric`; `landscape.yml` `components`; `opportunities.yml` `desired_outcome.metric` and `north_star_input_ref` | Gilad, *Evidence-Guided* p43; Wardley p12; Cagan, *Empowered* p113-118; Torres, *CDH* p27, p44-47 |
| **L3** Solution | the L2 lock, plus a chosen target opportunity with evidence behind it (anecdotal or better, with the source it came from), worked in **a live L2 diamond on that opportunity** (v0.247.0). The choice is reversible | the diamond's `object_ref` (or its L2 parent's) resolves to an opportunity with `provenance.evidence_type` and `evidence_sources`; at entry it must be open and, with several roots, name one in `rolls_up_to` | Torres p101-107; Gilad, *Testing Product Ideas* p9 |
| **L4** Delivery | the L3 lock on the L3 it delivers, plus **what the delivery tests** (v0.256.0): the L3's riskiest assumption with a named test, not recorded as failed. The L4 is the delivery, so it cannot wait on a result that only a delivery produces; an assumption that only real use can answer is answered by the L4 delivering to the people its test needs, through the same Security, Privacy and Service Quality gates as any delivery | the L4's `parent` (or the L3 with the same `object_ref`), and `riskiest_assumption.cheapest_test` (or `test_design`) on the solution it builds, with no failed `verdict` | Gilad, *Testing Product Ideas* (early adopters, alpha and beta are tests run by delivering) |
| **L5** Market | the L4 lock on its parent L4, that L4 shipped, launch data (usage, feedback or movement in the target metric), and **the L3's evidence at medium confidence or higher** (`data-supported`, `test-validated` or `launch-validated`, the best of its own `evidence_type` and the evidence on the solution it builds; a `validated` riskiest assumption counts as test-validated). The L4's delivery produces it; the bar rises with cost and risk | the L5's `parent`, `launch_data` on that L4 or the L5, and the L3 below it | Gilad, *Evidence-Guided* p123, p158-159; *ICE Done Right* p14 |
...
**Artefacts at L1-L4, a confidence band only at L5, and why.** Every source read orders the levels top-down (Wardley p12, Cagan p113-118, Torres p27, Gilad *Evidence-Guided* p43), and none locks the upper levels on a score: Torres works the tree "bottom-up … evolving the entire tree at once" (p35) and has strategy emerge from discovery decisions (p101); Wardley's purpose changes as you act (p32). Gilad's numbers are for ideas only, and he calls the ICE scores themselves "meaningless — they're just a way to compare ideas" (*ICE Done Right* p4). So the lock at L1-L4 is that the parent's artefact EXISTS (at L4: the test the delivery carries), and the only numeric lock is the Confidence band where Gilad puts it, before the release to the market. Until v0.256.0 the band sat on the L4, and an assumption that only real use can answer could never reach it: E2E run 29 froze a live trial at a pilot site as its test, which needed the build live, which needed the L4. Gilad's own tests include early adopters, alpha and beta, which are deliveries to test users; that is what an L4 is. ICE ranks; it never unlocks.
...
- L3 spawns L4 when an increment is ready to deliver and the L3 has named the test its delivery answers (the event offer: `/preflight`; since v0.254.0 the next item proposes it as soon as the L4 lock holds; since v0.256.0 the lock no longer waits on the test's result, which only the delivery can produce)
