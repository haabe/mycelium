...
- **A record is not a cycle, and this skill offers the door between them (v0.217.0).** After an opportunity is scored and evidence-backed, ask whether to open an L2 diamond on it (`/mycelium:diamond-progress` spawn shape, `object_ref` naming the opportunity id), and say so in one line if not: "opp-NNN stays a record". **Offer it only when the L2 entry lock holds** (`engine/diamond-rules.md`, Entry locks, v0.245.0): a stated purpose, a strategy (a live L1 diamond on its decision, a North Star, the landscape; v0.247.0) and a desired outcome, the root of this tree, naming the North Star it serves. If the strategy is missing, the next step is L1, not this tree. Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/scale_locks.py" --can-open L2 --object-ref <opp-id>`; if it prints what is missing, say so in one line and offer the step that produces it (the desired outcome is set in this skill) instead of the L2. The parent's PHASE is not the lock: an L0 still in discover with a stated purpose does not hold the door shut, and withholding the offer on the parent's phase re-closes the door this line exists to open. Until 0.217.0 the only instruction that created a child diamond sat under `diamond-progress` step 9, "If progressing", so a stuck parent closed the only door through which every scale below it could begin; a dogfood canvas reached 73 opportunity records against one L2 diamond ever, and 54 solutions against none at L3. `check_scale_occupancy.py` prints the count; `diamond-assess` reads it.
