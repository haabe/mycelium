...
- **Scoring is the last step before a cycle, not a filing (v0.217.0; condition repaired v0.242.4).** **After the leaves are scored, ask whether to open an L3 diamond on the highest-ranked one** (`object_ref` = the solution id), and say so in one line if not: "sol-NNN stays a record". The human's answer IS the selection — there is no prior state to reach.
  **Offer it only when the L3 entry lock holds** (`engine/diamond-rules.md`, Entry locks, v0.245.0): a stated purpose, a desired outcome, and the leaf's opportunity open with evidence behind it (anecdotal or better). Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/scale_locks.py" --can-open L3 --object-ref <sol-id>`; if it prints what is missing, name it in one line and offer the step that produces it instead of the L3. **The lock is the parent's ARTEFACT, not its phase or its open gates**: an L0 with Evidence, BVSSH and Privacy still open does not hold this door shut if the purpose, the outcome and the opportunity's evidence exist. (A dogfood full-ladder run, 2026-09-23, withheld the offer on the L0's open gates; v0.242.5 then removed every parent condition, which let an L3 open on nothing. Both were wrong in opposite directions.) **ICE ranks the leaves; it never unlocks one** — Gilad, *ICE Done Right* p4: the scores are "just a way to compare ideas".
  **WHY THE CONDITION CHANGED, because the old one could not be met.** Until v0.242.4 this read *"When a leaf is `selected`"*, and **nothing in the shipped plugin could set that**: `opportunities[].solutions[]` carries four properties (`minted_by`, `provenance`, `purpose_stance`, `shipped_at`) and no status field, the only status enum in the file belongs to the opportunity (`open`/`closed`/`resolved`/`addressed`/`discarded`), and a grep of every `.md`, `.yml` and `.json` in the plugin found the word in three places, all describing this gate and none setting it. **A door gated on a state with no writer is a door that never opens**, which is what a consumer's canvas reported as 53 solution records and no L3 diamond ever — the exact intake-with-no-outlet this door was added to fix. Found by a full-ladder dogfood run whose `/ice-score` pass ranked six solutions, recommended one, and correctly declined to open anything.
  The new condition mirrors the sibling door that works: `/ost-builder` offers an L2 "after an opportunity is scored and evidence-backed" — properties of the record, checkable on sight.
## Canvas Output
