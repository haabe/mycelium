...
- **Scoring is the last step before a cycle, not a filing (v0.217.0; condition repaired v0.242.4).** **After the leaves are scored, ask whether to open an L3 diamond on the highest-ranked one** (`object_ref` = the solution id), and say so in one line if not: "sol-NNN stays a record". The human's answer IS the selection — there is no prior state to reach.
  **Offer it whether or not the parent diamond has progressed** (`engine/diamond-rules.md`): diamonds progress at their own speed, so an unvalidated parent neither forbids the cycle nor holds it back (v0.243.0). **Do not withhold the offer because the parent's gates are open** — that is the exact failure this door exists to remove. v0.217.0 added it because *"a stuck parent closed the only door"*; an agent that keeps the door shut because the parent is stuck re-closes it. Measured on a dogfood full-ladder run (2026-09-23): the offer was made, then WITHHELD on the grounds that the L0 parent had Evidence, BVSSH and Privacy open — the rule that permits it lived only in `diamond-rules.md`, never on this page. If the parent's state should shape the cycle, say so in the offer; it is not a reason to skip it.
