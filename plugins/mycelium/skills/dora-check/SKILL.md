...
- Raw artifact (PRIMARY — the live instrument): scored battery result/aggregate JSONs under `.claude/evals/results/`. Each scenario-result carries `{passed, expected_to_fail, invalid, token_usage, score}`; aggregate files are JSON arrays of scenario-results, single-scenario files are one object. These are the actual eval-runner / battery outputs and reflect real model behaviour, which `pass-history.json` (a derived counter) often does not.
