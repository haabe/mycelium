...
4. **The typical flow from here**:
   ...
   - Each scale opens on what its parent has established (`engine/diamond-rules.md`, Entry locks): L1 on the purpose this interview states, L2 on a desired outcome, L3 on a target opportunity with evidence, L4 on an L3 at medium confidence. solo_hobby skips the L1 diamond, not the lock: the L2 still needs the desired outcome
...
1. **Build the chain to the first L3, in the smallest steps, then open it.** Don't ask whether to track the work — do it. The interview leaves a purpose and nothing else, and an L3 opens only on what its parents have established (`engine/diamond-rules.md`, Entry locks, v0.245.0): **(a)** the desired outcome — who to reach, doing what — ASKED of the user in one question, never inferred from the brief (the rule above against writing `opportunities.yml` from the brief alone still holds), and written as one line in `opportunities.yml` `desired_outcome.metric` (a learning goal is enough, Torres p46); **(b)** the opportunity the first build serves, with the evidence the user already has and where it came from (`provenance.evidence_type` and `evidence_sources`: their own past instances count as anecdotal, named as such; a guess is speculation and does not open the door, and a source the user never gave must not be written); **(c)** the L3 with `object_ref` naming that opportunity, phase Discover, confidence 0.15. `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/scale_locks.py" --can-open L3 --object-ref <opp-id>` says what is still missing. **Then the phase follows the work (v0.246.0):** initialise the L3's `theory_gates_status` with the L3 gate set, not the L0 one; progress it to **Develop** (`/mycelium:diamond-progress`: Four Risks, Privacy) before the first source file, and to **Deliver** (Security, Privacy, Service Quality) before anything meets real people, a pilot included. Say plainly why: building before the opportunity has any evidence is building the wrong thing fast. **Enforced:** the discovery gate refuses new source files until an L3 whose chain holds is open, and the scale-lock gate refuses the L3 itself before its parents are ready. An end-to-end dogfood run had the old one-line rule in context and shipped a whole release under an L0 in discover.
