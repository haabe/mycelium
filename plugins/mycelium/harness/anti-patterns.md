...
- **Catalogued sub-classes** (8+ instances across both repos as of 2026-05-23 — full instance log in `.claude/memory/cluster-instances.md#consistency-as-evidence`):
  ...
  - **(i) source-analysis-narrated-as-runtime-verification** *(graduated 2026-06-15)* — agent authors a RUNNABLE artifact (script / plugin / hook / config / command) from source / spec / dev-branch reads, tags it "source-verified", and treats that as sufficient to claim it works / ships — conflating "I read the source and it should work" with "I ran it and it does." Worked failure 2026-06-15: the Mycelium→opencode scaffold, authored from dev-branch source analysis and tagged "source-verified", carried 3 ship-blocking bugs (config rejected by opencode's schema, provider unresolved, `chat.message` hook crashed every run) caught ONLY when a user-prompted runtime test ran it on opencode 1.17.7 — the safety came from a user prompt, not a gate. **Mechanism (shipped):** `G-V13` (guardrails-delivery) — a runnable-artifact works/ships claim needs a runtime existence-proof (`Ran: <cmd> → <result>`) or an explicit "source-verified, untested" downgrade; plus the runnable-artifact tier of the verification-surface rule (communication-rules.md). Sibling of (e) trust-without-verification and (h) write-narration-verification: "source-verified" only proves you READ it, as "validator passed" only proves it RAN. Generalizes the existence-proof rule from measurements to artifacts; detection-mechanization deferred (no clean signal for "did you run it").
...
### 9. Fail-Open on Absent Input
- **Description**: A component that consumes another component's output treats *absence or corruption of that output* as a benign default instead of a distinct failure type — and downstream layers then score, summarize, or act on the phantom as if it were a real measurement. The shape: an action is expected at T, nothing arrives, the system proceeds with defaults/empty-state at T+1, and the absence is laundered into a plausible-looking result. Distinct from Stale State Read (#8) — there the state is real but the wrong version; here the state never existed. The cross-cutting principle: **scores require existence proofs** — a verdict, score, or pass/fail must carry evidence that the work it grades actually ran.
- **Detection rule**: A score/verdict/summary is emitted with no proof-of-work (a zero-token run scored, untouched fixture state graded, "hooks passed" on a run where no hook executed). Or, at the read site: an `except → default` / `2>/dev/null` chain around a state read where a parse failure or missing file silently degrades to a plausible default instead of failing loud. Any place where "input absent" and "input says the benign thing" produce the same downstream behavior.
- **What to do instead**: Type absence as its own outcome (`INVALID`, not `FAIL`/`PASS`). A pass/fail verdict must carry a nonzero work-evidence field (tokens spent, files touched, process exit proof); a verdict without it is INVALID, not a result. State reads fail loud on parse error / missing file rather than defaulting. CI guards reject zero-work runs before they archive. When a component produces an error signal, verify it is actually consumed — the abort that was never wired is dead code.
- **Graduation status**: prose-graduated 2026-06-15 at 6 instances in 5 days (densest cluster logged in the dogfood repo; full log `mycelium-roadmap/.claude/memory/cluster-instances.md#fail-open-on-absent-input`). Subclass mechanisms already shipped for the most expensive case, scoring-of-absent-work: runner dead-CLI tagging, orchestrator INVALID verdict typing, CI zero-token guard, abort-wiring fixture tests (2026-06-12). The cross-cutting *mechanism* — an evaluator-layer rule that every verdict carries a work-evidence field, OR a `/framework-health` check flagging `except→default→2>/dev/null` chains around state reads — is **deliberately held** until the next instance appears in a NEW surface (outside auto-dogfood/eval), the cluster's pre-registered graduation trigger. Prose-naming now (well past the ≥3 bar); mechanism on trigger.
- **Source**: Mycelium recurrence — `cluster-instances.md#fail-open-on-absent-input` (6 instances 2026-06-08…06-12, four conspiring in the workflow-#96 phantom-baseline chain). Related: Stale State Read (#8); Ohno (jidoka — fail visibly at the source); POSIX exit-code discipline (fail loud, not silent).
