...
**The `anecdotal`/0.3 ceiling is calibrated to human-research methods and applies to them** (v0.213.0). One interview is one person's report, so a single human-research method caps at anecdotal. A single technical method is labelled by REPRODUCIBILITY instead: a defect reproduced by a controlled experiment, a blind replication or an internal measurement is validated by the reproduction, however many methods produced it, and forcing it down to anecdotal would mislabel in the other direction (G-D4: the label matches the coverage). `artifact_forensics` and `independent_report` read what an artifact or a third party SAYS and stay under the ceiling. Found on the dogfood canvas 2026-08-31: a CI-reproduced path leak at `test-validated` scored as a G-D2 violation, and a source pair reading `[artifact_forensics]` beside `[internal_measurement]` scored as one method because the second tag was not on the list.
