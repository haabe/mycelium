...
- **Von Restorff (isolation)** — the distinctive item is the one remembered. A **blocker or gate must visually *pop*** (`ON HOLD (pending X)`, `Gated by:`), never blend into prose — otherwise the one line meant to stop the user scrolls past unseen. This governs *static* skill output templates too: a `## Output` block that renders a gate verdict (`Gate: [Pass/Fail]`) must instruct the agent to render the **Fail** case distinctly (a leading `BLOCKED:`/`Blocking:` line or bold `**FAIL**`), not as a value indistinguishable from `Pass`. `/framework-health` step 4e audits templates for this; the canonical fixed examples are `skills/regulatory-review` and `skills/diamond-assess`.
