...
*Source: OpenAI Codex `/goal` continuation template (2026-04). **OWASP Top 10 for LLM Applications 2025**, LLM01 *Prompt Injection* (genai.owasp.org/llm-top-10/) — canonical taxonomy for the threat class; LLM02 *Sensitive Information Disclosure*, LLM05 *Improper Output Handling*, and LLM06 *Excessive Agency* are the other three load-bearing entries for an agentic developer-tool harness. **OWASP Agentic AI Threats and Mitigations** (Feb 2025, genai.owasp.org/resource/agentic-ai-threats-and-mitigations/) adds T1 *Memory Poisoning* (direct hit on `memory/corrections.md`, `memory/patterns.md`, decision-log; addressed in `${CLAUDE_PLUGIN_ROOT}/hooks/session-start.sh` Check 7 since v0.23.6), T2 *Tool Misuse*, sub-agent escalation, and T15 *Human Manipulation* as agentic-specific threats beyond the LLM-only Top 10. **Anthropic prompt-injection defenses** (anthropic.com/research/prompt-injection-defenses, Feb 2026) — substrate-layer RL-trained refusal of embedded instructions; reports ~1% attack success on Opus 4.5 for browser agents. The tag-wrapping convention here is folklore-grade alone (per Microsoft Research **Spotlighting**, arXiv 2403.14720 — three techniques: delimiting, datamarking, encoding); it compounds with the substrate-layer defense rather than replacing it. Not a substitute for input validation at trust boundaries — defense in depth.*
