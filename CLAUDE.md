...
*Version 0.49.3 -- **Attribution label: opencode-model-toolcalling-guidance-2026-06-16** (deep-dive after the 14b value-test: runtime-isolated that the opencode local-model failure is NOT hardware/size/provider-path — it's a model-specific Ollama tool-call TEMPLATE gap. llama3.1:8b emits structured tool_calls at 8B; stock qwen2.5-coder fails at any size). Corrects the now-falsified model guidance: `docs/integrations/opencode.md` reframed tool-calling-first (template support + `num_ctx` > size; llama3.1 verified, qwen2.5-coder known-broken), scaffold default model → `llama3.1:8b`, new `check-tool-calling.py` diagnostic (verified PASS/FAIL), README model+context notes. Retracts the earlier "qwen2.5-coder 14b/32b sweet spot" claim. Doc + scaffold fix. **PATCH**. **Priors**: v0.49.2 **runtime-verification-before-ship**, v0.49.1 **opencode-clone-onboarding-fix** (see `docs/changelog.md`).*
