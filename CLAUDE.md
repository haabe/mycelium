...
*Version 0.118.0 -- **Attribution label: tasks-that-cannot-be-closed-2026-08-21**. **A NEW WARN-TIER CHECK: an open human-task carrying no closure criterion cannot be closed on evidence, only abandoned by neglect.** Found by a founder reading his own open list by eye, not by any check — five open tasks had no `success_criteria`, no `pre_registered_outcomes`, no `scoring_rules` and no `stop_condition`, and two had no horizon either, so nothing would ever prompt a look. The same sweep closed three tasks whose outcomes had been recorded days earlier and left open regardless. **WARN AND NEVER FAIL, DELIBERATELY**: downstream projects carry tasks created before the check existed and a hard failure would break their CI for a defect they did not introduce. It surfaces inherited debt without punishing it. **15 tests**, including that a retro-fitted bar still counts and that closed tasks are never flagged.*
