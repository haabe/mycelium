...
*Version 0.155.1 -- **Attribution label: the-reconcile-check-shipped-with-a-false-positive-generator-2026-09-01** (PATCH). The v0.155.0 extractor could not read the file it was built for. It broke on a list whose items share the key's indent (`cycles:` and `- cycle_id:` both at column 0, which is how cycle-history.yml is written), returning an EMPTY SET so every id read as missing; and a `#` comment banner at the key's indent also ended the block, so a comment written in the same commit as the new rows hid them. Both covered by tests, plus one asserting a real sibling key still ends the block. **A broken comparator fails LOUD, not quiet** -- an empty set reports everything as orphaned, which reads as a dramatic finding; it was caught only because the number contradicted something known.*
