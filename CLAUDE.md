...
*Version 0.39.12 -- **Attribution label: get-started-c001-hygiene-2026-06-05**. `docs/get-started.md` voice pass: five em-dash replacements (C-001 hygiene) plus one bureaucratic-phrasing tighten. L8 rhetorical em-dash split to sentence-break; L12 and L31 section-header parentheticals to comma; L58 and L59 link-title em-dashes to colon; L35 "remains available for projects that prefer not to" to "is still there for projects that would rather not". File is procedural install register, not pitch prose, so Layer 4 cadence work was minimal. Companion to v0.39.11 README sweep; docs/ survey concluded the rest of docs/ already reads in voice and does not warrant a full pass. **PATCH** (docs only, no skill or engine or hook changes). **Prior**: readme-ai-aesthetic-prose-sweep-2026-06-05 (v0.39.11); log-evidence-untracked-channel-backfill-2026-06-05 (v0.39.10).*
