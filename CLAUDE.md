...
*Version 0.41.2 -- **Attribution label: autonomous-mode-tier-caveat-2026-06-11** (lived-friction-triggered). Hardening patch ahead of the cross-model test: `engine/autonomous-mode.md` §Evidence-integrity boundary gains an explicit model-tier caveat — the boundary ships as prose with zero enforcement (nothing blocks an `external_*` tag, `evidence_type` upgrade, or `validated: true` flip in an autonomous run), verified to hold only at n=1 (Fable 5). Until the cross-model test settles downward transfer (`.claude/evals/assumption-tests/cross-model-evidence-boundary.md`; opp-011 assumption #2 = designed, not run), prefer a present human or a tier ≥ Sonnet for autonomous runs on weaker models; if that test invalidates, the boundary graduates from prose to a hook/validator guardrail. **PATCH** (engine-doc caveat line; no new convention; closes the one corruption-risk gap that ships otherwise). **Prior** (v0.41.1): **autonomous-mode-stage3-permission-verdict-2026-06-11** (lived-friction-triggered).*
