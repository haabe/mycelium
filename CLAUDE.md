...
*Version 0.41.6 -- **Attribution label: github-adapter-stargazer-dates-2026-06-11** (lived-friction-triggered). GitHub metrics adapter gains OPTIONAL date-only `starred_at` capture: a `stargazer_dates.in_window` map of per-date star-landing counts, so a pull can check whether new stars fell inside a known event window (post/launch/outreach date) instead of only seeing a bare count delta. Privacy HARD RULE: bucket to dates, NEVER persist usernames (third-party PII stays out of committed snapshots; identity lookups are ad-hoc live only). Cost guard: pull only when stars ≤ 1000. New delta rule flags in-window landings; date-alignment sharpens attribution but stays consistency_only on cause (a date is not a source). Motivated by recurring Step-10 friction (every star increment marked consistency_only for want of dates) + the live ht-026 Discord-funnel question. Additive + backward-compatible; old snapshots simply lack the field. **PATCH** (reference-adapter capability + privacy rule; no schema-validation or skill change). **Prior** (v0.41.5): **human-tasks-outreach-type-2026-06-11** (lived-friction-triggered).*
