...
*Version 0.31.12 -- **Attribution label: audit-medium-group**. Third audit fix batch (Medium), scoped by risk. (M1) `gate.sh` parses `tool_name` + `file_path` in a single NUL-separated `python3` spawn instead of two — this hook is on the Write/Edit hot path, so it drops one interpreter startup per gated edit; secret-detection and path-gating behavior unchanged (verified across non-source / benign-source / planted-secret cases). (M2) CI `setup-python` gains `cache: 'pip'` keyed on `requirements-ci.txt`. Deliberately DEFERRED as needing their own design (logged in `decision-log.md`): manifest / skill-count / version auto-generation (large, miswiring risk). Left as-is by design (not bugs): Check 33 CI fail-open (documented private-registry rationale) and Check 17 as the single source of truth for ruff/shellcheck/pytest. **PATCH** (hot-path + CI perf; no behavioral-contract, canvas, or schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: audit-high-group (v0.31.11), hook-dual-path-hardening (v0.31.10).*
