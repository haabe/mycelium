...
*Version 0.23.38 -- **Attribution label: maintenance-housekeeping**. Auto-dogfood architectural decision (continued from v0.23.37): reinstatement target = roadmap-private (`mycelium-roadmap/.claude/auto-dogfood/`), NOT framework. v0.23.37 commit caught by Check 28 (manifest dual-source byte-match): the two manifests had different rationale comments in their `auto-dogfood` removal block. Resolved by syncing `.claude/manifest.yml` from canonical `plugins/mycelium/manifest.yml` per the validator's own remediation hint. Same shape as the v0.23.34 sync-trap that originated the atomic-commit rule (v0.23.35) — except this time the trap was caught BEFORE push by Check 28 rather than after by Check 30/26. Lesson: when editing both `.claude/manifest.yml` and `plugins/mycelium/manifest.yml`, ALWAYS sync the legacy copy from the canonical AFTER editing the canonical, never compose two independent edits. Worth flagging as a candidate for corrections.md graduation review (sister to the v0.23.35 plugin.json sync-lag pattern). PATCH per version-discipline; atomic-commit rule applied.*
