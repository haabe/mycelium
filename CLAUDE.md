...
*Version 0.146.0 -- **Attribution label: the-remaining-five-consumer-findings-2026-08-31** (MINOR, consumer-reported). The five findings that needed a decision, all built. **F2: the absence-claim guard measured 29 fires lifetime, 16 in one day, and ZERO catches of four confirmed errors** -- the SECOND lexical detector to measure zero after devils-advocate Technique 5; its warning now carries that number and says a fire is not coverage. Not removed: whether it deters errors never made is unfalsifiable from logs. **F3: `state what was searched` -> STATE THE DENOMINATOR**, because two errors were summaries of evidence in hand, not claims about a search. **F4: diamond-assess 7c now asks the owner to state the DoD FROM MEMORY before showing it** -- a DoD its owner cannot recall steers nothing and reads as satisfied to every presence check. **F5: `weight` 1-10 alongside `binding`, plus a constraint `unlock` whose `state` must be an EVENT not a mood** -- binding stays boolean as the compensation gate, because a pure weighted score lets a high total outvote a violated must. **F8: three capture-template lines + a paper POCKET CARD** as a first-class handoff output; the demand question went unasked twice and the cause was medium, not attention.*
