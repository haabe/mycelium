...
*Version 0.110.5 -- **Attribution label: the-guard-warned-people-who-had-already-applied-the-remedy-2026-08-16**. The backtick rule was a bare character search with no exclusion for the QUOTED HEREDOC its own message recommends, so it fired on commands that were already following its advice. Measured on the dogfood project: **~13 fires in a single session, correctly ignored every one**, against a sibling guard in the same family (`absence-claim-guard`) that fired 4 times and changed the text 4 times. A guard with a near-zero action rate is not merely useless — it trains the reader to skim hook output, degrading the guards that work. Quoted-heredoc BODIES are now stripped before the backtick test; unquoted `<<EOF` still warns because it still expands, and a backtick on the opener line still warns because only the body is inert. This is the same defect already fixed once in this file for PIPESTATUS — warning the author who has applied the remedy.*
