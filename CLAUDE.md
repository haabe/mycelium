...
*Version 0.48.2 -- **Attribution label: dora-firstpass-battery-locations-2026-06-15** (lived-friction-triggered: dogfood follow-up to v0.48.1 caught that the battery actually writes to `.claude/auto-dogfood/results/`, not just `.claude/evals/results/` — and that an incomplete search led to a false "battery harness doesn't exist" conclusion). `/dora-check` first_pass_success_rate now searches BOTH `.claude/evals/results/` and `.claude/auto-dogfood/results/` (most-recent across both) and adds a **locate-before-deny** rule: search both dirs in the current tree AND `git log --all` before reporting null/absence — a negative grep in one place is not absence. Skill-doc/behaviour change. **PATCH**. **Priors**: v0.48.1 **dora-firstpass-live-instrument**, v0.48.0 **framework-health-instrumentation** (see `docs/changelog.md`).*
