...
*Version 0.39.14 -- **Attribution label: check-40-sync-derived-pre-push-gate-2026-06-05**. New CI check (Check 40) gates `sync_derived.py --check` pre-push: any drift in mechanically-derived tokens (CLAUDE.md `*Version` vs plugin.json + docs/ai-system-card.md vs marketplace.json + diamond-assess/SKILL.md skill-count tokens) blocks push. Closes a real failure: docs/ai-system-card.md version token sat at 0.38.0 across six framework bumps (v0.38.1 → v0.39.13) without sync_derived.py being invoked — caught by today's /mycelium:xai-check Stage 4 re-audit when the user asked "why is the system card stale". Paired with new /canvas-health 9b sub-check (system-card content freshness vs services.yml :: svc-mycelium.xai.*) for the substantive content (audit date, eval status) that sync_derived.py doesn't cover. **PATCH** (CI gate + canvas-health sub-check; no engine/hook/skill behavior change). **Prior**: changelog-regenericize-leaked-names-2026-06-05 (v0.39.13); get-started-c001-hygiene-2026-06-05 (v0.39.12).*
