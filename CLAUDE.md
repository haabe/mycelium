...
*Version 0.49.8 -- **Attribution label: framework-guard-sync-message-harness-neutral-2026-06-18** (closes the item v0.49.7 flagged: the dogfood-sync guidance in `framework_guard.py` named `.claude/scripts/upgrade.sh` as the way to sync framework changes — wrong for any non-legacy form). Fix: both deny messages now give install-form-aware sync guidance (Claude plugin: `/plugin update`; legacy: `.claude/scripts/upgrade.sh`; opencode/vendored: re-run `/mycelium:setup`; see `docs/install-paths.md`) instead of assuming one mechanism. `/plugin update` is Claude-plugin-only; opencode re-provisions via `provision-skills.sh`; `upgrade.sh` is legacy-degit-only — no single command is universal. The guard still allowlists `bash .claude/scripts/upgrade.sh` (a true, harmless legacy permission); its docstring reworded "the legitimate" → "the legacy (allowlisted)" mechanism. Scripts. **PATCH**. **Priors**: v0.49.7 **legacy-path-rot-runtime-strings-2026-06-18**, v0.49.6 **legacy-path-rot-sweep-2026-06-18** (see `docs/changelog.md`).*
