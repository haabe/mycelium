...
*Version 0.37.1 -- **Attribution label: delegation-authority-prepush**. Pre-push remediation of the v0.37.0 commit: syncs legacy `.claude/manifest.yml` with the canonical `plugins/mycelium/manifest.yml` (the `delegation-authority.md` entry), and merges the Anti-Patterns + Cognitive Biases bullets into one line to hold the 200-line CLAUDE.md ceiling (Check 36). Zero behavior change to the delegation-authority doctrine itself. **PATCH** (manifest sync + line-budget edit; no doctrine change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: delegation-authority (v0.37.0) — execution-authority layer binding BLOCK/REVIEW/NUDGE to execution decisions keyed to consequence = effective-reversibility × aggregate-blast-radius; ruff-cleanup (v0.36.3).*
