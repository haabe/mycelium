...
*Version 0.55.1 -- **Attribution label: sinek-why-first-2026-07-01** (theory-fidelity B5). Makes Sinek "start with Why" REAL in the primary brief flow. The audit found `/interview` Q1 asked "what are you trying to build, and for whom?" (build-first) then back-labeled it as "Theory: Sinek (purpose)" in the decision-log — the exact inside-out inversion. Q1 now reads **"What are you trying to change, and for whom?"** — the *change in the world* is Sinek's Why — with `purpose.yml` populated from the change/why answer, not the build. **Blind-tested BEFORE commit** — a first reframe ("what problem are you solving") was caught as back-labeling ONE LAYER DOWN (that's Torres/Christensen's *job*, not Sinek's *belief*), plus triple-barreled under a one-sentence limit, plus presupposing a problem-frame that solution/curiosity-first builders lack. Fixed: two-clause open stem (leads with the *change*), a light follow-up that reaches the belief ("what becomes true if it works?") AND captures the build for the brief, solution-first answers pass without a forced problem-frame. Sinek stays **not hard-gated** (a communication principle, faithfully represented as elicitation-order, not a gate). **PATCH** (elicitation fidelity fix on the live interview flow). Updated: interview, theories.md. **Priors**: v0.55.0 **build-mode-gate-2026-07-01** (B1), v0.54.0 **torres-selection-authority-2026-07-01** (B4; see `docs/changelog.md`).*
