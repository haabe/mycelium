...
*Version 0.23.39 -- **Attribution label: lived-friction-triggered**. `/interview` skill enhancement closing the cold-start onboarding gap surfaced by auto-dogfood Phase 3c (roadmap-private, 2026-05-22). The onboarding-solo-cold-start scenario (non-developer persona, empty canvas) failed `jtbd_mapped` and `diamond_created` checks at 50% score. Root cause: brief flow wrote JTBD-shape into `purpose.yml` but did NOT create `jobs-to-be-done.yml`, AND the diamond was created with phase variations that didn't match the evaluator's strict scale+phase check. Fix: Step 2 of the brief flow now also writes a stub `jobs-to-be-done.yml` with all JTBD dimensions (functional populated; emotional/social/hiring/firing/opportunity_score as placeholders for `/mycelium:jtbd-map` enrichment) AND explicitly specifies `scale: L0, phase: discover` (lowercase per active.yml schema). This makes the brief flow's "10-min first value" promise structurally complete — downstream skills + auto-dogfood verification both see the JTBD canvas file existing with the right shape. Validates Known Issue #7 from original April 2026 LEARNING-STRATEGY.md ("No onboarding scenario") — the gap was real AND now closeable. Particularly relevant for the Edith-Mari first-non-developer-user signal logged 2026-05-20. PATCH per version-discipline: atomic-commit rule applied (CLAUDE.md + plugin.json + changelog + interview/SKILL.md); no schema change, no new skill, no theory change.*
