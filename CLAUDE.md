...
*Version 0.108.1 -- **Attribution label: the-working-tree-masked-what-the-widening-was-for-2026-08-08**. **Patch on v0.108.0, and the reason is the lesson.** Widening `check_doc_references` to scan `.claude/` was the point of v0.108.0 -- the consumer's content lives there. Every local run was green: 870 tests, validator 53/0, every guard. **CI failed on the first run**, on an EXISTING test, because `.claude/README.md -> drafts/` and `.claude/evals/README.md -> results/` point at directories that are gitignored or untracked -- present in the author's working tree, absent from the fresh clone CI makes. A third instance of the same shape (`worktrees/`) had been caught locally an hour earlier and was not generalised from. **Widening a guard's file scope changes what counts as its environment**: before it, tracked files only, so the working tree was a valid proxy; after it, the untracked files the working tree carries are precisely the ones that mask the defect the widening exists to find. All three allowlisted as local-only directories their READMEs correctly document. Reproduced by cloning the pushed branch to a scratch dir (1 failed / 869 passed, matching CI; 870 after the fix) -- two minutes, where the CI log gave only "pytest: tests failing" because the validator swallows pytest output.*
