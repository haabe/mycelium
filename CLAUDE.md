...
*Version 0.143.0 -- **Attribution label: the-record-of-what-was-read-was-empty-by-construction-2026-08-31**. **THE READ LOG WAS BLIND TO EVERY READ DONE THROUGH BASH, AND SESSIONS ARE ROUTINELY INSTRUCTED TO PREFER BASH.** `read-log.sh` was bound `PostToolUse matcher=Read`, so `cat`/`head`/`sed`/`grep`/`python` reads were recorded nowhere: 3,401 rows on the dogfood repo, every one from the Read tool. **THE CONSEQUENCE IS NOT BOOKKEEPING.** On 2026-08-31 an agent characterised four canvas records it had never opened -- a `user_type` field, two `breadth_note`s and a `kill_criterion.date` -- every one a field-level miss inside a document it was actively citing, all four caught by the operator and none by any check, because nothing could establish that a record was unread. The matcher is now `Read|Bash`. **AN INFERRED READ IS NOT AN OBSERVED ONE AND THE LOG SAYS SO**: a Read tool call is proof a file was opened, a path in a shell command is evidence it was consulted, and Bash rows carry `inferred: true` so a consumer can weight them rather than discovering the distinction later. **IT UNDER-REPORTS ON PURPOSE** -- a read verb must be present, so `rm` and `git add` are excluded; the path must exist on disk; it must resolve inside the project; redirect targets are stripped, because a log that lets an agent prove it read what it only deleted is worse than no log. This is the substrate for `verify_citations`, which cannot check a claim against a record of reads that was empty.*
