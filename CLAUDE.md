...
*Version 0.23.28 -- **Attribution label: user-detected**. First non-developer user test (Edith-Mari Pedersen Bartnes, book project content_publication product-type, 2026-05-20). Brief synthesis produced a near-tears recognition moment; assumption test left her feeling "really seen." Receipt logged at `docs/receipts/cases/2026-05-20-edith-mari-book-project.md`. CONTRIBUTORS.md adds v0.23.28 cycle credit. corrections.md adds "You are here" wayfinding gap recurrence — orientation mechanism previously covered initial flow only; recurs at phase transitions (assumption-test → deep-dive-interview); correction extends the surface to fire at every transition with explicit non-dev-user rationale (developers carry mental models of agent flows by default; non-developers do not). Attribution-registry (private roadmap repo) updated: Edith-Mari as `public_ok` with explicit 2026-05-20 consent. PATCH per version-discipline: new receipt + corrections entry + CONTRIBUTORS entry + version line + changelog; no schema, skill, or behaviour change.*
