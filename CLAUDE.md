...
*Version 0.127.0 -- **Attribution label: the-canvas-is-not-a-cache-it-is-a-record-of-judgements-2026-08-24**. **Mycelium had a read-before-WRITE rule on 24 of its skill files and NO read-before-RESEARCH rule anywhere** -- verified across `skills/`, `engine/`, `harness/`, `scripts/`, `hooks/` and the consumer `CLAUDE.md`. Found by the **i-productified** dogfood project, which produced two failures in one session. (1) Six searches and four fetches rebuilt a holding company's ownership that the canvas already held **in a sharper form** from a brreg sweep three weeks earlier. (2) **Minutes later, having just written down the rule against exactly this**, the agent recommended a company as *"a genuine find"* that `purpose.yml` records the founder rejecting on ethical grounds, whose rebrand he had called greenwashing -- he had to hand his own recorded position back to the agent. **THE WRITE-SIDE PREFLIGHT CANNOT COVER THIS**: the write is fine, the file is fine, the validator passes. What is lost is that **research conducted without the canvas is analysis without the constraints the canvas already recorded**. **NOT a token-efficiency check -- a CONSTRAINT-LOSS check**: instance 1 lost a better answer, instance 2 lost a DECISION. **PROSE IS ALREADY FALSIFIED** -- instance 2 happened after the prose rule existed, in the same turn, by the same agent -- so this ships as a `PreToolUse` hook on web search and fetch, WARN-never-block, failing open, registered in all three runtime manifests. **It will be noisy in a canvas naming hundreds of entities, and that is planned for rather than discovered**: every firing logs to `.claude/state/read-before-research-log.jsonl`, and `sol-048a` already commits this project to retiring a guard whose action rate stays near zero.*
