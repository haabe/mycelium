...
*Version 0.117.4 -- **Attribution label: the-project-it-talked-me-out-of-2026-08-21**. **THE README NOW MAKES ITS BEST CLAIM INSTEAD OF DESCRIBING IT.** *"Partly built from things it stopped itself"* stood unsupported for months while the evidence sat three clicks away; a blind reader called it the most interesting sentence in the file and asked which decision it killed. **It killed a macOS file viewer that never got a line of code written, and that kill produced ten of the mechanisms this project runs on.** Named at the claim and again in the lede. **AND "WHY THIS EXISTS" IS FIRST PERSON AGAIN.** It had been rewritten into assertions about what the reader has seen, carrying two invented quotes that a forensic read and the repo's own claims-about-people hook flagged the same minute. Restored from the operator's own words, dated rather than counted (since 1997, which never expires and matches what he has said publicly). **The measured PMs/CTOs bounce is fixed** with the wording recorded when it was found. One em-dash in the file; 1,808 words.*
