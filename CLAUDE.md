...
*Version 0.41.5 -- **Attribution label: human-tasks-outreach-type-2026-06-11** (lived-friction-triggered). Adds `outreach` to the `human-tasks.schema.json` task-type enum (outbound contact or channel post — DM, article, community post; often paired with pre-committed observation thresholds for the response/funnel signal). Closes a standing schema-vs-skill misalignment: both `/handoff` and `/log-evidence` descriptions already promised "outreach" while the enum omitted it, forcing outbound tasks into ill-fitting types (dogfood evidence: roadmap ht-009 → `usability_test`, ht-026 → `observation`, both with semantic-intent comments). `/handoff` type list + brief template synced. Additive + backward-compatible; existing entries stay valid. **PATCH** (schema enum addition + skill sync; no behavior change to existing types). **Prior** (v0.41.4): **claudemd-dispatcher-slim-2026-06-11** (housekeeping).*
