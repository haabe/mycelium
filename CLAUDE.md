...
*Version 0.23.29 -- **Attribution label: dogfood-audit-trail**. Diamond assessment 2026-05-20 logged to decision-log.md: L0 Purpose confidence 0.61 → 0.63 (half-weight increment absorbing Edith-Mari behavior-validation signal); L1 Strategy held at 0.24 (2026-05-12 arms-length-validation rule respected — relationship-class signal does not substitute). Includes cognitive-forcing pre-read ("might move us a little further, I hope"), explicit why-not-alternatives, anti-pattern #7 risk-engagement, system archetype watch (Shifting-the-Burden risk), reversibility framing. Honest hedging on Edith-Mari signal: real behavior-validation by the framework's own definition, but relationship-class not arms-length; arms-length cohort signal (ht-014/ht-015) remains the L1-movement prerequisite. PATCH per version-discipline: decision-log entry + this version line + changelog; no schema, skill, or behaviour change.*
