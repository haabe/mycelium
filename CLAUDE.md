...
*Version 0.111.0 -- **Attribution label: every-route-into-discovery-waits-to-be-invoked-2026-08-20**. Every route into discovery in this framework is INVOKED: `/assumption-test`, `/user-interview` and `/handoff` all wait to be called. The founder's report from dogfood is that the moment needing discovery is exactly the moment nobody recognises it — "the user might not be aware that this requires discovery in the post" — and a skill cannot fire on a case its author did not notice. `discovery-trigger-guard` is a UserPromptSubmit advisory that notices when the author states something whose truth depends on other people's wants or behaviour, and offers the cheap move first: TYPE it as an assumption, with TEST as the second step. It never blocks and never opens with a study, because a guard that asks for work on its first false positive is disabled by its user within a day. Every fire is logged so the action rate is computable, since the rule this hook must live under is that a guard with a near-zero action rate gets narrowed or retired.*
