...
*Version 0.31.5 -- **Attribution label: loop-extremity-streetlight**. Sharpens AP-MS-1 (Streetlight Effect) with a metric-gaming-via-optimization-loop detector: detection-rule clause (4) for the **loop-extremity signature** (an autonomous minimize/maximize loop driving a proxy metric out-of-distribution with no out-of-band check on the proxied property — Goodhart/Campbell under optimization pressure) plus a "verify the proxied property out-of-band" remediation and a holdout-split pointer. Scoped out a *new* anti-pattern (Goodhart already in AP-CD-3, content shape in AP-MS-1; N=0 internal → HiPPO/AP#7 risk) but the maintainer overrode the evidence-gate ("do as recommended") to ship the AP-MS-1 sharpening now rather than waiting for a lived instance. Triggered by Hashimoto's "agent psychosis" thread (external N=1, `Per user:`); Mycelium keeps the sharper Goodhart/Streetlight framing over the popular umbrella term. **PATCH per version-discipline**: single NUDGE-tier detection-rule addition to one harness file, no BLOCK/REVIEW gate added. **Attribution class**: external-signal-triggered + user-gate-override. Scoping rationale + override both in `harness/decision-log.md` 2026-05-29; changelog entry per Check 34.* **Prior**: contributor-credit-receipts (v0.31.4).
