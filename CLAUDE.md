...
*Version 0.134.0 -- **Attribution label: validate-canvas-was-blind-to-duplicate-yaml-keys-2026-08-26**. **`yaml.safe_load` resolves a duplicate mapping key by keeping the LAST value, so the earlier one is gone before any schema or trace check runs — the file parses, it just no longer says what it said.** `validate_canvas.py` now composes the node stream, which is the only layer where the duplicate is still visible, and reports it as silent data loss. **THERE IS NO BENIGN CASE IN A CANVAS FILE**: the second key always destroys the first, so a duplicate is a deletion by definition. **SURFACED BY DOGFOOD, TWO REAL INSTANCES**, both of which had passed validation, pre-push and CI: an appended touch landed between a list entry's `- date:` and its own body, duplicating two keys and emptying both entries; and an older duplicate `tested:` made a SCORED assumption test read as `null` — never-run — to every instrument consuming it. **Negative control run before shipping**: green on 25 upstream + 25 dogfood canvas files, red on a planted duplicate.*
