...
*Version 0.110.3 -- **Attribution label: the-sync-script-tried-to-make-a-true-sentence-false-2026-08-14**. `sync_derived.py` rewrote EVERY `<N> skills` token in its target files to the total skill count, on an assumption stated only in a code comment: that in those files every such token IS the total. **A CLAUDE.md version note reading "shipped in 22 skills" -- 22 of them, not the total -- was one sync away from being rewritten to "60 skills" and made false, silently, behind a gate that then reported PASS.** The script cannot distinguish a stale total from a real quantity, so it no longer guesses: when a file holds more than one distinct value it refuses to write anything, names every token with its line number, and exits 2 in both modes. A `skills-literal` line marker exempts a deliberate non-total number. Drift reports now carry line numbers, which is why nobody caught this earlier -- the old output said only "skill count -> 60".*
