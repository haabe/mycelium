...
*Version 0.39.5 -- **Attribution label: v0.39.4-validator-followup-2026-06-03**. Validator-hygiene follow-up to v0.39.4 (canvas-first-on-product-questions): plugin.json version was at 0.39.3 in the v0.39.4 commit (Check 30 version-drift), and CLAUDE.md hit 201 lines vs the 200-line dispatcher ceiling (Check 36) after the trigger-extension rationale block. This version syncs plugin.json (0.39.3→0.39.5) and relocates the v0.39.4 rationale entirely to `docs/changelog.md` (the canonical home for that detail per the dispatcher discipline). No behavior change vs v0.39.4 — the Pre-Task Protocol trigger extension shipped in v0.39.4 stands. **PATCH** (validator hygiene; no new gate, no new skill, no new file, no behavior change). **Prior**: canvas-first-on-product-questions-2026-06-03 (v0.39.4); cross-repo-stale-state-arm-2026-06-02 (v0.39.3).*
