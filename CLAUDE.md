...
*Version 0.23.36 -- **Attribution label: maintenance-housekeeping**. agentskills.io spec compliance: `instruction_budget` migrated from top-level frontmatter to `metadata.instruction_budget` across 45 SKILL.md files (4 skills never had the field; no migration needed). Discovered via comp-028 audit in roadmap landscape.yml 2026-05-22 — Mycelium frontmatter was on-spec for `name` + `description` but the `instruction_budget` custom field was at top-level rather than under the spec's `metadata:` namespace. Single non-SKILL.md doc updated: `harness/context-management.md` references the field's new location. Zero code references existed (verified by grep across .py/.sh/.md/.json); migration is YAML-only with no behaviour change. Primary-source verification of Anthropic + OpenAI harness articles also landed today (post-surgery follow-up completed early); enrichment recorded in roadmap purpose.yml. PATCH per version-discipline: atomic-commit rule from v0.23.35 applied (CLAUDE.md + plugin.json + changelog + 45 SKILL.md + context-management.md in one commit). No skill behaviour change, no schema change, no new skill, no theory change.*
