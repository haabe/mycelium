...
*Version 0.131.0 -- **Attribution label: an-unused-schema-drifts-silently-2026-08-24**. **`completed_tasks` had two of its field types BACKWARDS relative to four months of real records, in opposite directions, and nothing noticed because nothing had ever validated against it.** v0.130.0 gave the dogfood project a `completed_tasks` list to move 61 misfiled closures into; the moment they arrived, **11 schema errors fired on records that had been schema-VALID the whole time**. `evidence_logged_to` was specified `array` while all seven real records are STRINGS (a single anchor with a parenthetical); `key_findings` was specified `string` while real records are LISTS of distinct findings. **A schema with both fields inverted relative to use is not a contract anyone was writing against.** They were never checked because closures sat in `pending_tasks`, whose item schema constrains neither field -- so the drift was invisible until something finally used the list. Both now accept string-or-array. **THE DATE TYPE IS DELIBERATELY NOT LOOSENED**: `completed_at` stays a strict ISO timestamp, and the two records carrying the literal string `unknown` are fixed as DATA rather than by widening the type, because a date field that accepts prose stops being a date. **THE GENERAL LESSON, and it is the third instance of the family in one day**: a specced field with no reader, an instruction with no destination, and now a schema with no validator all fail the same way -- they report nothing, for as long as nobody uses them.*
