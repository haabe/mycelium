...
*Version 0.45.2 -- **Attribution label: chat-ux-4e-graduation-2026-06-12** (lived-friction-triggered). The `/framework-health` 4e chat-UX audit's graduation trigger fired (same skills flagged across two temporally-independent assessments, 2026-06-05 + 2026-06-12): Hick's-Law recommend-lead fixes on `/ice-score` + `/ost-builder` + `/canvas-update` output templates; Von Restorff verdict-lead fixes on `/bvssh-check` + `/dora-check` (`/canvas-health` self-resolved); **Check 45** enforces the axiom markers on the graduated five (marker-presence, Checks 41/42 pattern; new skills stay under 4e's heuristic — graduation is the only path onto the list). Same assessment: AP#7 instance #24 fixed (cycle-history aggregate, pre-mechanism append), router-discipline eval re-run PASS (baseline gap stays closed). **PATCH**. **Priors** (same day): v0.45.1 **gap-prevention-registry**, v0.45.0 **gap-analysis-mechanisms**, v0.44.2/.1/.0 (see `docs/changelog.md`).*
