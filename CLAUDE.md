...
*Version 0.23.37 -- **Attribution label: maintenance-housekeeping**. Auto-dogfood architectural decision: reinstatement target moved from framework (`plugins/mycelium/auto-dogfood/`, per the deletion-commit's original intent) to **roadmap-private** (`mycelium-roadmap/.claude/auto-dogfood/`) per founder direction 2026-05-22. Reasoning: at this stage no other Mycelium operator will plausibly auto-dogfood the framework end-to-end; the 19-scenario battery + orchestrator + persona-simulator are founder-internal validation tooling, not user-facing primitives. Framework cleanup: removed `.claude/auto-dogfood/` from both manifests (replacement, not re-population); updated `plugins/mycelium/engine/dogfood-mode.md` + `.claude/evals/dogfood-reports/README.md` to point at roadmap-repo location. Full rebuild plan at `mycelium-roadmap/.claude/auto-dogfood/REBUILD-PLAN.md` (10 sections, ~580 lines, all 5 open questions answered). Promotion to framework re-evaluated if/when a second operator demand triggers. PATCH per version-discipline: atomic-commit rule from v0.23.35 applied (CLAUDE.md + plugin.json + changelog + 2 manifests + 2 doc references); no skill behaviour change, no schema change, no new skill.*
