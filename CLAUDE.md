...
*Version 0.117.2 -- **Attribution label: the-section-that-scored-worst-2026-08-21**. **CORRECTS v0.117.1, which shipped a false sentence**: its changelog said no competing tool appears anywhere in the README. **Two do** — Paddo's *boring agents* and Addy Osmani's *agent-skills*, both named and linked in *Who it's not for*. The claim came from a section-scoped read stated as a file-scoped fact. **What is genuinely missing is narrower**: both appear as scope exclusions for when the build is already decided, and there is no comparison for the case where deciding is the hard part. **AND THE SECTION v0.117.1 ADDED TO IS THE ONE THAT SCORES WORST.** Two blind readers rated the README 6/10 and 7/10 machine-written and both named *What it feels like*, at 9/10 in the forensic read — an announced template, four identical bolded headers, four same-length scenes, invented agent dialogue. It is rewritten here rather than deleted. Also gone: a verbatim eleven-word tricolon that appeared twice, two sections apart. **The README is now shorter than before either release.***
