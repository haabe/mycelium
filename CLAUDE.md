...
*Version 0.171.0 -- **Attribution label: the-threat-model-example-could-not-validate-2026-09-03** (MINOR). The dogfood run of `/mycelium:threat-model` shipped in v0.170.0 followed the skill's own canvas example and produced a **schema-invalid file — 18 validation errors**. Three of the example's six fields were wrong: `provenance` and `trace` were shown as scalar strings where the schema `$ref`s them as objects (`provenance` requires `evidence_type` + `evidence_sources`; `trace` is `{target_id, edge}` edge lists), and `category` was documented as *"STRIDE category, or OWASP LLM id"* against a schema enum holding the six STRIDE values only. **The `or` was the design error, not the enum.** STRIDE and the OWASP LLM top-10 are orthogonal, not alternatives — an indirect prompt injection is LLM01 **and** tampering — so the field forced a false choice and, whichever way an agent resolved it, made `category` uncountable. Fixed by keeping `category` STRIDE-only, adding an **optional independent `owasp_llm`** field (LLM01-LLM10), and rewriting the example to something that validates. The `Security` gate is Required at L3-L5 and reads this file: **an example that cannot validate is a gate that cannot be fed.** Checked and NOT generalised — `service-check` and `privacy-check`, changed in the same v0.170.0 commit, use flat scalar fields with no `$ref` sub-objects and are correct as written.*
