...
*Version 0.155.0 -- **Attribution label: two-canvases-disagreed-about-whether-anything-was-killed-2026-09-01** (MINOR). `archived-solutions.yml` held THREE killed leaves (all `archived_at: 2026-08-16`, `reason: failed-assumption`, with ICE at archive and decision-log refs) while `cycle-history.yml` reported **16 launched, 0 killed** -- and NOTHING compared the two surfaces. Not a discipline failure: the discard record is strong and two leaves were falsified by their own pre-registered probes. The failure is that a reader sees a 0% discard rate and concludes nothing is ever killed -- **an agent did exactly that before opening the archive**. `check_log_reconcile.py` EXTENDED with a `CANVAS_RECONCILIATIONS` list rather than a rival script. **Its extractor's first version matched nothing and reported 'absent or empty' over three entries**, because the first field of a YAML list item sits on the `- ` line; now covered by a test that fails by assertion without it.*
