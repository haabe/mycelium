...
*Version 0.173.0 -- **Attribution label: one-unknown-name-blocked-all-names-unknown-did-not-2026-09-03** (MINOR). threat-003 from the STRIDE pass, and the defect is an inconsistency inside `render-conventions.md`'s **own** reasoning. An identifier missing from the attribution registry **blocks the whole render and emits nothing**, on the stated ground that *an unresolved consent state makes the exposure declaration impossible to give honestly*. An ABSENT REGISTRY is that identical condition applied to **every identifier at once** — and it only warned, then emitted. **The rules were inverted in severity: one unknown name produced no artifact; all names unknown produced an artifact plus a header line.** Now an absent registry BLOCKS for audience `external`, the audience where a leak becomes the violation, and keeps failing open for `founder`/`cohort` so forks and CI runners with no private companion repo can still render internally. Sharpest case was `receipt-render`, whose audience is FIXED `external`: it emitted a hand-it-over artifact behind a `do NOT share this receipt externally` header — **putting the artifact in the operator's hands and asking them not to use it**, for a skill whose own rationale says the gate must be *"conservative by construction, not by operator vigilance."* A header asking someone not to share a shareable artifact IS operator vigilance. A nudge is not a control.*
