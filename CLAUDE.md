...
*Version 0.53.0 -- **Attribution label: outcome-discovery-loop-move1-2026-07-01** (convergent-validated + theory-fidelity). Wires the back half of the product loop — delivered outcomes flowing back into discovery (Kim's Second Way; convergent across BVSSH outcome-over-output, Team Topologies value-streams, Kersten Flow/Output-to-Outcome, DORA). The loop is OPT-IN (requires a DoD `measure`) + PROMPTED (human runs `/metrics-pull`), automated only where a metric adapter exists; `session-start` surfaces opted-out + not-yet-checked shipped diamonds so the gap is never silent. Surfaced when Mik Kersten publicly suggested it AND an audit found `engine/feedback-loops.md` over-claimed "market signals flow back into discovery" while the mechanism was manual/partial. **Move 1 (research-grounded, 5 pieces reusing existing idioms):** (1) `define-done` gains an optional `measure` block (`source` incl. first-class `manual`, `field`, `target`, per-DoD `check_after` — lag matched to `kind`, not a global window — `guardrail` Goodhart counter-metric, `last_checked`); (2) `diamond-progress` stamps `completed_at` at Deliver→Complete (the ship timestamp; none existed); (3) `metrics-pull` Step 8b checks shipped diamonds' DoD target-vs-actual and drafts opportunity-routed evidence (met → confidence-up; missed → reopen-discovery), manual outcomes prompted not faked; (4) `session-start` CHECK 10 nudges when a shipped diamond's outcome-check is overdue; (5) `feedback-loops.md` rewritten honest (real closed loop for automated sources; prompted-not-automated for manual/interview outcomes). Schema documents `measure` + `completed_at` (additive). Research applied: outcomes-are-lagging, per-DoD lag, Goodhart guardrails, vanity-vs-actionable, close-the-loop-to-opportunity. **HONEST BOUND:** tightening the loop is necessary-not-sufficient — it can't manufacture outcomes without real users; does not displace the get-real-users work. **MINOR** (new mechanism across 5 files + schema). **Priors**: v0.52.0 **non-software-routing-fix-2026-06-29**, v0.51.0 **output-discipline-interface-load-2026-06-28** (see `docs/changelog.md`).*
...
**Always communicate in plain language first, technical details second.** Use `plugins/mycelium/engine/status-translations.md` for ALL user-facing output: translate diamond states, AND apply its interface-load/problem-load discipline (cut framework-facing words, keep problem-facing substance, never cut the differentiator) to everything you show the user — status, evidence write-ups, challenge results, summaries. When reporting confidence, always include: the level, the evidence type, WHY it's appropriate, and what would increase it. (Say "Confidence: Moderate -- based on 2 user interviews" not "Confidence: 0.5".)
...
**Always route through skills; the canvas is the source of truth, human docs are derived from it.** When a skill covers the task, use it (`/devils-advocate` to challenge, `/handoff` to prep an interview, `/wardley-map` to map a domain, `/log-evidence` to capture findings). Do NOT write a parallel freeform document that REINVENTS a skill's output (a hand-written "devil's advocate" file; a standalone interview guide when `/handoff` owns that). The canvas + decision-log are the source of truth; human-facing documents are *derived* from them (render fleet), not invented in parallel. **Escape valve:** if no skill covers the task, or no skill/render emits the artifact the user actually needs, you MAY write a freeform document — source its content from the canvas and flag the skill/emitter gap (upstream candidate). The prohibition is on *reinventing* an existing skill's output, not on producing artifacts the system can't yet emit. Holds for EVERY object, software or not — surfaced 2026-06-29 when a non-software project drifted into reinventing skills as `outputs/*.md` instead of routing.
