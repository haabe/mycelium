...
*Version 0.110.1 -- **Attribution label: the-indexed-field-carried-the-worst-copy-2026-08-13**. The plugin's `description` is the ONLY field the Claude Code community catalog indexes, and it is what the `/plugin` Discover pane shows and what third-party directories scrape. It read *"Theory-grounded product-thinking discipline for AI agents. 60 skills, 13 theory gates, six diamond scales"* -- an adjective and a component count -- while the README two clicks away already carried the sentence that says what the thing does. **The best copy in the project sat where a stranger arrives last, and the worst sat in the only field a stranger searches.** Rewritten operator-first. Two agent drafts were killed on a devil's-advocate pass before this one: both were written to make the reader AGREE, which is the one thing this project has never been short of and which has converted nobody, and both buried the mechanism -- the hooks -- that every practitioner who has ever engaged actually responded to. The shipped text is the operator's, and it fixes a factual overclaim the agent had introduced: the plugin will touch files in an existing repo if instructed to, so "never modifies project root files" was an absolute a first user could disprove in one session. Catalog `description` bounds are 10-2000 chars; this is 572.*
