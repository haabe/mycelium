...
*Version 0.42.3 -- **Attribution label: system-card-autonomous-disclosure-2026-06-11** (lived-friction-triggered). `/xai-check` (fifth audit) refresh of `docs/ai-system-card.md`: discloses the **autonomous operational mode** + the `autonomous-evidence-guard` across §2 Intended use (two modes; autonomous is declaration-only), §3 Model details (guard added to the hook list; the runtime is decision-maker except under a declared autonomous run), §4 Known limitations (autonomous evidence-integrity is model-dependent — enforced for the cardinal write-path, prose-only otherwise, does NOT transfer below Fable 5; operating rule + residual gaps), and §5 Explainability (autonomous operation is disclosed + gated, with a mandatory substitution ledger). Closes the Stage 4 disclosure gap a docs-consistency audit flagged — the card had drifted six patches behind on a substantive new capability. **PATCH** (system-card doc refresh; no skill/engine/hook/schema change). **Prior** (v0.42.2): **hooks-readme-guard-catchup-2026-06-11** (lived-friction-triggered).*
