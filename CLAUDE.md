...
*Version 0.39.15 -- **Attribution label: system-card-substantive-refresh-plus-coupling-sharpening-2026-06-05**. Substantive content refresh of docs/ai-system-card.md (the thing v0.39.14's 9b sub-check flagged on its first real run, as designed): §1 Last-updated stamp moved to 2026-06-05; §4 evaluation methodology rewritten to record the 2026-05-12 INSTRUMENT FAILED eval closure and the C1 mechanical-capture replacement (read-log.sh + verify_citations.py, v0.23.8, 376 reads captured to date); §5 explainability + fidelity caveat reframed around C1; §9 last-full-audit date moved to 2026-06-05 with the fourth-audit narrative + the meta-findings that drove v0.39.14's structural fixes. Plus: audience markers added to docs/context-surface.md + docs/install-paths.md (the two files /canvas-health 9b flagged as missing the markers). Plus: canvas-health 8d skill text sharpened — the actually-used coupling-tag convention `[<gap-handle> → <ref>]` is now named explicitly alongside the literal `[target → <ref>]` form, both match the same regex shape, with an added scope-narrow clause stating 8d applies only to feedback-EXTRACTION tasks (not warm-referral asks, broadcast recruits, or close-the-loop receipts). **PATCH** (docs + skill text; no engine/hook behavior change). **Prior**: check-40-sync-derived-pre-push-gate-2026-06-05 (v0.39.14); changelog-regenericize-leaked-names-2026-06-05 (v0.39.13).*
