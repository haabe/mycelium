...
*Version 0.118.2 -- **Attribution label: the-entry-outside-its-register-2026-08-21**. **A NEW WARN-TIER CHECK: an ID prefix must define entries in exactly ONE top-level section.** Two `comp-NNN` competitor entries had been appended to `out_of_scope`, which holds framework boundary entries, in a file where both lists take `- id:` items. **It survived four days and fooled the mechanism meant to catch it**: a harvest check greps the destination FILE for a token, the token matched, and the entry was recorded as landed in the register it was not in. **An entry outside its register is invisible to every count and render that reads that list, while looking present to anything that greps the file.** Measured before shipping: across 25 real canvas files every prefix already lived in one section, so zero false positives. **8 tests**, including that cross-references in prose are not definitions — the false positive that would mute it on any mature canvas.*
