...
*Version 0.158.0 -- **Attribution label: the-team-types-were-re-verified-before-the-enum-was-pinned-2026-09-01** (MINOR). Third and fourth of seven 'parse-checked only' canvases schematised: `team-shape` (the largest unvalidated body of canvas data, 61 leaves) and `bounded-contexts` (ZERO real leaves — which is the argument FOR schematising it, since first population happens later, unobserved, and nothing else checks the shape at that moment). **The enums were not inferred from the one live sample**: team-shape's four types come from the SKILL's documented contract, and bounded-contexts' seven context-map relationships and `core|supporting|generic` were already written in the template's own YAML COMMENTS, where nothing could enforce them — documented-rule-diverges-from-enforcement. **The four/three model was re-verified against primary sources before pinning** (2nd edition, Sept 2025, and the 2026 AI material): still four types, three modes, no AI-specific type from the authors; InfoQ's 'Innovation and Practices Enabling Team' is a named specialization of an existing type, not a fifth. Interaction modes deliberately NOT pinned — the site says 'Facilitation', the book says 'facilitating'. **Test-harness defect fixed**: the new tests built a bare validator with no `referencing` registry, so they could not resolve a $ref into `_common` at all — a schema could pass the suite via a path production never takes. One rule, two implementations, again. They now call `validate_canvas_against_schema` itself.*
