...
*Version 0.120.4 -- **Attribution label: the-guard-that-contradicted-its-own-comment-2026-08-23**. **`check_theory_fidelity.py` FAILED IN EVERY CONSUMER REPO FOR FOUR CONSECUTIVE ASSESSMENTS.** Its honest N/A branch fired only when auto-detect returned None, so passing `--root` explicitly — which is exactly what `/mycelium:framework-health` instructs — skipped it and hit a path error. **The guard's own comment already said exiting 2 with a path error is dishonest for a consumer; the code said otherwise whenever a root was named.** Not-a-framework-tree is now N/A and exit 0; a framework tree missing its own theory surface is still a hard error, so the teeth survive the fix. **v0.120.1 SHIPPED A RETROFIT PATH THAT WAS UNUSABLE THE MOMENT ANYONE WALKED IT.** Solutions written before `purpose_properties` existed cannot have declared a stance against them, and nothing exempted them: on the dogfood canvas that is 53 solutions x 8 binding properties = **424 findings on day one**, on the only project then able to adopt. `purpose_properties.grandfathered` is an EXPLICIT LIST of exempt solution ids — not a date comparison, because only 10 of those 53 carried any date at all and an inference would have exempted the wrong 43. **The count is printed on every run**, because an exemption nobody sees becomes the permanent state of the canvas. **A PROPERTY MUST SIT AT THE ALTITUDE WHERE VIOLATION IS MEANINGFUL.** Two independent blind extractions both read "theory-guided decisions" as a per-decision citation requirement — the words invite it — which is micromanagement, and the same project's always-injected `(per: <source>)` rule sits at 4 citations across 197 decision-log entries. `/mycelium:purpose-properties` now asks, of every candidate: what would have to be true for this to be violated, a single move or a design choice? A single move means a BEHAVIOUR rule, not a product property, and it is not recorded. `binding` limits how many properties are checked; altitude limits how granular each one is, and only the first is visible in the field. **v0.120.0 SHIPPED THE MECHANISM AND NO WAY FOR AN EXISTING PROJECT TO REACH IT** — the extraction lived in `/mycelium:start` Step 3b, and Step 4 exits for already-initialized projects, so every project created before v0.120.0 was silently excluded. `/mycelium:purpose-properties` is the retrofit AND the re-derive path for when why/how/what change and every stance below goes stale. **THE `why`/`how`/`what` NOW BIND EVERY SUB-ELEMENT BELOW THEM.** Until now nothing read their values: two scripts opened `purpose.yml` and read neither, four hooks checked whether it existed or had changed, and no theory gate named Sinek — whose model is the shape of the file. So a solution could contradict the product's own definition and pass every gate. `purpose_properties` derives binding properties from those three fields (agent-proposed, HUMAN-confirmed); every solution carries a `purpose_stance` of `preserves`/`not_applicable`/`contradicts` with a note; `check_purpose_stance.py` runs advisory in CI and `--strict` at Define->Develop and Develop->Deliver. **IT NEVER JUDGES WHETHER A SOLUTION SERVES THE PURPOSE** — that judgement is measured at 39-77%% accuracy in the published benchmarks. It checks that somebody SAID something, and silence is the finding. **An agent may not clear a `contradicts`**: without a human on the override, an agent could declare one and clear it in the same run. Quality adjectives ("secure", "accessible") are caught at the door and sent back to the builder for a definition, because a blind test on 2026-08-23 showed they yield nothing checkable. Projects without `purpose_properties` see no change.*
