...
*Version 0.140.0 -- **Attribution label: shown-not-described-2026-08-28**. **THE README NOW SHOWS THE THING RUNNING, WHICH v0.117.5 SAID COULD NOT BE WRITTEN.** That release closed by naming what three cold reads agreed on and it could not fix: *"the README describes the experience three times and shows it zero times. That cannot be written, only run."* Seven rounds of blind arms since then said the same thing every round, and every attempt to satisfy it with better prose failed. **It is fixed by a run, not a rewrite**: an unedited screen capture of `/mycelium:start` on a new idea, the brief it produced quoted verbatim, and the public repo it wrote so a reader can check the excerpt. **AND THE DEMO IS THE TOOL DISAGREEING WITH ITS AUTHOR** — the founder is in the cohort he is building for and behaves the opposite way the product needs; the brief recorded that as the most important line in the file and marked the whole thing `speculation`. **A FABRICATION WAS FOUND AND CORRECTED IN THE SAME PASS.** The founding story had said *"three days in, an agent happily building"* and named a person who wanted it; `docs/receipts/cases/2026-04-macos-fileviewer.md` says the project **never got a line of code** and was killed on a persona exercise. **The invention survived three drafts and six blind judges, none of whom could check it** — it was caught only by going to fetch the artifact. Written to `engine/audience-register.md#human_persuasive`, whose falsifier rule the file had been breaking: it now names a condition that can come back negative, and its own known weakness. 1,610 words.*
