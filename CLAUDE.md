...
*Version 0.112.1 -- **Attribution label: the-drift-report-cried-wolf-on-its-own-header-2026-08-20**. `check_instrument_contract` reported DRIFT against the first instrument retrofitted with its own contract. A `#` comment inside the YAML header is not a markdown heading, but it looks exactly like one to a block finder, and header comments on THESE files talk about predictions and freezing by definition — the matched "heading" read "# It held frozen thresholds for 102 days". Found by running the check against a real 36-file corpus rather than against its own fixtures, which is the only place it could have been found. A drift report that fires on the contract header would train its reader to ignore the one report the check exists for.*
