...
*Version 0.39.21 -- **Attribution label: faros-whiplash-receipts-voice-revise-2026-06-07**. Voice-revise pass on `docs/receipts/cases/2026-06-07-faros-whiplash-integration.md` (the case shipped in v0.39.20). Applied: personal-os voice discipline (`harness/voice-gate.md` + `context/voice-essence.md`), C-001 em-dash removal (15 → 0), C-092 voice-essence pass (bookend close added returning to title's "report fact-checks you" frame; one dry-wit ambush in close, C-007-safe), C-096 output-side anti-calque check, banned AI-tell sweep (`leverage`, `encompass` removed), three-noun balanced rhythm broken with parataxis. Substance preserved; voice observably shifts per C-029. **PATCH** (single docs/ file revised; no skill, hook, or engine behavior change). **Prior**: faros-counter-receipts-rotation-2026-06-07 (v0.39.20).*
