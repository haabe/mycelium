...
*Version 0.23.34 -- **Attribution label: dogfood-audit-trail**. Plumbing-only patch: plugin.json version-line drift caught by validator Check 30 after the 0.23.31/0.23.32/0.23.33 patch sequence on 2026-05-22 missed syncing plugin.json alongside CLAUDE.md. Three rapid PATCH-class bumps in one session created a sync trap; Check 26 then chained-failed when the fix-up commit changed plugin.json (a material framework file) without its own version bump. Resolved by bumping CLAUDE.md + plugin.json + changelog in a single coordinated commit. No theory change, no skill change, no schema change. Worth flagging for corrections.md graduation review: the validator mechanism is working (Check 30 catches the drift) but the agent-workflow needs to internalize that plugin.json sync belongs in the SAME commit as the CLAUDE.md bump, not as a fix-up.*
