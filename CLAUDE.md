...
*Version 0.152.0 -- **Attribution label: source-class-target-was-one-hop-from-an-existing-reader-2026-08-31** (MINOR). Founder-ruled "wire it properly". 20 human-tasks declared the KIND of evidence they intended to produce and NOTHING read it, while `check_source_class_fidelity.py` already read the sibling `source_class` -- the cheapest instance of this defect class: not a missing mechanism, a missing connection. What is checked is THE LOOP, not the label: a COMPLETED task declaring a target evidence class should record where that evidence landed. **15 completed tasks carried the target, 5 recorded a pointer, TEN declared an intent without ever saying whether they produced it** -- all ten fire today. Pending tasks exempt: a running task has not produced its evidence yet. 35 of 37 promise-shaped fields now have a consumer.*
