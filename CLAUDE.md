...
*Version 0.104.0 -- **Attribution label: after-a-clear-the-agent-has-nothing-2026-08-07**. **SessionStart was registered for `startup|resume` only, so after a `/clear` the agent had no canvas, no diamond state, no open loops AND no reminders** -- the one moment orientation is most needed was the one moment it was absent. SessionStart actually fires for FIVE sources: startup, resume, clear, compact, fork (verified against code.claude.com/docs/en/hooks 2026-08-07). Matcher widened to `startup|resume|clear|fork`. **compact is EXCLUDED deliberately**: it fires mid-task, possibly repeatedly, the agent still holds a summary, and PreCompact/PostCompact exist for that seam. **THE WIDENING ALONE WOULD HAVE MADE THINGS WORSE, which is the real finding.** CHECK 6 WRITES: it increments a session counter for longitudinal assumption tests, and a counter that counts HOOK FIRINGS instead of SESSIONS reports a shadow-log as more observed than it is -- inflating N, the dangerous direction. `resume` has been incrementing since v0.68.0, so the error predates this change and the widening would have multiplied it. Fix is two-part: everything PRINTED is read-only orientation and now fires on all four sources; the WRITE is skipped whenever the source is a known continuation (resume/clear/compact/fork). **Unknown source still counts** -- runtimes that send no payload (Codex CLI, manual invocation) keep working rather than silently freezing every counter, which would be the worse failure. 8 new bash tests, one per source plus no-payload and malformed-payload.*
