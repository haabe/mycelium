...
*Version 0.23.3 -- Bare-path discipline sweep (round 2) — closes gaps left by the 0.20.7 sweep. Trigger: 2026-05-10 mycelium-roadmap dogfood ran `/mycelium:metrics-pull` and the skill failed to find adapters because `metrics-pull/SKILL.md` referenced `metrics-adapters/<source>.md` as a bare path that resolves relative to user project root, not the plugin tree. Audit found two classes of gap: (1) **`metrics-adapters/` references** — 18 sites across `metrics-pull/SKILL.md`, `metrics-detect/SKILL.md`, `interview/SKILL.md`, `jit-tooling/metrics-detector.md`, and the four files inside `jit-tooling/metrics-adapters/` itself (TEMPLATE.md, github.md, GENERATING.md, active-metrics.example.yml) — none had been touched by 0.20.7's first-level sweep because the directory is a sub-directory of `jit-tooling/`, below the sweep's depth. (2) **Cross-plugin-tree refs in framework files** — 12 sites in `harness/anti-patterns.md` (engine refs), `orchestration/operations.md` + `orchestration/modes.md` (engine + project-state refs), `engine/leaf-lifecycle.md` (engine sibling refs + orchestration ref + project-state ref), `engine/feedback-loops.md` (harness + skills refs), `engine/xai-canvas-threading.md` (harness + jit-tooling + domains + status-translations refs — mix of plugin and project-state), `corrections-audit/SKILL.md` (harness refs). All routed: framework-resident → `${CLAUDE_PLUGIN_ROOT}/<dir>/`, project-state → `.claude/<dir>/`. Glob patterns in prose (`engine/*.md`, `schemas/canvas/*.schema.json` describing scan targets), GitHub-rendered markdown links in README files, and `setup/SKILL.md` AGENTS.md template body bullets (which are inside the `.claude/` section header) deliberately preserved as bare. PATCH per version-discipline (mechanical content rewrite, correctness fixes; no new mechanism). Same class as v0.20.7. Triggered by real lived-friction (the metrics-pull skill failed visibly) — same epistemic structure as the 0.20.11 plugin-form dogfood findings: subagent simulations missed what real invocation surfaced.*
