...
*Version 0.169.0 -- **Attribution label: a-seed-register-small-on-purpose-2026-09-01** (MINOR). `check_citations.py` shipped in v0.167.0 with no register for consumers to read, so it reported NOTHING WAS CHECKED on every project but the one that built it — honest and useless. `harness/do-not-cite.yml` now ships as a starter and `/mycelium:setup` copies it. **FOUR ENTRIES, AND THE SMALLNESS IS THE DESIGN**: only claims with a PUBLISHED debunking, each naming its debunker (Bossavit on the cost-of-defect curve, Jorgensen & Eveleens on Standish CHAOS rates, Cohn on the 64%-of-features n=4, Kohavi as the `qualified` example). Shipping this framework's OWN statistical judgements would make every consumer inherit any ruling that turns out wrong — and a ruling turned out wrong twice today. The Standish entry demonstrates the narrowness rule in situ: it rules on the RATES and says so, because the cause-ranking is a defensible use. Two tests assert the seed loads AND that every entry names a published debunker, so the constraint is checked rather than trusted.*
