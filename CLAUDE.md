...
*Version 0.122.0 -- **Attribution label: the-instrument-for-finding-unenforced-rules-was-itself-unenforced-2026-08-23**. **A RULE CENSUS EXTRACTED 135 FRAMEWORK RULES NAMING A COUNTABLE CORPUS** (from 468 normative sentences) and asked of each: what corpus, what detector, what compliance. **The finding is never which rules are violated -- it is which rules nobody can state a number for.** Seven drifted; two are the framework's own and ship as registry rows. **P5**: `engine/mutation-log.md` fully specifies `canvas/mutation_log.jsonl` and claims replay, audit, regression support and drift detection -- **the file exists in no project, nothing writes it, nothing reads it**, and the likely right close is to stop promising rather than build it. **P6**: **G-V14 is tagged `BLOCK`** and names its own failure condition (*"Not satisfied by a CI step alone"*) while 3 of its 4 fitness functions run only in CI. **The rows are not the point -- the registry is.** It gained nothing for ten weeks while the failure class kept occurring, because its sweep is `/framework-health` 4f: prose in a skill. So it could not tell *"nothing to add"* from *"nobody looked"*. Now a `last_swept:` marker plus **Check 54** asserts the sweep is inside the 90-day cadence -- advisory by default, `--strict` has teeth, **a future date refused as its own finding** because that is the bypass path, and absent reported separately from stale. **A general promised-artifact check was prototyped and REJECTED with numbers recorded** (61 hits broad; 3 false flags narrow, and it MISSED its own founding case). What is mechanizable is not whether a promise is kept, but whether anyone looked.*
