...
*Version 0.102.0 -- **Attribution label: prose-can-outlive-the-field-beside-it-2026-08-07**. **New advisory `check_stale_prose.py`: v0.101.0's absence-claim guard inspects a claim as it is WRITTEN and says nothing about one that was true when written and stopped being true when a sibling field moved.** Dogfood found FOUR in one session, none caught by any check -- including a task advertising a live obligation to a real person for five days while its own touch_log recorded the reply sent. Two heuristic rules: an unanchored temporal word, and a FRAMING field asserting not-done while a LOG field records it done (narrower than 'record contains both', which fired on 14 records, mostly noise). Session-start advisory, NOT a validate-template check -- that runs in the framework repo whose canvas has none of these records and would read green forever (the v0.100.0 lesson). **FIVE BUGS FOUND WHILE BUILDING IT, four by running against the real pre-fix record and one by the framework's own guard**: a flat field split made the checker MISS ITS OWN MOTIVATING CASE; it re-flagged repair notes that quote the stale phrase; `note` as both record field and touch_log child dragged log prose into framing; `direction: outbound` is not a done-marker (negative control caught it); and the first cut EXITED 0 OVER AN EMPTY REPO -- reports-green-measured-nothing, committed inside the checker written to report it. 9 tests, fixtures verbatim from the session.*
