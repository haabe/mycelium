...
*Version 0.61.1 -- **Attribution label: ruff-0-16-upgrade-2026-07-26** (completes the named v0.61.0 follow-up). Pin moved 0.15.12 -> 0.16.0 and its 59 new findings triaged three ways rather than blanket-ignored: CPY001 (43, per-file copyright) ignored as a policy choice for an MIT repo with one LICENSE; **ISC004 (9) FIXED** — unparenthesized implicit concat inside a collection is a real footgun (a missing comma silently merges two list items) and 6 of the 9 came from the previous day's refactor; PLR0917 (7, pytest fixture arity) scoped to tests/ beside its sibling PLR0913. Also removed a committed `ruff.toml` snapshot from the Check 17 fixture — it drifted on the very next policy edit and failed Check 17 inside its own coverage proof, so the harness now copies the LIVE config at run time. **Priors**: v0.61.0 **wiring-guard-2026-07-26** (see `docs/changelog.md`).*
