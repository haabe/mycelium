...
*Version 0.39.13 -- **Attribution label: changelog-regenericize-leaked-names-2026-06-05**. `docs/changelog.md` regenericization: two pre-existing leaked names (v0.39.10 dogfood-trigger paragraph; v0.36.x feedback list) replaced with generic framing per `attribution-registry.yml` consent: generic_only state. Substantive framing preserved (PM-skills comparator maintainer ~3,800★; peer-practitioner Norwegian feedback quote). Surfaced by Check 33 on first push after both entries had landed. Includes the prior v0.39.12 get-started.md C-001 hygiene pass content; this is a roll-up bump for the consent fix rather than a new functional change. **PATCH** (docs only, no skill or engine or hook changes). **Prior**: get-started-c001-hygiene-2026-06-05 (v0.39.12); readme-ai-aesthetic-prose-sweep-2026-06-05 (v0.39.11).*
