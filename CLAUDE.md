...
*Version 0.39.1 -- **Attribution label: readme-language-revision-2026-06-02**. README newcomer surface revised against N=2 peer-practitioner friction (Drew Hoskins 2026-05-11 "jargon-y"; Bård Øien 2026-06-02 "litt akademisk språk, vanskelig å umiddelbart skjønne hvorfor mycelium var produktet jeg trengte"). Replaced jargon-stack "What it is in 5 lines" (theory gates, six scales, four-phase diamond) with no-vocabulary "Your first ten minutes" scene; demoted "Where it sits in the field" academic positioning from paragraph 3 down to after "How it works". Mechanism vocabulary (scales, diamonds, gates, canvas) now first appears in "How it works", introduced after the reader has a reason to learn it. Scope deliberately limited to external-comms surface — in-product vocabulary (skill descriptions, gate copy, /interview flow) held per `opportunities.yml#opp-005` Torres-open status pending wider-N evidence on whether the in-product vocabulary IS the product. **PATCH** (doc-only README revision; no skill/gate/hook change). **Prior**: framework-health-followup-2026-06-02 (v0.39.0); architecture-discovery-narrowed-receipt (v0.38.4); mental-model-docs-page (v0.38.3); framework-health-remediation (v0.38.2).*
