...
*Version 0.49.21 -- **Attribution label: cowork-preflight-robustness-2026-06-19** (lived-friction-triggered). Hardens `hooks/preflight.sh` against two bugs surfaced by a Claude Cowork dogfood run: (1) project-dir resolution no longer trusts a bare `.`-fallback when `CLAUDE_PROJECT_DIR` is unset (some runtimes don't set it) — it walks up from `$PWD` to find `.claude/`, so an initialized project stops falsely reporting "Memory not yet initialized"; (2) the corrections counter no longer emits `0\n0` ("integer expected") on a zero-entry corrections.md. CLI behaviour unchanged (env var trusted verbatim when set); 5 resolution cases verified incl. the real Cowork project; Cowork in-runtime confirmation pending re-test. Hook fix. **PATCH**. **Priors**: v0.49.20 **release-backlog-convention-2026-06-19**, v0.49.19 **theory-fidelity-anti-drift-guard-2026-06-18** (see `docs/changelog.md`).*
