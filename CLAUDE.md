...
*Version 0.101.4 -- **Attribution label: the-gate-asked-in-this-session-not-in-this-project-2026-08-07**. **The discovery gate fired on files the project does not own; it now tests containment first.** Every check in `discovery-gate.sh` reads state from `$PROJECT_DIR`, and nothing tested whether the TARGET was inside that root -- so the trigger was session-scoped while the state was project-scoped, and a write to an unrelated file was judged by a different project's discovery. Reported from a Cowork session rooted in a Mycelium project blocking a write to `personal-os/evals/*.sh`; reproduced with both arms. **This is the other half of the 2026-07-26 fix**, which made the `.claude/` exemption project-relative and left the trigger session-scoped. **The skip-ack was the wrong remedy** and is documented as such at the code: it is permanent and project-scoped, so acking an out-of-project write would silence the gate forever for a project that never came up. Resolution is physical (`pwd -P`) on both sides so symlinks and `..` cannot walk out and still read as inside; the nearest existing ancestor is resolved since `Write` may create parents; an unresolvable root allows rather than blocks. 4 new tests, 20 assertions (was 15), incl. a negative control that fires if containment is ever written too broadly. **Caught during the work**: the new tests were first appended below the runner's `report` call, so they were defined, never invoked, and the suite printed green without them -- built-not-wired, committed inside a fix for a scoping bug, found by noticing the names were missing from the output.* **CLAUDE.md carried a rolling window of eight superseded version labels; it now carries the current one only.** The history already lives in `docs/changelog.md`, which the line below has always pointed at, so every superseded label was a second copy of a record that had a canonical home -- and a second copy is a second thing to drift. The window had no eviction rule either: it grew by whatever each release felt like leaving behind, which is how it reached eight. Nothing read them (0 files under `plugins/`, `tests/`, `.github/` reference `Superseded label` or `Prior version`), so removal is inert to every guard. **The version prose line stays** -- it is the one-glance statement of what this release changed, and it is what a fresh session reads first.*
