...
*Version 0.154.0 -- **Attribution label: the-field-wiring-gate-could-not-see-88-percent-of-fields-2026-08-31** (MINOR). `check_field_wiring` read SCHEMAS only, and `additionalProperties: true` means **2210 of 2494 live canvas keys (88%) are declared by NO schema** -- `unlocked_at` was caught in three hours only because it happened to go through one; `kill_criterion.date` did not and sat unread for months. New `--live` mode scans the canvas itself: **40 undeclared promise-shaped fields, 15 consumed, 25 not**. One-off keys excluded as prose -- 65 of 84 were used exactly once and were annotations; recurrence is what separates a field from a sentence, and without that filter the check would be muted in a day. Same baseline+fail-on-new ratchet, with the baseline in the CONSUMER's repo since canvas content is project-specific. EXTENDED the existing script rather than adding a synonym -- step (b) of the four-step rule applied to the framework's own code.*
