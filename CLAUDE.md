...
*Version 0.147.0 -- **Attribution label: a-field-nothing-reads-is-now-a-gate-2026-08-31** (MINOR). **357 schema field names, 234 with no code reader; of the 38 whose NAME asserts a machine-checkable promise, SIX had no consumer at all** -- one being `unlocked_at`, added HOURS EARLIER in the release that fixed this class. **THE FOUNDER'S RULE IS ABOUT PURPOSE:** if any writer intends a field to be read as part of the pipeline -- INCLUDING being rendered into a humane form such as a mermaid chart -- it is wired; a RENDERER IS A CONSUMER, and `human` is legal once DECLARED. Only the undeclared case fails. `unlocked_at` was WIRED not ruled: a fired unlock now releases its constraint and says so. `check_field_wiring.py --strict` runs locally and in CI with parity asserted; five fields are baselined UNRULED in `harness/field-consumers.yml`, so only a NEW unwired field fails. **The instrument laundered itself on first run** -- the registry names every unwired field and lives in a scanned directory, so it reported 38/38 wired; the exclusion is now covered by a test.*
