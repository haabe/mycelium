...
*Version 0.103.0 -- **Attribution label: a-handle-is-not-a-person-2026-08-07**. **New advisory `check_source_authenticity.py`: the evidence tiers grade evidence by WHERE IT CAME FROM, and `external_human` means a human outside the team said it -- while NOTHING ANYWHERE CHECKS WHETHER IT WAS A HUMAN.** Provenance and authenticity are different questions in a public channel. Dogfood 2026-08-07: a templated account (9 months, 10 comment karma, every comment one frame) was logged as a second practitioner arriving INDEPENDENTLY at a conclusion the framework already held; it appeared THREE TIMES in one afternoon and would have entered canvas as three convergent strangers. A second account had 26 post karma, ZERO comment karma, and one founder story reskinned across five subs with a different product placement in each. **WHY A CHECK AND NOT A NUDGE: synthetic accounts restate sensible heuristics fluently, so the contamination arrives AGREEING WITH THE THEORY, in the class this framework weights most heavily. Agreement is nearly free to manufacture; disagreement carrying receipts is not.** Rule A: external tier + public handle + no authenticity note. Rule B: the same plus a convergence claim and >=2 DISTINCT handles -- a convergence claim resting on one account is not one. Cheapest test came from a channel native, not from us: DOES THE OP REPLY, free and run on the open page. Count ACCOUNTS, not comments. **FOUR DEFECTS FOUND BY RUNNING AGAINST A LIVE CANVAS RATHER THAN A FIXTURE**: a plugin marketplace name read as a person; the founder's own handle, quoted inside INBOUND messages, flagged as a source; that self-exclusion silently missing for want of accent folding; and rule B firing on one handle. After: 2 findings on 25 files, both true, zero false positives. 10 tests. Session-start advisory, NOT validate-template (the v0.100.0 lesson). **Also fixed: backticks in a comment inside a double-quoted `python3 -c` string made every session start run `internal` as a command** -- shipping silently, and the exact trap this project's own shell-safety hook warns about.*
