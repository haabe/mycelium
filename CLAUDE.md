...
*Version 0.30.0 -- **Attribution label: cursor-codex-hook-adapters**. Two new runtime adapters land alongside Claude Code (primary) and opencode (extended, deferred): **Cursor 1.7+** and **OpenAI Codex CLI**. Both upstream hook systems are near-supersets of Claude Code's — Cursor exports `CLAUDE_PROJECT_DIR` as an alias and ships native `postToolUseFailure`; Codex clones Claude Code's PascalCase event vocabulary verbatim. Mycelium's twelve hook scripts run unmodified on both. **Shipped**: `plugins/mycelium/hooks/hooks.cursor.json` (camelCase event-name translation, `failClosed: true` on safety-critical hooks, `beforeSubmitPrompt` ≡ `UserPromptSubmit`); `plugins/mycelium/hooks/hooks.codex.json` (identical event names to Claude Code); `plugins/mycelium/hooks/codex-postfailure-shim.sh` (25-line stdin filter wiring reflexion-gate into Codex's `PostToolUse` since Codex lacks a dedicated failure event — inspects `tool_response` for `success: false / error / exit_code != 0 / is_error`, delegates to `reflexion-gate.sh` only on failure); `docs/integrations/cursor.md` and `docs/integrations/codex.md` mirroring the opencode.md structure with honest gap labeling. Verified against [cursor.com/docs/agent/hooks](https://cursor.com/docs/agent/hooks) and [developers.openai.com/codex/hooks](https://developers.openai.com/codex/hooks) 2026-05-26 — claim that the adapters work end-to-end is **consistency-only** (live runtime test not yet executed); docs invite PR-based receipts per anti-pattern #7 discipline. Calibration: opencode adapter remains deferred behind 3 upstream issues; Cursor and Codex have those primitives natively, so both ports are trivial. **MINOR per version-discipline**: new feature surface (two new hook configs + shim + two integration docs); backward-compatible (additive; no existing hook config touched; Claude Code remains primary runtime). v0.29.0 entry migrated to docs/changelog.md per Check 34.* **Prior**: metrics-pull-anomaly-devils-advocate-chain (v0.29.0).
