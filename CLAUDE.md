...
*Version 0.129.0 -- **Attribution label: five-releases-is-throughput-two-value-three-failure-is-a-measurement-2026-08-24**. **A cycle record could say how MUCH work happened and never why it existed.** The founder named the unit on 2026-08-23 -- *"most of it is work that is extra work, refactoring, bug fixing, because of missing wiring"* -- which is **Seddon's failure demand vs value demand**, and the dogfood project measured its own day at **five releases, roughly two value and three failure**. *"Five releases"* reads as throughput; *"three of five were rework"* reads as a system generating its own work, and only the second is true. Ships as `demand_type` / `demand_origin` on the cycle record, schema-enforced, with an optional `demand_cost_class` (Crosby/Juran) that may stay null. **SET AT CYCLE OPEN, NOT AT RETROSPECTIVE** -- Seddon classifies demand as it ARRIVES, origin is knowable when a cycle starts and progressively less knowable after, and a reconstructed type is reported separately and excluded from the split rather than averaged in. **NOT AN EXTENSION OF `rework`**: that block is post-delivery defects in a 14-day window, and work begun because earlier work was incomplete is not a post-delivery regression -- it is the reason the cycle exists. **AND THE CONSUMER SHIPPED IN THE SAME COMMIT AS THE FIELD, WHICH IS THE WHOLE POINT.** `gates_fired` and `regressions` were specced to close named `/framework-health` dimensions, read by nothing for months, and sat at **0 of 16** compliance; v0.121.0 wired their consumers after the fact. A new `Demand Mix` dimension reads `demand_type` from day one, reports `no-data` with a count rather than inferring from commits or transcript, and **names itself the counter-metric to release throughput** -- because a rising release count reads as productivity while the same count with a rising failure share does not.*
