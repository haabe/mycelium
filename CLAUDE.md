...
*Version 0.46.1 -- **Attribution label: readme-jargon-reduction-2026-06-13** (lived-friction-triggered: peer-practitioner academic-language friction). Reduced framework jargon across the README's "How it works" (dropped theory-gates / diamond-transition / the Cagan citation / regresses / bare L-numbers) and "Where it sits in the field" (dropped the harness-engineering / feedforward-feedback / computational-inferential lecture; kept the credibility links), plus one receipts phrase. Core model names (Scales, Diamonds, the L0–L5 graph) deliberately KEPT — whether the in-product vocabulary itself changes is the open opp-005 question, evidence-gated. Doc-only human-surface change; no schema or skill-behavior change. **PATCH**. **Priors**: v0.46.0 **scenario-primitive-falsifiable-grounded**, v0.45.2 **chat-ux-4e-graduation** (see `docs/changelog.md`).*
