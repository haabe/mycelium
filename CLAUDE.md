...
*Version 0.39.19 -- **Attribution label: faros-whiplash-plus-datadog-soae-integration-2026-06-07**. Faros "Acceleration Whiplash" 2026 + "Harness Engineering" 2026 + Datadog "State of AI Engineering" 2026 (incl. [S1]-[S5] inline source provenance) shipped as three coordinated framework changes: (B14) new skill `/mycelium:scaffold-cost-check` measures Mycelium's stable scaffold token cost (CLAUDE.md + engine + harness + canvas + memory) via byte-count÷4 heuristic; print-only default, `--write` persists to `dora-metrics.yml#apex.scaffold_token_estimate`. (B2) `/mycelium:dora-check` MANDATORY extended with Part 2b: three APEX fields (`ai_rework_rate`, `ai_acceptance_rate`, `hook_detection_rate`) now MUST be computed from raw artifacts (git log + corrections.md `detection_origin` tags), promoting the corrections.md 2026-06-02 "0% caught by hook/evaluator" finding from prose to tracked metric; Goodhart pair preserved. (B13) new orthogonal `runtime_llm: true|false` axis in `engine/canvas-guidance.yml#runtime_llm` + `/mycelium:interview` Phase 6 asks the question; precondition for the planned Runtime-LLM Harness Gate (v0.40.x) modeled on the Regulatory Gate template. **PATCH** (skill addition + canvas-guidance dimension extension + dora-check MANDATORY extension; no engine/hook behavior change). **Prior**: read-before-recommend-stage-2a-plus-chat-ux-proof-of-pattern-2026-06-05 (v0.39.18); agents-md-plugin-cache-path-fix-2026-06-05 (v0.39.17).*
