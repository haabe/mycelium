...
*Version 0.161.0 -- **Attribution label: nobody-was-watching-that-arc-2026-09-01** (MINOR). The cycle-coverage check counted RECONSTRUCTED records — backfills of work that shipped before the trigger existed — as missing `gates_fired`, `regressions` and `rework`. Nobody was watching those arcs, so writing `[]` would not record a measurement; it would add a FABRICATED ZERO to framework-health's Gate-effectiveness denominator and deflate the number the field exists to produce. Derived, not invented: `engine/cycle-learning.md` already exempts these records from every calibration aggregate, and v0.98.1 exists because a rule once shipped and instantly created three violating reconstructed rows — the SAME THREE ROWS have now met a second rule. **The exemption is per-FIELD, not per-record**: `demand_type` stays required, because Seddon's type classifies why work was ASKED FOR rather than what was observed while it ran, and that is recoverable. The dogfood project filled it on all three by tracing them to opp-047 — a shipped mechanism that measured a zero fire rate — moving its demand mix to **13 failure / 7 value**. The coverage line now names how many records it excluded, so a clean result is never read as full coverage.*
