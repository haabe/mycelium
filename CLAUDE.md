...
*Version 0.58.0 -- **Attribution label: plugin-form-operating-contract-injection-2026-07-25** (roadmap dogfood — FIXES an unintended plugin-migration gap). The v0.20.0 plugin migration kept the harness's computational half (hooks fire) but silently dropped the always-on **inferential** half for plugin-form consumers: the operating-manual `CLAUDE.md` lives at repo root, OUTSIDE `plugins/mycelium/`, so it was never packaged, and the legacy `degit`-templating that copied it into each project's `.claude/CLAUDE.md` was removed in the 0.20.14 cleanup with nothing to replace it. Consumers (external users + the dogfood repo) ran without the Communication Rules + Mandatory Pre-Task/Pre-Ship/Post-Task Protocols; the framework repo never noticed because its own sessions load the root `CLAUDE.md` natively (the bug is invisible from the author's seat). **Fix**: the always-on rules are extracted to a single canonical `engine/agent-operating-contract.md` (packaged, plugin-form `${CLAUDE_PLUGIN_ROOT}` paths), injected into every session by `hooks/session-start.sh` (unconditionally, feedback reminders appended), referenced — not restated — by this `CLAUDE.md`, listed in `manifest.yml`, and guarded by CI **Check 47** (wiring integrity + no legacy `.claude/` framework paths) with a bash G-V12 test. Lesson: dogfood from a CONSUMER repo — the framework repo is the one place this class of bug cannot appear. **Priors**: v0.57.5 **evidence-type-swap-detector-2026-07-19**, v0.57.4 **evidence-type-enum-canvas-surface-2026-07-19**, v0.57.3 **diamond-evidence-type-enum-2026-07-19** (see `docs/changelog.md`).*
