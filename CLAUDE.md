...
*Version 0.82.0 -- **Attribution label: pointer-source-class-2026-08-03**. `source_class` gains `pointer`, for a source that CITES another canvas entry rather than being evidence itself. Found in a live classification pass: 14 of 21 unclassified provenance objects contain at least one such reference, and because a partial `source_classes` array is refused, none of them could be written however carefully their other sources were sorted. **The rejected alternative is the interesting part.** Copying the target's class into the pointer looks tidier and duplicates a derived value — which goes stale silently the moment the target is reclassified, the same defect that produced a stale correction count and a stale gate heading in the same week. CLASSIFYING a pointer needs no judgement — the text cites a canvas path — so it arrives pre-classified and is never queued for a human; an earlier draft conflated the class with its target's resolution state and would have manufactured 28 interviews for the one classification requiring none. The TARGET is still not auto-resolved, because it is named in free prose and any resolver would be guessing at a citation. **Semantics, all fixture-pinned**: a pointer is excluded from method diversity (two interviews plus a pointer is still one method — otherwise any single-method claim could look triangulated by citing itself); an entry whose sources are all pointers is reported UNJUDGEABLE rather than passed; and the count set aside is printed, so the limit is visible instead of absorbed.*
