...
*Version 0.41.1 -- **Attribution label: autonomous-mode-stage3-permission-verdict-2026-06-11** (lived-friction-triggered). Verification patch on v0.41.0's one Unverified claim, settled interventionally the same day by a Stage 3 headless run + direct probe: settings allowlists do NOT unblock `.claude/` writes in headless mode (denied "sensitive file" despite explicit `Write(.claude/evals/**)` allow rules) — `engine/autonomous-mode.md` §Harness-permission story rewritten accordingly (mirror paths must be pre-authorized; NEW final fallback codified: in-conversation proposal delivery with inline ledger + handover list, demonstrated by the run completing its full scope with zero persistable paths). Companion positive result: the same run carried ZERO substitution rules in its prompt and still followed the engine doc's ladder, ledger, tagging, pre-commit ordering, and evidence boundary correctly — doc sufficiency runtime-confirmed; opp-011 assumption #1 REFUTED + confidence 0.7 → 0.75 (pre-committed riser). **PATCH** (engine-doc + canvas update; no new convention). **Prior** (v0.41.0): **autonomous-mode-fable5-dogfood-graduation-2026-06-11** (lived-friction-triggered).*
