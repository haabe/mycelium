...
*Version 0.31.0 -- **Attribution label: jit-tooling-4-layer-composition**. Three test cycles (Python Flask auth / Node Express notes / Go stdlib upload / Python Flask gallery / Node Express admin) showed Mycelium's JIT-tooling subsystem was silent at bootstrap and caught at best 1/8 deliberately-planted bugs. Founder rejected the obvious fix ("auto-install bandit/semgrep") with a principle: **framework offers menu, never pushes tools**. Deep study of 10 adoption approaches (PAVED-ROAD, STARTER-PACK, CONVENTION-OVER-CONFIG, PEER-LEARNING, etc.) found the highest-evidence composition compatible with the principle: **OFFER-MENU (smallest-friction first) + RISK-TRIGGERED re-offer + NUDGE-AT-FAILURE + PR-TIME gap flag**. Strongest single-finding: contextual nudges at decision moments produce ~8× higher detection (Less is More, arxiv 2202.04586; *consistency_only*). **Shipped**: `delivery-bootstrap` SKILL.md Steps 3a/3b (menu surfacing + risk-shape detection for AUTH / AI / PII / public-endpoint / file-upload, with SAST-blind-spot disclosure inline); `security-review` and `reflexion` SKILL.md NUDGE-AT-FAILURE pattern; `definition-of-done` SKILL.md PR-TIME gap-flag in Security checklist; `jit-tooling/adoption-strategy.md` documenting the composition + why-not-alternatives + honest SAST coverage table. **Critical cycle-3 finding**: Node admin dashboard with self-promote-to-admin design flaw passed every SAST tool clean (eslint-security 0, npm audit 0, gitleaks 0) — confirming the SAST-blind-spot disclosure layer converts a green-SAST false-comfort outcome into an explicit "/security-review is the gate for this class" handoff. **Universal-product-model fit**: applies to any adopter writing code in any stack — language-agnostic by construction. **Honesty caveats**: 5 test runs across 4 stacks = *consistency_only*, not settled; "8× detection" claim is single-experimental-study; tool-catch rate is a noisy metric across stacks because worker discipline varies; framework changes don't normalize worker quality, only review can. **MINOR per version-discipline**: new feature surface on existing skills + 1 new framework-doc file; backward-compatible (additive; existing bootstrap steps unchanged; user owns all install consent). v0.30.0 entry migrated to docs/changelog.md per Check 34.* **Prior**: cursor-codex-hook-adapters (v0.30.0).
