...
*Version 0.106.0 -- **Attribution label: a-gate-you-can-pipe-away-is-not-a-gate-2026-08-08**. **New `scripts/gates.sh`: the gates were never the defect, the way they get RUN was.** `bash tests/validate-template.sh | tail -4 && git commit` reads TAIL's exit status -- tail succeeds at printing a failure -- so the chain saw success and **a commit landed on a red gate** on 2026-08-07. The same exit-status trap fired THREE times in that session. 'Never pipe a gate' is a rule, and rules rot (v0.105.0: one rule, two implementations, one fixed), so this is a script instead: it captures each gate's own status BEFORE anything can mask it, prints the failing gate's output tail as evidence, and puts the verdict on **stderr as well as stdout** so piping stdout away cannot hide it. **A MISSING GATE EXITS 3, NOT 0** -- an absent gate is not a passing gate, the same empty-input honesty v0.102.0/v0.103.0 enforce for checks. 8 tests, the core one being that a failing gate produces a non-zero exit; if that ever passes, the wrapper is decoration. Found while writing them: an all-passing fixture with an EMPTY test file exits 5 (no tests collected) and the wrapper correctly refuses to call that a pass -- the fixture was wrong, not the wrapper.*
