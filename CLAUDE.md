...
*Version 0.156.0 -- **Attribution label: a-schema-for-the-canvas-that-had-just-acquired-a-dependent-2026-09-01** (MINOR). `archived-solutions.yml` was one of seven canvases reported as 'parse-checked only', a warning that read as cosmetic until v0.155.0 made `check_log_reconcile` depend on `archived[].leaf_id` — **a check reading a field nothing guaranteed**. `leaf_id` now required. Field set and `reason` enum taken from `engine/leaf-lifecycle.md`'s documented contract, NOT inferred from the three entries present. **`ice_score_at_archive` has three key shapes across the framework** ({i,c,e,total} in the engine doc, {impact,confidence,ease,total} live, {impact,confidence,ease,score} in opportunities.yml); all three permitted as a RECORDED COMPROMISE, because pinning one invalidates real records and converging them is a migration. Six schemaless canvases remain; this one had the only consumer.*
