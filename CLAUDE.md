...
*Version 0.113.1 -- **Attribution label: a-required-field-nothing-enforces-is-a-suggestion-2026-08-20**. `check_instrument_contract` declared five required fields and acted on ONE. A header with an empty `frozen_before` passed GREEN, which is this check's own failure mode sitting inside it: an agent greps the field, gets silence, and cannot establish that the prediction preceded the data. Found while retrofitting a real 32-instrument corpus by hand, where three files legitimately state no freeze event and their headers were written empty rather than invented — and passed. Now reported as INCOMPLETE, separately from the expiry check, because an unenforced requirement is worse than an absent one: it looks like a guarantee.*
