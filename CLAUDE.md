...
*Version 0.57.4 -- **Attribution label: evidence-type-enum-canvas-surface-2026-07-19** (roadmap/i-productified dogfood — validator-correctness, completes v0.57.3). **Enum-consistency walk**: v0.57.3 closed the `evidence_type` enum gap on the *diamond* surface via a schema `$ref`, but the same defect was latent on *canvas entries* — a bare entry-level `evidence_type`/`source_class` outside a provenance block is an undeclared key that `additionalProperties` waves past (no canvas schema declares those fields). `validate_canvas.py` now runs an enum-consistency walk over every canvas + diamond node, checking each `evidence_type`/`source_class` value against its `_common` enum EVERYWHERE (present + future files), with a disjoint-set hint ("that is a source_class value; did you mean source_class?"). +3 regression tests. Chosen over per-schema `$ref` (16 canvas schemas, none declare the fields, wouldn't cover future files). **Priors**: v0.57.3 **diamond-evidence-type-enum-2026-07-19** (diamond-surface `$ref` fix), v0.57.2 **opencode-upstream-closure-2026-07-19** (recorded bot-closure of opencode #27899 + PR #32542), v0.57.1 **docs-drift-skillcount-2026-07-18** (documentation drift sweep — skill counts → **58**) (see `docs/changelog.md`).*
