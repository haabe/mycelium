...
*Version 0.117.1 -- **Attribution label: the-readme-that-read-as-ai-2026-08-21**. Two README edits, both from outside readers on the same day. A senior practitioner read the repo and stopped before installing: the README is too wordy, the user showcase is **AI word salad**, and the incumbent check is missing. **She was right on the first two and they are fixed here** — the showcase ran five consecutive bullets on one template, *what Mycelium learned about X*, and that template is gone. **A fourth moment joins What it feels like**, the post-ship one: it is out, nobody came, and the next feature is one you wanted yourself. It is there because a builder described exactly that state the same day and read it as needing more testers rather than as a question about demand. **The missing incumbent check is NOT fixed here** and is the open half.*
