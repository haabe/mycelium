...
*Version 0.39.4 -- **Attribution label: canvas-first-on-product-questions-2026-06-03**. Pre-Task Protocol trigger extended to fire on any non-trivial product question against a project with non-null `.claude/diamonds/active.yml`, not just implementation tasks. Targets the headless-mode canvas-skip pattern surfaced by the roadmap brownfield-iteration eval same day (10-run picture, $2.40 spend): on naive product questions ("I want to add X. What should we do?") the agent reasons directly off the file tree and produces an implementation answer that bypasses the diamond's open gates + corrections + JTBD evidence sitting in the canvas. The pre-task trigger already enumerated the right reads (active.yml, corrections, domain guardrails); it just didn't fire on this surface. Verified against opus headless on 2 fixtures (Householdr + Ledgerlink) — engages reliably with this trigger language present, produces discipline-notes footers citing the priming sentence. Sonnet headless does NOT internalize CLAUDE.md as discipline reliably (same eval, Eval A); the prompt-layer fix for sonnet ships in the roadmap auto-dogfood orchestrator's `build_mycelium_prompt` as canvas-first preamble injection (roadmap commit 44c869a same date). **PATCH** (trigger-language widening; no new gate, no new skill, no new file). **Prior**: cross-repo-stale-state-arm-2026-06-02 (v0.39.3); rule-6-fp-measurement-2026-06-02 (v0.39.2); readme-language-revision-2026-06-02 (v0.39.1).*
...
**Why the trigger extension (added v0.39.4):** before this version, the protocol triggered on "implementation task." Roadmap dogfood eval 2026-06-03 (`mycelium-roadmap/.claude/evals/results/brownfield-iteration-2026-06-03/`) showed that naive product questions on a filled-canvas project bypass this trigger entirely in headless `claude -p` mode — the agent reads source code, proposes an implementation choice, and ignores the active diamond's open gates + corrections that warn against the move it's about to make. Extending the trigger to "non-trivial product question on a project with non-null active_diamond" closes this surface for any agent that loads CLAUDE.md as discipline (verified: opus headless across two fixtures). Sonnet headless does not reliably internalize CLAUDE.md as discipline; the prompt-layer fix for sonnet lives in the auto-dogfood orchestrator's `build_mycelium_prompt`.
