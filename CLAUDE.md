...
*Version 0.23.41 -- **Attribution label: lived-friction-triggered**. Continuation of v0.23.40. Auto-dogfood diagnostic post-v0.23.40 (run with --keep-workdir, 2026-05-22) revealed the agent IS reading the v0.23.40 SKILL.md (via the new --plugin-dir flag in the roadmap-private orchestrator) but still NOT writing the decision-log entry. Selective instruction-following: agent wrote purpose.yml + jobs-to-be-done.yml + active.yml (3 of 4 Step 2 bullets) but skipped decision-log.md (4th bullet). Two diagnosed contributing causes: (1) the decision-log bullet was last in the list and verbose, low salience; (2) "After the Interview" section had a competing decision-log instruction the agent may have been deferring to. Fix: (a) hoist decision-log write to FIRST position in Step 2 list with "Hard requirement: all FOUR files must be written" framing; (b) update "After the Interview" Decision Log Entry section to explicitly say "EXTEND Step 2's minimal entry, do NOT duplicate." Numbers in bullet labels ("(1 of 4)" through "(4 of 4)") increase mechanical visibility of the complete required set. PATCH per version-discipline: atomic-commit rule applied.*
