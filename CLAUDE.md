...
*Version 0.49.19 -- **Attribution label: theory-fidelity-anti-drift-guard-2026-06-18** (secures the v0.49.17/18 theory work against future drift — single-loop audit → double-loop gate). Three layers: (1) **`check_theory_fidelity.py`** CI guard (wired into `validate.yml` + the pre-push delivery gate) enforcing the STRUCTURAL invariants of `theories.md` — every Tier-1/2 skill/gate/engine-path reference resolves, every gate carries a `**Source**:`, no name-only theory sits in a load-bearing tier (the doc's own "no theatre" rule, mechanized); 96.9% test coverage. (2) **`/framework-health` step 4g** recommends `/theory-fidelity` (the SEMANTIC audit) when the theory surface (`theories.md` / `theory-gates.md` / skill `Source:` lines) changed since the last run or quarterly. (3) **consistency-check-spec promise-registry convention** — a newly-cited theory with no `theories.md` mapping becomes a tracked row. Honest boundary: the guard catches structural drift (phantom refs, name-only theories, ungrounded gates); semantic distortion + citation truth stay with the LLM skill + source-grounding. New script + test + 2 doc edits + CI/pre-push wiring. **PATCH**. **Priors**: v0.49.18 **theory-fidelity-skill-2026-06-18**, v0.49.17 **theory-fidelity-doc-corrections-2026-06-18** (see `docs/changelog.md`).*
