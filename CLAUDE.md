...
*Version 0.110.7 -- **Attribution label: the-policy-the-project-forbade-for-ruff-and-practised-for-shellcheck-2026-08-16**. The ruff gate tells contributors "do not re-introduce a tolerated-debt baseline"; shellcheck ran against a baseline of 3 described as documented historical tech debt. v0.110.6 made shellcheck run locally for the first time and the inconsistency became visible. All three findings were SC2034 assigned-but-unused and one was not lint noise: a METRICS_SKILL routing slot sat beside METRICS_CANVAS and METRICS_LABEL that NO case arm ever set, while the reminder hardcoded the skill anyway — checked before removal rather than assumed half-wired, since /dora-check handles all four metrics canvases itself. Also a positional read now using shellcheck's `_` discard idiom, and a dead NOW=$(date +%s). **Baseline is now 0**: a regression threshold that tolerates known findings cannot distinguish "no problems" from "the same three problems".*
