...
*Version 0.42.2 -- **Attribution label: hooks-readme-guard-catchup-2026-06-11** (lived-friction-triggered). Doc-only: `hooks/README.md` Layer-1 (PreToolUse) section now documents the full hook chain that runs on the Write/Edit/MultiEdit matcher — `scope-gate.sh`, `framework-guard.sh`, and the v0.42.0 `autonomous-evidence-guard.sh` — which the README previously omitted (it listed only `gate.sh`). Found by a docs-consistency audit after this session shipped the guard. No behaviour change. **PATCH** (hook doc catch-up; no skill/engine/hook/schema change). **Prior** (v0.42.1): **dod-design-doc-2026-06-11** (lived-friction-triggered).*
