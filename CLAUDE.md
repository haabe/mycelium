...
*Version 0.172.0 -- **Attribution label: canvas-text-reached-every-session-undelimited-2026-09-03** (MINOR). The first STRIDE pass on the harness itself found its own highest-severity threat in the SessionStart hook: **Check 5 prints up to three human-task objectives, 70 chars each, VERBATIM into every session's reminder block** — and those objectives carry text harvested from Reddit digests, competitor READMEs, and inbound DM and email subjects. **Demonstrated on the live dogfood canvas, not speculated**: one emitted objective began *"Surfaced by the 2026-08-24 scout digest"*. A crafted objective becomes system-adjacent context in every future session, with no expiry. **Check 7 is the right control and reaches no canvas file** — but extending its list would have added a FIFTH lexical imperative-regex to a project that measured its lexical detectors at **0 of 4** confirmed errors caught on 2026-08-31, and a regex written for markdown bullets sees almost nothing in YAML prose: a check that runs, reports clean, and cannot see the event it guards. **So the fix is structural, not lexical.** The emitted objectives are now delimited as `<untrusted_user_content>` and introduced as data — which holds however the injection is worded — and a literal closing tag inside an objective is defanged so it cannot end the block early. Nine assertions across happy/sad/bad/edge, including that an injection-shaped task is still SHOWN rather than suppressed.*
