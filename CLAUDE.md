...
*Version 0.171.1 -- **Attribution label: the-date-was-on-the-node-not-in-the-block-2026-09-03** (PATCH). The provenance-dating coverage line printed on every push read `98 of 356 blocks carry no date` and named `purpose.yml (93)` as most-affected. **95 of those 98 carried a valid date one level up** — `purpose.yml :: evidence.sources[]` writes `date: 2026-04-16` on the source node and keeps a snapshot pointer in `provenance:`, so `_count_provenance` read inside the block, found nothing, and called UNCHECKABLE a date sitting in plain sight. **A 97% false-positive rate on a line printed every push**, pointing at the one file with nothing wrong while burying the three entries that are genuinely undated. The check now accepts a date on the containing node. Real ratio: **353 of 356**, and the remaining 3 are named where they actually live (`opportunities.yml`). Two tests pin both halves — a node-level date silences it, and a block undated at BOTH levels is still reported, so the widening cannot swallow the real ones. This is the project's own false-positive rule applied to its own instrument: classify the FPs before filtering, and a warning that is wrong 97% of the time teaches its reader to skip the class.*
