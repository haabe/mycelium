...
*Version 0.23.23 -- **Attribution label: lived-friction-triggered**. Three-part graduation of the named-attribution mechanism stack: (a) Check 33 scope expanded from `plugins/mycelium/` only to all publicly-visible framework-repo paths (`plugins/mycelium/`, `CLAUDE.md`, `README.md`, `AGENTS.md`, `CONTRIBUTORS.md`, `docs/`, `.claude/memory/{corrections,patterns,cluster-instances}.md`, `.claude/canvas/`, `.claude/harness/`, `.claude/engine/`, `tests/`) — the prior plugin-tree-only scope was the right boundary for plugin-marketplace shipping but not for public GitHub visibility, and the v0.23.21 leak hit exactly the docs/CLAUDE.md/.claude/memory paths the prior scope missed. (b) WARN→FAIL graduation: leaks now block the validator run (previously WARN-only "pending pre-disclosure cleanup"). (c) Pre-disclosure cleanup: 15 leaks across 4 files (`docs/changelog.md` ×3, `docs/receipts/cases/2026-05-09-consistency-as-evidence-graduation.md` ×1, `.claude/memory/patterns.md` ×1, `.claude/memory/corrections.md` ×6 across multiple entries) regenericized to cohort-class language; private roadmap registry expanded to include the two newly-added cohort participants as `generic_only`. Check 33 now reports 0 leaks across the full public-visibility scope. The mechanism is complete: registry-driven detection + expanded scope + fail-on-leak + cleaned baseline. Sequential 4-version arc completes (0.23.20 pre-push hook → 0.23.21 cohort-driven fixes + leak regenericize → 0.23.22 mechanical gates Layer 1+2 → 0.23.23 scope expansion + WARN-to-FAIL + cleanup). PATCH per version-discipline: validator behavior change + content regenericize; no schema change, no skill change, no behavior change for downstream users.*
