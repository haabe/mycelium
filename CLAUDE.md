...
*Version 0.130.0 -- **Attribution label: a-closed-task-read-as-an-open-commitment-2026-08-24**. **Of 94 entries under `pending_tasks` in the dogfood project, NINE were pending.** 61 were `completed`, 7 `abandoned`, 1 `cancelled`, 16 `in_progress` -- and the founder caught it, not the framework, because he remembered closing one of them ten days earlier while it was still being read as owed. **THE SCHEMA IS WHY NOTHING CAUGHT IT**: `pending_tasks.items.status` shares the full six-value enum, so a completed task inside the pending list is schema-VALID and the file passed for four months. **THE ROOT CAUSE IS ONE LAYER FURTHER OUT**: `/log-evidence` says move a closed task to `completed_tasks`, that list did not exist in the canvas, and **an instruction whose destination does not exist does not fail -- it silently does nothing**, the same shape as a specced field with no reader. Ships a WARN-tier check that a task's own `status` agrees with the list it sits in, a `closed_without_evidence` list for abandoned/cancelled work that **deliberately does NOT require `source_class`** (that field classifies where EVIDENCE came from, and a task producing none has no source class -- the exact correction `/log-evidence` took on 2026-08-03), and **`completed_tasks` relaxed from three required fields to one**: measured against four months of real closures, **47 of 61 carried neither `completed_at` nor `source_class`**, because no closure path ever populated them. A schema asserting a contract its own producers never met is the divergence, not the records -- and holding the line would have forced 45 invented values. **ALSO CLOSES A GAP IN v0.129.0 ITSELF**: `demand_type` shipped with its consumer but without the absence WARN its two siblings carry, so a missing one was silent where theirs are noisy -- which is exactly how 0-of-16 compliance goes unnoticed for months.*
