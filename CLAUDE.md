...
*Version 0.114.1 -- **Attribution label: a-regression-guard-that-says-it-ships-green-2026-08-20**. `check_log_reconcile` reconciles dated decision-log events against the canvas history rows they should have produced, generalising the one-off BVSSH orphan check into a registry. It guards two recorded failures — an assessment orphaned to the log, and a DORA measurement taken but never written to `measurement_history`, leaving one file carrying three dates for one measurement. **IT SHIPS GREEN AND ITS OWN OUTPUT SAYS SO**: on the corpus that motivated it, BVSSH ran 8 log events against 14 canvas rows and DORA 3 against 9, with zero orphans either way, so it is a regression guard rather than a discovery tool and should be narrowed or retired if it stays quiet. Wired into `/mycelium:canvas-health` and deliberately NOT gated, because a gate with no failing instance is the near-zero-action-rate guard this framework already warns about.*
