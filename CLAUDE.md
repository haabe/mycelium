...
*Version 0.59.1 -- **Attribution label: self-correcting-release-capture-2026-07-25** (repo-only validator check, WARN tier). Adds **Check 48**: a release whose changelog says it `corrects`/`reverts`/`supersedes` an earlier release must have a `corrections.md` entry within 14 days. Deliberately NOT a shipped consumer script -- the principle is general but the detector assumes a versioned-release workflow with a dated changelog, which most consumers do not have; a consumer check that can never fire is how promise-registry rows accumulate. `completes` is excluded (follow-through is not a mistake). Surfaced + backfilled the v0.57.4 lesson, which was never lost -- it lived in the changelog, but not in the memory `/corrections-audit` and the cluster catalogue actually read. Sibling of v0.59.0: **right content, wrong surface**. 6 G-V12 fixture tests. **Priors**: v0.59.0 **bvssh-assessment-reconcile-2026-07-25** (see `docs/changelog.md`).*
