...
*Version 0.49.15 -- **Attribution label: scope-expansion-guardrail-receipts-backfill-2026-06-18** (closes the last two framework-health recommendations). (1) **G-P9** added to `guardrails-core.md`: re-surface scope when a bounded task starts spawning ships (NUDGE) — the dogfood's most consistent calibration miss (scope-expansion-blind, N=4: cycle-006/008/009/012; the session that named it was itself instance #4). A checkpoint, not a prohibition: scope can grow, but the user re-authorizes the new shape rather than discovering it after N commits. (2) **Receipts indexes backfilled**: `by-date.md` + `by-mechanism.md` had lagged since 2026-05-30, missing 6 cases (2026-06-01 → 2026-06-18); now current. Guardrail + docs. **PATCH**. **Priors**: v0.49.14 **pre-push-delivery-gate-2026-06-18**, v0.49.13 **framework-health-calibration-class-aware-2026-06-18** (see `docs/changelog.md`).* (the reckoning: Mycelium's delivery-quality discipline — test-on-add, coverage, the doc guards — had NEVER fired automatically on its own development. The DoD gate only runs at a product-leaf Deliver→Complete, of which there have been 0; the test/coverage/legacy/dead-ref gates lived only in `validate.yml` (CI), invisible to the local loop. The operator was always the delivery gate). Fix: `git-pre-push-example.sh` gains a **Layer 3 delivery-quality gate** that runs the CI-equivalent set LOCALLY at push-time and **hard-blocks** — pytest + `--cov-fail-under=85` + `check_coverage_floor.py` (a shipped script with no test cannot be pushed) + `check_legacy_paths.py` + `check_doc_references.py`. Framework-repo only (gated on `tests/python`); downstream user projects skip it. Bypass remains `git push --no-verify` (document it). This is the FIRST mechanism that makes the framework's own delivery discipline self-apply rather than depend on the operator remembering. Scripts (hook). **PATCH**. **Priors**: v0.49.13 **framework-health-calibration-class-aware-2026-06-18**, v0.49.12 **per-file-coverage-gate-2026-06-18** (see `docs/changelog.md`).*
