...
*Version 0.121.0 -- **Attribution label: the-fields-that-closed-a-dimension-nothing-read-2026-08-23**. **TWO CYCLE-RECORD FIELDS WERE SPECIFIED TO CLOSE NAMED `/framework-health` DIMENSIONS AND APPEARED ON ZERO OF SIXTEEN RECORDS** on a real project, including one written the same day its cycle closed. Three failures stacked: the rule lived only in skill prose, `cycle-history.yml` had no schema so nothing could detect the omission, and **the consumer never read the field anyway** -- `framework-health` computed gate effectiveness while naming no source, and `gates_fired` existed in exactly two places, the spec that defines it and the skill that writes it. Now: a schema for `cycle-history.yml` that requires only what consumers key on and mechanizes the previously-prose rule that a `product-leaf` cycle cannot carry a zero ICE total; a WARN-tier field-coverage report that never flags a recorded `[]` or `0` and excludes cycles too recent for the 14-day `rework` lag; `framework-health` naming its source and reporting `no-data` with a count rather than reconstructing gate outcomes by hand. **`purpose-properties` no longer relies on the extractor NOTICING a quality adjective** -- that behaviour has one blind run of one model family behind it, so a listed adjective now routes to the builder interview regardless of what was extracted. All WARN-tier; nothing is required of existing projects.*
