...
*Version 0.49.18 -- **Attribution label: theory-fidelity-skill-2026-06-18** (the "be able to evaluate the framework against its theories" capability, graduated from the v0.49.17 audit). New skill **`/theory-fidelity`**: audits whether each theory a project claims is faithfully operationalized vs name-dropped / partial / distorted / over-claimed — three axes (Representation × Fidelity × evidence-basis), source-grounds the load-bearing theories and tags the rest provisional, with an anti-pattern-#7 recursion guard (don't grade a theory against your own recollection) and the Lopopolo attribution-fix rule (ground-truth every occurrence; never blind-sweep). Pairs with `/framework-health` quarterly. Skill count 55→56 (`sync_derived` propagates the token). **PATCH**. **Priors**: v0.49.17 **theory-fidelity-doc-corrections-2026-06-18**, v0.49.16 **readme-receipts-rotation-2026-06-18** (see `docs/changelog.md`).*
