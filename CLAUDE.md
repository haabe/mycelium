...
*Version 0.116.1 -- **Attribution label: two-blind-readers-found-the-boundary-2026-08-21**. **A REAL OFF-BY-ONE, IN v0.116.0, PAST 20 TESTS AND FOUR GATES AND CI**: `review_by` used `when < today`, so a cluster review falling due TODAY reported as quiet and failed a day late. Found by two independent blind readers with no repository access, who both said the boundary was untested — it was untested AND wrong. Fixed to `<=`, with a test on each side so a slip in either direction is caught. **The same pass corrected a CHERRY-PICK in the v0.115.0 note**: compliance was described as bimodal, "at or near zero on every other day", while four days ran at 25-33% and appeared in no evidence list. The 16%-vs-29% headline is unaffected and the shape claim was overstated — one day at 89%, four around 30%, ten at zero is a spread, not two modes. **Also names a known hole rather than papering over it**: a pending cluster with no `review_by` is exempt forever, every available timeout anchor is arbitrary, so the hole is printed instead of closed with an invented deadline.*
