...
*Version 0.49.17 -- **Attribution label: theory-fidelity-doc-corrections-2026-06-18** (from a theory-fidelity audit — Tier-1 source-grounded, Tier 2/3 model-knowledge). `docs/theories.md` was the least-faithful artifact against its own "citations without mechanism-mapping are theatre" standard. Corrections: Torres row no longer credits ICE for solution selection (Torres rejects it; assumption-testing is the method, ICE is Ellis); Wardley "gate at L1→L2" → NUDGE + suggested trio skill (no such gate); DORA reliability re-classified as a 2021 operational adjunct (not the "2024 fifth delivery metric" — that was rework rate) across theories.md + `/dora-check` + gate 10; Sinek "refuses to spawn L1 before why populated" softened to match the generic Evidence gate; Senge row now cites the system archetypes (anti-patterns #12–15 in `/diamond-assess`); Three Ways + Patton/Cagan + Cagan-"canvas IS spec" pointers corrected; Argyris promoted Tier-3→Tier-2 (load-bearing: fractal double-loop + G-P7); Hoskins citation re-anchored to *The Product-Minded Engineer* (2025, SAP-talk citation tagged unverified); Reflexion mis-attribution fixed Lopopolo→**Shinn et al. 2023** in theories.md + glossary (narrow — Lopopolo stays correctly cited elsewhere for the harness-context reframe; a repo-wide sweep would have corrupted 16 valid citations). Docs + one skill + one gate. **PATCH**. **Priors**: v0.49.16 **readme-receipts-rotation-2026-06-18**, v0.49.15 **scope-expansion-guardrail-receipts-backfill-2026-06-18** (see `docs/changelog.md`).*
