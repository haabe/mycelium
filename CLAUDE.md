...
*Version 0.112.0 -- **Attribution label: a-prediction-with-no-home-is-an-opinion-with-a-date-2026-08-20**. `/assumption-test` Step 5 asks for a prediction before the run and never said where it lands, so the instrument document carrying it was written by no skill, governed by no schema and read by no check. Measured in dogfood: 27 of 36 instrument files stated a freeze convention IN PROSE, in 27 different phrasings, and none carried an expiry — the author wrote the contract by hand every time and never the same way twice, which is exactly why nothing could check it. Two frozen predictions were lost to that. Step 5b adds a four-field header (deliberately four: AsPredicted asks nine for all of science, and length kills completion) and `check_instrument_contract.py` reports uncontracted, undated, overdue, untracked and DRIFTED instruments from `/mycelium:canvas-health`. Drift is the one it exists for: COMPare found 67 registered trials reporting an average 62% of their pre-specified outcomes while silently adding 5.3 new ones — the registry existed and nobody diffed it. Amendments are RECORDED, never forbidden, which is what mature registries actually do. It prints its own limits every run, because a filled header does not make a test severe and a green here is a green about paperwork.*
