...
*Version 0.103.2 -- **Attribution label: a-finished-task-is-allowed-to-say-what-it-was-for-2026-08-07**. **`check_stale_prose.py` rule B no longer fires on terminal records** (completed / cancelled / abandoned). Its premise -- a FRAMING field asserts not-done while a LOG field records it done -- is EXPECTED AND CORRECT on a finished task, whose objective legitimately describes what it was FOR. Dogfood 2026-08-07: 4 of 7 candidates on the consumer canvas were exactly this, and **the only way to silence them would have been to rewrite finished records -- destroying provenance to satisfy an advisory, which is the Goodhart failure this checker's own docstring warns about.** **RULE A IS DELIBERATELY NOT SKIPPED**: an unanchored 'today' is wrong the day after it is written whatever the status, and one of the three REAL finds that day was a bare 'due today' inside a completed task's log. The other two were live: a pending task advertising an undrafted reply that had been sent twelve days earlier, and a three-month-old diamond entry reading 'mitigated by today's audience'. 12 tests (3 new), and the consumer canvas now reads clean across 130 files rather than nagging about four records nobody should touch.*
