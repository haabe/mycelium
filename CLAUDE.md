...
*Version 0.164.0 -- **Attribution label: the-refresh-protocol-wrote-a-field-the-schema-forbade-2026-09-01** (PATCH-shaped MINOR). `engine/evidence-decay.md` instructs *'update `validated_at` to the current date'* after refreshing evidence, `canvas-health` step 7 scans for it, and the decay comparison reads it — while `_common.schema.json#/$defs/provenance` sets `additionalProperties: false` and **never declared it**. For the life of the schema, a consumer following the documented refresh protocol produced an INVALID canvas. That is a plausible reason the dogfood project had 29 entries past their 90-day horizon and not one refreshed: **the decay loop could not close, and the blocker was the framework's own validator.** Found by trying to run the refresh rather than by reading about it. Declared now, typed as a date so declaring it does not make it a free-text hole, with `captured_at` untouched — a refresh records when a claim was RE-CHECKED and must never overwrite when it was first gathered.*
