...
*Version 0.28.0 -- **Attribution label: skill-framework-dependency-frontmatter**. Verification of majiayu000/claude-skill-registry (336 stars, 50+ haabe-mycelium-named files actively scraped) confirmed: every Mycelium skill is indexed across three Google-visible surfaces with attribution preserved (source_url, repo, author). Registry conservatively classifies Mycelium as license: NOASSERTION / distribution: restricted (crawler missed the MIT LICENSE in plugins/), which limits the redistribution risk but leaves the standalone-mis-fire risk: a user finding `four-risks-haabe-mycelium` via Google and running it without the framework gets a skill that assumes canvas + gates + harness it cannot find. **Shipped**: `metadata.framework_dependency: "mycelium"` + `metadata.framework_dependency_note: "..."` added to all 49 SKILL.md files. Invisible to in-framework users (Claude Code does not render metadata as user-facing text); machine-readable for any scraper/registry consumer; propagates wherever the frontmatter goes (registry preserves frontmatter per `audit_category_quality.py --include-frontmatter`). Spec-compliant per agentskills.io (custom fields under metadata: are spec-allowed; same pattern as v0.23.36 instruction_budget migration). **MINOR per version-discipline**: new feature surface on skill metadata (framework-dependency declaration); backward-compatible (additive; existing instruction_budget fields preserved; skills without prior metadata: block got a fresh one). 49 files modified in one mechanical sweep, no body content changed. v0.27.0 entry migrated to docs/changelog.md per Check 34.* **Prior**: empty-fixture-dir-check (v0.27.0).
