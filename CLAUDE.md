...
*Version 0.145.3 -- **Attribution label: three-consumer-findings-from-a-non-software-object-2026-08-31** (PATCH, consumer-reported). A plugin consumer running Mycelium on a NON-SOFTWARE object filed ten findings; three were mechanical and are fixed. **`bvssh-health.yml` had NO SCHEMA** while `session-start.sh` reads `last_assessed` from it every session and falls back to 999-days-overdue on any parse failure -- so malformed was indistinguishable from stale; schema added, strict on that field only. **`devils-advocate` Technique 7 never named that it REQUIRES a subagent**, so a no-subagent session silently ran the weaker self-attack fallback; it must now declare a degraded pass. **`log-evidence` read `pending_tasks` only**, leaving no branch for evidence belonging to an already-COMPLETED task; appending a dated sub-entry without reopening is now written down. Their F1 residual case was verified ALREADY CLOSED by v0.145.x from the other side. F2/F3/F4/F5/F8 need decisions and are named, not absorbed.*
