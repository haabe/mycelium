...
*Version 0.49.13 -- **Attribution label: framework-health-calibration-class-aware-2026-06-18** (the `/mycelium:framework-health` confidence-calibration dimension computed over ALL cycles, but `engine/cycle-learning.md#cycle-class` + `cycle-history.yml#calibration_summary` already exclude `meta-dogfood` + `observation` from ICE-calibration — the skill was out of sync with the model). Fix: calibration now scopes to `product-leaf` cycles only; with 0 product-leaf it reports status `empty-by-design` instead of flagging a gap or synthesizing a factor from non-product cycles. **Masking guard** added so this can't hide a real signal for non-dogfood product projects: a project doing actual delivery work yet showing 0 product-leaf cycles is a `cycle_class` mis-assignment to investigate, NOT empty-by-design. Also repairs a malformed v0.49.12 version line (leftover v0.49.11 prose appended in the bump edit). Skill + docs. **PATCH**. **Priors**: v0.49.12 **per-file-coverage-gate-2026-06-18**, v0.49.11 **python-test-coverage-62-to-90-2026-06-18** (see `docs/changelog.md`).*
