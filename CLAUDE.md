...
*Version 0.113.0 -- **Attribution label: the-header-is-an-agent-interface-not-paperwork-2026-08-20**. `check_instrument_contract` shipped advisory and its own output called a green "paperwork". The founder rejected that framing and he is right: the contract block is the INTERFACE BETWEEN AGENTS. A malformed header does not mislead a human, who skims past it — it makes an agent grepping `status: live` conclude there are no live instruments, which is the false-absence failure this project logs more than any other. The risk scales with the number of WRITES, not the number of files, and every write is an agent that has repeatedly proven unreliable at structured documents. It is now a CI gate and a pre-push gate, on the same reasoning already recorded for `check_source_class_fidelity`: machine-consumed fields get gated because the cost of a wrong one is silent. The output now separates the two claims it had collapsed — a green means the fields parse and agree with git, and still says nothing about whether the test was any good.*
