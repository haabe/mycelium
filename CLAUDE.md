...
*Version 0.23.14 -- **Attribution label: lived-friction-triggered**. Two coalesced fixes both flowing from a single discipline: keep deferred versions and private-architecture details out of this file. **(1)** Forward-only regenericization of two changelog entries (now migrated to `docs/changelog.md`) that named a private companion repo by its specific path while describing the v0.23.13 registry-move fix. The lesson generalizes v0.23.13's own lesson one step further: commit messages and changelog text are themselves public-disclosure surfaces, even when discussing the architecture *that supports* privacy discipline. **(2)** Migrated v0.23.8 through v0.23.13 entries from this file to `docs/changelog.md` per the established convention (CLAUDE.md keeps only the current release; full history lives in the changelog). User-noted that this session's five consecutive in-CLAUDE.md entries violated that discipline. Both lessons share the same root: every text artifact carries forward more than its intended content unless the author asks "what does this disclose beyond what it claims to be about?" Git history retains the original phrasing of commits `c539f29` and earlier; the working-tree view is now generic. PATCH per `version-discipline.md`: doc-only regenericization plus a docs migration, no behavior change.*
