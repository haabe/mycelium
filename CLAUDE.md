...
*Version 0.153.1 -- **Attribution label: three-documented-versions-had-no-release-2026-08-31** (PATCH). `release_gaps.py --check` failed on main: v0.150.0, v0.151.0 and v0.152.0 were documented and unreleased, because each commit went red on the flag-parity defect v0.153.0 fixes. Content folded under v0.153.0 and shipped there. **Folding rather than back-releasing, for the SECOND time today** -- a commit CI rejected does not get a release even when the content was sound. Structural cause recorded: the changelog is written at COMMIT time and the release happens at CI time, so any red commit opens a gap; the backstop catches it every time, and it will recur until those moments coincide. v0.153.0 fixed the flag-parity defect itself.*
