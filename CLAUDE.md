...
*Version 0.57.5 -- **Attribution label: evidence-type-swap-detector-2026-07-19** (roadmap dogfood — CORRECTS v0.57.4's over-fire). **Polymorphism fix**: v0.57.4's enum-consistency walk asserted `evidence_type` ∈ the Gilad enum EVERYWHERE, contradicting the field's documented polymorphism (canvas-guidance.yml: Gilad ladder in diamonds/confidence-provenance, gathering-method [interview|survey|analytics] in `_meta`, signal-type [market_signal|...] in market provenance, plus intentional extensions llm_positioning_mirror/dogfood_result/stated-intent). Against a real dogfood canvas it produced 21 false positives on legitimate values. The walk is now a DISJOINT-SET SWAP detector — the source_class value set is disjoint from every legitimate evidence_type vocabulary, so it flags only a source_class value in an evidence_type field (or a Gilad evidence_type value in a source_class field): the actual error class (the i-productified/roadmap `evidence_type: external_human` bug), zero false positives. Diamonds keep the strict Gilad `$ref` (v0.57.3). +polymorphic-not-flagged regression guard; roadmap + i-productified + framework canvases all PASS. Lesson: a global canvas check must be dogfooded against a real, richer canvas before shipping — the framework's own clean canvas hid the false-positive surface. **Priors**: v0.57.4 **evidence-type-enum-canvas-surface-2026-07-19** (the over-fire this corrects), v0.57.3 **diamond-evidence-type-enum-2026-07-19** (diamond-surface `$ref` fix), v0.57.2 **opencode-upstream-closure-2026-07-19** (see `docs/changelog.md`).*
