...
*Version 0.153.0 -- **Attribution label: the-same-gate-had-different-teeth-in-ci-and-locally-2026-08-31** (MINOR). CI went RED TWICE (v0.151.0, v0.152.0) on `check_fail_open.py --strict` while every local push passed: both places ran the gate, **CI ran it `--strict` and the local set ran it BARE**, so the same gate was advisory locally and blocking in CI -- and `check_gate_parity` reported OK because it compared script NAMES, not invocations. That is the 2026-08-09 drift wearing a disguise: not a MISSING gate, a gate with DIFFERENT TEETH, producing the identical outcome of red-only-after-a-push. Fixed on both sides: the local set now runs `--strict`, and parity gained `strict_mismatches`, failing in BOTH directions. Immediate cause: two new advisory handlers, now reviewed. **Process failure: the agent stopped watching CI after pushing and the founder had to say so.**\*
