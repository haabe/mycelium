...
*Version 0.117.0 -- **Attribution label: the-technique-that-measured-well-2026-08-21**. `devils-advocate` gains **Technique 7, a blind adversarial pass**, and Technique 5 gains the note it has needed since 2026-08-16: **its measured fire rate is ZERO** — in a session where twelve claims were withdrawn it fired on none, and a detector keyed on its five named shapes scored 0 of 5 recall, because the real failures were universals and counts rather than causal assertions. **Techniques 3 and 5 ask you to attack your own position; measured against real withdrawn claims, that found little.** Technique 7 hands a claim plus one line of procedure to an agent with no repository access and no tools, requires that declining be permitted, and verifies each candidate defect against the artifact. It ships with its own measured limits: reader agreement is NOT independent confirmation, ~40% of output may be unrulable by a third party, "did it change something" measures conviction rather than correctness, and conversational provenance is invisible to it and to git — **a human who was there outranks both**.*
