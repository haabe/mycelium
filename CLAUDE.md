...
*Version 0.110.4 -- **Attribution label: the-staleness-check-could-not-see-most-of-the-activity-2026-08-14**. Human-task staleness read dates out of FIELD VALUES only, while canvases record events in FIELD NAMES — `reply_sent_2026_08_11`, `third_reply_inbound_2026_08_12`. **On the dogfood canvas 19 open tasks carried their latest activity that way, and a task touched two days earlier read as NEVER TOUCHED**: eight tasks were labelled STALE and six of them were false. A live conversation and an abandoned one scored identically. Field-name dates are now counted, floor-gated on the task's own creation date so the fix cannot make a task look fresher than it is — silencing the check is the more dangerous failure. Two smaller divergences fixed alongside: the bare `created` key is now read as well as `created_at` (9 of 83 dogfood tasks used it and fell through the fallback entirely), and `cancelled` is added to the hook's TERMINAL set, where it was missing while sitting in the shipped schema enum and in both `check_reply_owed.py` and `check_evidence_landed.py`.*
