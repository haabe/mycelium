...
*Version 0.141.0 -- **Attribution label: warnings-that-fired-on-health-2026-08-28**. **THREE CHECKS WERE FIRING ON THE FRAMEWORK WORKING CORRECTLY**, found by reading the pre-push warnings on the dogfood repo instead of scrolling past them. **The purpose hash was 98.9% evidence**: `what` held 483 characters of intent and 43,210 of evidence, so appending one citation marked nine human-confirmed properties superseded — properties backtested against 53 solutions and 197 decisions — while `why` and `how` were byte-identical. `purpose_hash` now strips evidence-bearing keys. **AND IT REFUSES A VERDICT IT CANNOT SUPPORT**: existing hashes were stamped by the old algorithm, so a mismatch against them is uninterpretable, and without the new `hash_algorithm: intent-v1` marker the check says it *cannot tell* rather than reporting staleness it has not established. **THE VALIDATOR ALSO CONTRADICTED ITSELF TWENTY LINES APART**: `id_prefix_section_warnings` flagged `ht-NNN` across three task lists as a misfiling while `TASK_STATUS_HOMES` declared all three legitimate. They now share one source of truth, and the docstring's claim of *"zero false positives, measured across 25 real canvas files"* is corrected — it measured prefixes, and `ht` was not among them. **A warning that fires on health is muted, and then it is absent on the day it matters.***
