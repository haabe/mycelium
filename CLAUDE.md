...
*Version 0.157.0 -- **Attribution label: thresholds-schema-and-the-ice-loop-has-its-first-points-2026-09-01** (MINOR). Second of seven 'parse-checked only' canvases schematised, from `engine/adaptive-thresholds.md`'s Threshold Registry. **The schema states its own limit**: it pins the SHAPE of a calibration record and cannot tell you calibration ever ran — the dogfood file explained its nulls by blaming a cycle count already exceeded, and only reading it against cycle-history caught that. **`ice_advance` documented as NOT an advancement gate** (removed v0.54.0 as scoring-for-selection; a validated low-ICE leaf is DEPRIORITISED, never archived) because the name says otherwise. **The calibration input is no longer empty**: three product-leaf cycles recorded on the founder's ruling give 448 launched-and-succeeded against 160 and 270 killed — n=3 of 10, nothing calibrated, but the survivor scored highest. Also caught by ruff: a duplicate test name that would have SILENTLY SHADOWED v0.156.0's live-canvas test.*
