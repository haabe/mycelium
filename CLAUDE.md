...
*Version 0.60.0 -- **Attribution label: opencode-consumer-path-2026-07-25** (dogfood-surfaced; the opencode path did not work for a consumer). Headline: `provision-skills.sh` computed `RESIDUAL=$(grep ... | wc -l)` under `set -euo pipefail`, and grep exits 1 when it finds nothing -- which is the SUCCESS case -- so **a perfect provisioning run aborted at exit 1 with no message while a run leaving residuals printed "Done."** `/mycelium:setup` Step 5 invokes it, so setup reported failure on a flawless install; invisible because nothing had ever run it against a clean project root. Also ships the **opp-009 data-loss guards** (fail-closed on a non-git project root, since opencode walks UP and adopts an ancestor repo; plus a refusal to vendor into the Mycelium checkout), an end-to-end **corrected setup path** (git init step 0, separate clone dir, the missing `.opencode/plugin/` + command install, AGENTS.md creation, `tool_call` not `tools`), **runtime-verified Mistral rows** (mistral-nemo + mistral PASS, control-gated at -c 32768), the **Ollama.app 4096 trap** documented with mechanical verification, and a **38-of-58** count correction. 14 G-V12 assertions. NOT included: hooks.opencode.json / hook parity (2 of 9 fire) -- gated on opp-014, unmet. **Priors**: v0.59.1 **self-correcting-release-capture-2026-07-25**, v0.59.0 **bvssh-assessment-reconcile-2026-07-25** (see `docs/changelog.md`).*
