...
*Version 0.169.1 -- **Attribution label: one-warning-two-parser-bugs-2026-09-02** (PATCH). A consumer reported `correction-attribution-guard` warning "no catcher named" against entries that named one — twice in one session, wrong both times, for two UNRELATED reasons, which is why it read as noise rather than as a bug. `correction_attribution_guard.py` imports both `_corrections_lib.entries` and `classify`, so two defects arrive as one indistinguishable message. **(1)** `ENTRY_RE`'s bullet form accepted a parenthesised date anywhere on a bullet that merely STARTED with bold, so a body bullet citing a dated sibling split its own entry in two and the phantom half carried no catcher; the date must now sit inside the first `**...**`. **(2)** `CATCHERS` required `by\s+` and the corpus writes `**Caught by**: the founder`, putting `**: ` between them; the separator is now bounded-non-alphanumeric. **Measured: 3 of 10 entries using that form were affected, and recovering them moved coverage 100 → 103 of 250 — not wrong data, real data the metric could not see.** Both bash twins moved with the Python, and the cross-language agreement test caught the `preflight.sh` drift mid-fix. The fixture gained the near-miss it lacked, on ONE line deliberately: the pattern is line-anchored, so a wrapped bullet cannot trip it and would pass against the bug it claims to cover — the first attempt was written that way and counted 8 under both patterns.*
