...
*Version 0.23.43 -- **Attribution label: lived-friction-triggered**. Follow-up on v0.23.42's surfaced finding. The router-discipline scenario meta-finding was that `canvas-guidance.yml#action_flags.transitions.timeout_handling` (added 2026-05-03) said "Surface as a stale flagged item via /canvas-health" but `/canvas-health` had no scanner for ON HOLD calendar timeouts — the convention pointed at a downstream skill that didn't implement what was claimed. **Same shape as instance 10** (test-driver bypassing SKILL.md) — two cluster instances of `documented-rule-diverges-from-enforcement` in one session. Recursive surfacing recorded as instance 11. Fix: Step 9c added to `canvas-health/SKILL.md` scanning for date-named ON HOLD markers, parsing dates, comparing to today, flagging <30-day past as warning and ≥30-day past as escalation. Does NOT auto-transition. Cluster catalog + instance log updated to 11 instances; could justify a sibling subclass `convention-promise-vs-skill-implementation` if a 12th instance surfaces. Two same-cluster instances in one session is itself a pattern: the cluster's substrate (every place where doc-says-X-but-mechanism-doesn't-do-X) has more surface area than the spec's current taxonomy catches. PATCH per version-discipline: atomic-commit rule applied; SKILL.md doc-only addition (no schema/behaviour change).*
