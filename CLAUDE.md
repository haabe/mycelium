...
*Version 0.149.0 -- **Attribution label: a-new-field-is-a-four-step-act-2026-08-31** (MINOR). Founder-set HARD RULE in the always-injected agent operating contract: before writing an unseen canvas key -- **(a)** is it NECESSARY or derivable, **(b)** does a SIMILAR field exist (`check_field_wiring.py --similar <name>`, now mechanical: asked for `raised_by` it returns nine existing `*_by` fields including the 42-use agent-invented `surfaced_by`), **(c)** DECLARE it in the schema same commit (88% of live canvas keys are declared by no schema, because `additionalProperties: true` makes an undeclared field legal), **(d)** WIRE it and name the consumer same commit. Content goes in the VALUE, never the key name. Hard rather than advisory because six of 38 promise-shaped fields had no consumer and one was written hours earlier in the release fixing that defect. Research logged at dogfood `landscape.yml#comp-120`.*
