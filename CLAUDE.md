...
*Version 0.107.1 -- **Attribution label: the-refactor-plan-is-parked-with-numbers-not-intentions-2026-08-08**. **Doc-only.** `docs/design/check-architecture-refactor.md`: an inventory, classification and refactor plan for the 23-check surface, written because the founder asked whether checks that each follow best practice individually form a good system collectively. MEASURED: they do not, and the sharpest cost is not duplication -- **session-start.sh takes 4.08s and spawns 33 python3 processes against a 5-second hook timeout, 82% of budget**, and a timed-out SessionStart degrades to SILENCE rather than to an error. 23 checks / 6,286 LOC; three shared libs exist and 5 of 23 import anything; 22 hand-roll argparse; empty-input honesty is in 3 of 23. **DECISION: PARKED**, because this improves the machine that improves the framework while products_shipped is 0/10. Parked with FIVE THRESHOLD TRIGGERS rather than a date -- session-start >= 4.5s, any observed timeout, check count >= 27, a shared primitive written a third time, or empty-input adoption stalling -- captured by /bvssh-check's existing Automation measurement rather than by a 24th check, which would be the joke the document is about. Un-parks to the measurement spike only, not to the full plan.*
