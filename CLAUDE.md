...
*Version 0.45.1 -- **Attribution label: gap-prevention-registry-2026-06-12** (lived-friction-triggered). Converts the 2026-06-12 gap analysis's prevention conclusions into mechanism: **Promise registry** in `engine/consistency-check-spec.md` (capability claims prose makes that nothing implements — seeded with P1–P4; P1 parked-resume already CLOSED v0.45.0; OPEN 3 consecutive framework-health runs → cluster escalation), two preemptive-registry rows (promises ship with mechanism or `Gated by:`; new canvas files ship with schemas), and `/framework-health` **step 4f Gap-Analysis Diff Audit** (re-verify prior findings before new fan-out — 4/6 stale premises was the week's biggest waste; promise-registry sweep; schema-coverage trend). **PATCH** (spec + skill step; no enforcement change). **Priors** (same day): v0.45.0 **gap-analysis-mechanisms** (Check 44 / Rule 5 promotion, diamonds validation + schemas, parked-resume, CHECK 0), v0.44.2 **gap-analysis-doc-backfill**, v0.44.1 **hooks-parity-evidence-guard**, v0.44.0 **write-narration-surface-expansion** (see `docs/changelog.md`).*
