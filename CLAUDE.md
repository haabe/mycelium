...
*Version 0.110.0 -- **Attribution label: seven-gates-that-could-only-go-red-after-the-push-2026-08-09**. CI ran **11** gates; the shipped pre-push hook ran **4**. Seven classes of defect could only ever surface as a red build after a push. That is `corrections.md` 2026-06-18 recurring -- its own prevention rule said to add each new CI gate to the hook, and it was applied to the gate that motivated it and to none of the seven that followed, because **a rule that depends on an author remembering it has a decay rate**. Fixed as one list, not two: `scripts/local-gate-set.txt` is the single source, the hook loops over it, and `check_gate_parity.py` (in the set, gating itself) fails the push if CI gains a gate the set has not got. The hook also warns when the INSTALLED copy differs from the shipped example -- measured on the author's own machine, one gate behind for weeks. Also adds `surface-registry.yml#metric-measurement` after a DORA measurement landed in mutable `current:` fields with no history row, and gives `/dora-check` a named, mandatory history append. 14 tests; the parity guard was verified to bite in four directions, and a test using a digit in a script name exposed that the first pattern could not see such a gate at all.*
