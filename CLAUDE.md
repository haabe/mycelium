...
*Version 0.46.3 -- **Attribution label: amabile-creativity-bias-grounding-2026-06-14** (lived-friction-triggered: dogfood deep-dive in mycelium-roadmap cross-referencing Teresa Amabile's creativity / inner-work-life research against the framework). Added one agent-facing bias item plus two human-doc citations, all from verified Amabile sources: (1) new **Negativity-as-Competence (Brilliant-but-Cruel)** item in `harness/cognitive-biases.md` -- the adversarial-review stack (`/devils-advocate`, scrutinize-style review, `/code-review`) structurally elevates harsh findings, so they get weighed on the evidence they cite, not on severity (Amabile 1983); (2) Amabile added to `docs/theories.md` Tier 3, mechanism-mapped to the new bias item + the philosophy backbone; (3) one-sentence componential-theory grounding in `docs/philosophy.md` ("Why opinionated discipline") for the separable-process bet under "harness, not model". Human-doc prose passed the voice gate (receipt + output-side pass; the tier-3 em-dash is the list's existing separator, "harness" is the domain noun). Doc + harness-guidance change; no schema or skill-behavior change. **PATCH**. **Priors**: v0.46.2 **readme-hero-positioning**, v0.46.1 **readme-jargon-reduction** (see `docs/changelog.md`).*
