...
*Version 0.80.1 -- **Attribution label: both-entry-formats-2026-08-03**. `check_correction_attribution.py` shipped in v0.78.0 to print an honest denominator, and its own denominator was wrong. `corrections.md` carries TWO entry shapes — `## 2026-05-03` headings and `- **Thing (2026-05-03, class)**:` bullets — and the parser matched only headings. Found the first time it ran against a freshly-appended log: five entries added that morning, every one carrying an explicit "Caught by ..." phrase, were invisible, and the tool reported "measured over 14 of 74 entries" while the corpus held 97. The bullet form is what recent entries actually use. Both are matched now, interleaved in one scan so a heading body cannot swallow the bullets after it, and a dateless bullet (`- **Prevention**:`) is not mistaken for an entry. Real corpus after the fix: 97 entries, 25 attributed, 26% coverage, and the escape rate moves 57% to 60%. Regression verified by reverting the parser and watching the three new fixtures fail.*
