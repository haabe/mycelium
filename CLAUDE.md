...
*Version 0.151.0 -- **Attribution label: the-five-unruled-fields-ruled-and-acted-on-2026-08-31** (MINOR). **`target_value` WIRED** via `check_target_progress.py`: it sat beside `current_value` and NOTHING read either -- two halves of a measurement, adjacent, never compared; the north star now reads 0 of 10. **`technical_capabilities_required` WIRED**, and the investigation found that `derived_from` is a SYNONYM for the canonical `trace.upstream[].target_id`, which appears ZERO times in the canvas against `derived_from`'s 12 -- so the alias was wired rather than the data migrated, and the dangling-reference machinery finally has edges. **`publication_cadence` DELETED** (all sub-fields null; kept as a tombstone so the ruling is findable). **`derived_at` human-only**; **`validation_status_per_dimension` ruled by its OWN schema text**, which already said it is for legibility not gate computation. 38 fields become 37, unwired 5 -> 3, all three carrying a written verdict.*
