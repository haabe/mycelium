...
*Version 0.29.0 -- **Attribution label: metrics-pull-anomaly-devils-advocate-chain**. First compound-dogfood mechanism shipping framework-side, after architectural sharpening session 2026-05-25: framework hosts primitives, roadmap composes them into operations; framework-side mechanisms must pass the **universal-product-model test** (does this serve any adopter regardless of what product they're building?). #4 from the compound-exercise design passes; #2/#3/#5/#6 do not and land roadmap-side. **Shipped**: new **Step 10** in `/mycelium:metrics-pull` SKILL.md — anomaly → devils-advocate auto-chain. When the report flags an anomaly with an *inferred explanation* (prose like "Plausible drivers (Unverified):", "Possible cause:", etc.), the skill automatically follows `/mycelium:devils-advocate` Technique 4 (Attribution-vs-Consistency Check) against each inference before presenting to user. Each inference labeled `verified | consistency_only | unverified`; contrarian reads generated; if consistency-only AND user about to append canvas evidence, prompt explicitly for Unverified-marker / further-investigation / drop. Closes the **AP#7 sub-class (g) implicit-causal-link** surface in real time — today's session manually applied this discipline (Google referrer ratio anomaly, 5-24 unique-cloner spike); Step 10 makes it automatic. Auto-increments cluster-instances.md `consistency-as-evidence` instance count for each downgrade. Skip Step 10 cleanly if no anomalies OR all anomalies are raw observations without inferred explanation (Technique 4 challenges explanations, not observations — honest "X happened, no idea why" is fine). **Universal-product-model fit verified**: the AP#7 sub-(g) risk applies to any adopter writing evidence to canvas from metrics — SaaS, course, AI tool, service. Not Mycelium-team-specific. No dogfood-mode toggle needed. **MINOR per version-discipline**: new feature surface on existing skill (Step 10 added); backward-compatible (additive; skips cleanly when no inferences to challenge; existing Steps 1-9 unchanged). v0.28.0 entry migrated to docs/changelog.md per Check 34.* **Prior**: skill-framework-dependency-frontmatter (v0.28.0).
