...
*Version 0.23.35 -- **Attribution label: lived-friction-triggered**. Workflow graduation of the plugin.json sync-lag pattern after three observed misses in the 2026-05-22 session (0.23.31/32/33) and one plumbing resolution (0.23.34). New rule in `engine/version-discipline.md :: Coordinated commit — files that must move together`: every Version-line bump is atomic across CLAUDE.md + plugin.json + changelog + decision-log (for decision-bearing patches). Closes the agent-workflow gap that the validators (Check 30 + Check 26) kept catching at push-time. Full incident log in corrections.md (TL;DR + full entry) so future agents read the rule before composing the first version-bump commit of any session. PATCH per version-discipline: corrections.md graduation + version-discipline.md rule addition + version-line + changelog + decision-log; no schema, skill, or behaviour change beyond the documented discipline.*
