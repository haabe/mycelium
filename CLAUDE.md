...
*Version 0.41.0 -- **Attribution label: autonomous-mode-fable5-dogfood-graduation-2026-06-11** (lived-friction-triggered). New run-mode convention: `engine/autonomous-mode.md` codifies running Mycelium with no person in the loop — explicit declaration only (run-prompt consent + `autonomous: true` root flag in `active.yml`; non-interactive detection alone never activates it, a present human always outranks the flag); three-rung substitution ladder (documented per-skill default → persona self-answer tagged `internal_simulated` → honest HARD GATE log-and-continue); mandatory substitution ledger at `.claude/evals/autonomous-run-log.md` with end-of-run self-audit; evidence-integrity boundary (self-answering interaction prompts permitted, fabricating external evidence never — evidence gates are expected to fail honestly); human-only registry keyed to `delegation-authority.md`'s no-standing list; harness-permission story (operator settings allowlist — unverified, gated by the first run under the doc — + mandatory write probe + documented `mycelium-state/` mirror fallback). Inline markers in `diamond-assess`, `diamond-progress` (required-approval + kill = autonomous hard gates; re-invocation-as-approval revoked for autonomous runs), and `mocked-persona-interview` (pre-commits self-authorable under the pre-commit ordering rule). Evidence: 2026-06-11 Fable 5 dogfood evaluation (opp-011, test-validated; receipts case `docs/receipts/cases/2026-06-11-fable5-autonomous-run.md`) — a headless run completed the full minimal path only because its run prompt pre-authorized exactly this rule set. **MINOR** (new convention with structural impact; no hook, schema, or validator change). **Prior**: docs-skills-indexes-catchup-and-render-fleet-receipts-voice-revise-2026-06-08 (v0.40.4).*
