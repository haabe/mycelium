...
*Version 0.31.1 -- **Attribution label: post-build-silence-nudge**. Surgical PATCH addressing one carry-forward from cohort-tester-2's friction log (mycelium-roadmap decision-log 2026-05-26): after a build/POC produces working code, the framework "just stopped and didn't prompt for more info or advise me what to do next." Closes the post-build silence seam with a single addition to `/mycelium:diamond-progress` SKILL.md — explicit next-steps offer-menu (security-review / threat-model / definition-of-done / reflexion / refine-spec / ship-as-is), citing risk shapes that fired earlier in `/mycelium:delivery-bootstrap`. Never auto-invokes; offer-menu only, consistent with v0.31.0's "framework offers menu, never pushes tools" principle. **PATCH per version-discipline**: small extension to existing skill, no new files, no new feature surface, additive only. Other tester-2 carry-forwards (framework-wide output density, L0-L2 discoverability hardening) deferred to post-surgery synthesis pass per `health-surgery-window` memory. v0.31.0 entry migrated to docs/changelog.md per Check 34.* **Prior**: jit-tooling-4-layer-composition (v0.31.0).
