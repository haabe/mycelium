...
*Version 0.48.1 -- **Attribution label: dora-firstpass-live-instrument-2026-06-15** (lived-friction-triggered: dogfood `/dora-check` found `apex.first_pass_success_rate` reading the dormant `pass-history.json` counter (0 runs → perpetual null) while the live battery output sat unread in ephemeral `/tmp`). `/dora-check` Part 2b first_pass_success_rate now reads scored battery aggregates under `.claude/evals/results/` as the PRIMARY live source (pass-history.json demoted to fallback), with two MANDATORY guards encoded from lived incidents: a **validity guard** (skip `invalid:true` / `token_usage.total==0` dead-runner artifacts — the 2026-06-12 phantom-baseline lesson) and an **expected-to-fail guard** (gap-prover scenarios excluded from the denominator, never dragging the number to a fake 0%). Skill-doc/behaviour change. **PATCH**. **Priors**: v0.48.0 **framework-health-instrumentation**, v0.47.0 **framework-health-remediation** (see `docs/changelog.md`).*
