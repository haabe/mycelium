...
*Version 0.58.1 -- **Attribution label: consumer-dogfood-test-and-architecture-doc-2026-07-25** (roadmap dogfood — hardens v0.58.0 + fills a doc gap; no runtime change). Adds the META-FIX from the v0.58.0 finding: a **consumer-perspective delivery test** (`tests/bash/test_session_start_contract_delivery.sh`) that runs the REAL SessionStart hook against a downstream-consumer-shaped project (`${CLAUDE_PLUGIN_ROOT}` set, no `CLAUDE.md`, empty `.claude/`) and asserts the operating contract is actually injected — the end-to-end runtime proof that complements Check 47's static wiring guard, and the exact test that would have caught the original gap. Also adds **`docs/architecture.md`**, the previously-missing single "how Mycelium is built and connected" map (engine/harness/hooks/skills/canvas, the always-on-vs-JiT context surface, packaging→release→install), consolidating what was scattered across five files. **Priors**: v0.58.0 **plugin-form-operating-contract-injection-2026-07-25** (the fix this hardens), v0.57.5 **evidence-type-swap-detector-2026-07-19** (see `docs/changelog.md`).*
