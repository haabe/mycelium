...
*Version 0.125.0 -- **Attribution label: the-name-grep-answered-a-question-about-behaviour-2026-08-24**. **P6 CLOSED, AND THE ROW'S OWN FINDING WAS WRONG.** The 2026-08-23 census reported three architecture fitness functions as running *"only in CI"*. **They were never CI-only.** All four are listed in `scripts/local-gate-set.txt`, and the pre-push hook is **data-driven off that file** — it loops the lines and runs each script, **naming none of them**. So a grep for a script name finds only `validate.yml` and reads as CI-only, while the gate has been blocking every push all along. **A name-grep answered a question about behaviour.** **WHAT WAS GENUINELY UNMET**: `scripts/gates.sh` -- the command a builder runs after a change, and the closest thing to G-V14's own *"project's verification command"* -- ran **four** gates against **fifteen** in the set, reading a different list from CI and pre-push. It is now driven off the same single source, so the three surfaces cannot drift apart again. `check_coverage_floor` is **DEFERRED there with its reason printed** rather than dropped: its input is a fresh `coverage.json` that only the pytest-with-coverage step produces, so running it here would either read a stale file -- green on numbers nobody just measured -- or fail on a precondition the builder did not break. **An absent gate set is NOT a failure**, deliberately: a consumer repo running the shipped wrapper has no `plugins/mycelium/scripts`, and failing them for that is a defect they cannot have.*
