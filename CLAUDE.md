...
*Version 0.56.0 -- **Attribution label: discovery-gate-2026-07-02** (auto-dogfood utterance-mode findings — three fixes). **(1) Discovery gate (MINOR, the teeth for the deliver-framed routing gap)**: new `hooks/discovery-gate.sh` blocks scaffolding NEW source files in a project where discovery has never been engaged (no diamond in `active.yml`, no populated `purpose.yml`). Router-discipline prose alone did not hold — the roadmap's first utterance-mode battery (2026-07-02) mechanically reproduced the founder-observed failure: a "build me a REST API" first message on an empty workspace produced code scaffolds to the turn cap, twice. Deliberately narrow against the friction-wall risk: Write tool only (Edit/MultiEdit never gated — brownfield untouched), new files only, source/infra shapes only, one-time on-the-record escape hatch (`.claude/state/discovery-skip-ack`, written after the USER explicitly declines). Wired on all three runtime surfaces (hooks.json / codex / cursor, per the v0.44.1 lesson); 13-assert scenario-per-guardpost test suite. **(2) /start AGENTS.md deferral (patch)**: in the /start composition, setup's Step 4 AGENTS.md question defers to after the brief, and setup detects an existing Mycelium section BEFORE asking — dogfood showed a first-time user's opening two exchanges going to file administration before interview Q1. **(3) ost-render fail-loud semantics (patch)**: "identifier absent from registry → fail loud" clarified to WHOLE-render block, no artifact emitted (a dogfood run read it as per-entry and rendered around the unregistered source); owner-facing prose may name the offender, the artifact must not exist. **Priors**: v0.55.3 **scenarios-schema-2026-07-01** (theory-fidelity B6/B7 program close), v0.55.2 **cynefin-rigor-correction-2026-07-01**, v0.55.1 **sinek-why-first-2026-07-01** (see `docs/changelog.md`).*
