...
*Version 0.128.0 -- **Attribution label: a-publish-is-not-finished-when-the-page-is-live-2026-08-24**. **Mycelium's model of a shareable output was a local file a human hands over** -- `receipt-render` is explicit that nothing is transmitted and the carrier is handed to a colleague. That model is sound and stays. **It had no answer for an output living at a persistent, updatable address**: a framework-wide search for `outputs/`, `published_at`, `artifact_url`, `hosted_at` and `permalink` returned NOTHING, so an agent publishing one had to invent where the source lives, how the address is recorded, and how a later session finds it again. Found by the **i-productified** dogfood project, measured at real cost in one session: **three artifacts published, addresses recorded only in prose inside long canvas entries and only because the founder asked, sources left in session scratch.** Had that session ended -- sources gone, addresses greppable only from prose, no update path, and a 20-minute external data pull lost with them. Ships as a convention in `engine/render-conventions.md` (durable source + structured record + supporting data) and `check_published_records.py`, advisory via SessionStart. **THE PATH STAYS PROJECT-LOCAL**: `outputs/` is NOT promoted to a framework concept, because hard-coding a directory would repeat the v0.88.0 defect of an artifact asserting a location it cannot know. **AND THE CHECK STATES ITS OWN LIMIT IN ITS DOCSTRING**: it validates records that EXIST and cannot see a publish that recorded nothing -- the render fleet is read-only, so there is no producer to gate. **That is the `gates_fired` shape**, named in the check itself so a green is never read as coverage of the commonest failure.*
