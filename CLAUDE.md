...
*Version 0.34.0 -- **Attribution label: reference-integrity**. Closes the orphan/dead-reference gap deferred since v0.32.0, driven by a 2026-05-30 reference-graph recon. Three classes closed in one bundle. (1) **Dead manifest entries**: `framework.directories`/`preserved_dir_readmes` listed `.claude/optimization/`, `.claude/tests/`, `.claude/tests/README.md` — no source in either tree, so `upgrade.sh` silently no-ops; removed, and the orphaned `2026-05-28-canvas-drift-reconciliation` receipt wired into every index. New `tests/python/test_manifest_coverage.py` guards going forward. (2) **The deferred dead-reference validator now exists**: `scripts/check_doc_references.py` follows every markdown link in the doc + plugin tree and fails CI on any that resolve nowhere, applying the real path model (file-relative, `.claude/`↔`plugins/mycelium/` dual-tree, and lexical `normpath` runtime-equivalent resolution because repo-root `.claude/` is partial). Dropped the recon's 377 raw hits — ~95% false positives — to 3 genuine broken links, all fixed; dedicated CI step + 8 tests incl. a standing "real tree stays clean" guard. (3) **Cross-doc drift swept**: "40+ frameworks"→30+, "11 gates" (naming DORA, missing Regulatory)→canonical 13-gate roster (12 baseline + conditional Explainability/XAI; reconciled a live 12-vs-13 split across plugin/marketplace descriptions vs the transition roster), 44→49 skills, dangling `/mycelium:opportunity`→`/mycelium:ost-builder`; the skills-count restale root-caused by extending `sync_derived.py` to sweep `diamond-assess/SKILL.md`. **MINOR** (new CI validator + manifest-coverage guard + generator coverage; no agent-behavior or schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: card-integrity (v0.33.0), deferred-generators (v0.32.0).*
