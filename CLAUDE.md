...
*Version 0.119.0 -- **Attribution label: the-warning-nobody-counted-2026-08-22**. **`shell-safety-guard` AND `absence-claim-guard` NOW LOG EVERY FIRE, WITH A SIGNATURE.** A session measured both: each fired correctly and repeatedly, each was read past, and each error they described happened anyway — a push reported successful when it had not landed, and a false absence claim that reached a released changelog. **That override was visible only in a transcript, and transcripts die.** The signature is what makes overrides countable WITHOUT self-reporting: the same signature firing again is an agent that was warned and carried on. This closes `opportunities.yml#sol-048a` for these two hooks — a guard whose action rate stays near zero must be narrowed or retired, which is unenforceable without a count. **8 tests**, including that a silent run writes nothing and that an unwritable state dir cannot break the warning.*
