...
*Version 0.110.8 -- **Attribution label: the-check-said-i-cannot-measure-and-the-hook-rendered-it-as-silence-2026-08-16**. `check_cycle_recording.py` correctly reports `no-releases-matched` with the pattern it used and the commits it scanned — its own docstring says silence there would reproduce the defect the check is named for — and then returned `status: "ok"`, which `session-start.sh` drops because it reacts only to named statuses. Measured in the dogfood project: two minor releases shipped upstream that day while the consumer repo doing the work got a silent ok/0, because the trigger counts releases in the project's OWN commits. "Could not measure" is now its own status and the hook surfaces it, worded without framework vocabulary and asking for nothing. The test that allowed this asserted the detail string and never the `status` field, so it passed identically before and after; two tests now read the field consumers actually read.*
