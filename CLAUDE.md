...
*Version 0.49.9 -- **Attribution label: legacy-path-rot-receipts-case-2026-06-18** (receipts case capturing the v0.49.6–v0.49.8 legacy-path-rot arc — surfaced by `/mycelium:framework-health` Step 4c). Adds `docs/receipts/cases/2026-06-18-legacy-path-rot-guard.md`: the dogfood house-cleaning found stale `.claude/{engine,orchestration,schemas}/` references the v0.49.5 link-only sweep structurally couldn't catch (they live in code-spans/prose), the fix shipped across three patches + a new `check_legacy_paths.py` CI guard, and the lesson — a passing check is a scoped claim, not a global one. Docs (receipts). **PATCH**. **Priors**: v0.49.8 **framework-guard-sync-message-harness-neutral-2026-06-18**, v0.49.7 **legacy-path-rot-runtime-strings-2026-06-18** (see `docs/changelog.md`).*
