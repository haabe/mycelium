...
*Version 0.116.0 -- **Attribution label: a-deferral-that-nothing-read-2026-08-21**. `check_cluster_reconcile` gains the direction it was missing: it walked corrections INTO the cluster catalogue and nothing walked a cluster OUT of it, so a cluster could meet its stated criterion and sit at `pending` forever while the file read identical either way — a gap named in the script's own docstring, where such cases *"were found by a human reading the file"*. **`pending` alone never fails**, because deferring is often right; the one real hold here was correct and reasoned. **What gets teeth is the DATE.** A cluster past its own `review_by:` fails, so silent re-deferral is a failing state rather than the default; a pending cluster with no date is reported, never failed, since demanding a date before the author has one just produces invented ones. Also adds `--today` so the tests do not read the wall clock — a review-date check whose only clock is `date.today()` has tests that pass the day they are written and rot after.*
