...
*Version 0.175.0 -- **Attribution label: a-correct-decision-that-produced-a-permanent-red-2026-09-03** (MINOR). `check_log_reconcile.py` had no way to record that an entry legitimately has no counterpart. A dogfood canvas archived a HALF leaf with no cycle row, deliberately: only the parent ever carried an ICE score, the schema forbids a zero-ICE product-leaf cycle, and **a zero would be ingested by calibration as a real score** — the corruption the founder's scoring ruling exists to prevent. The decision was right, reasoned, and written down **in a YAML comment**, which no checker can read. So a correct call produced **exit 1 on every run forever**, and a permanently-red check is one people stop reading — the failure this script family keeps finding elsewhere. `reconciliation_exempt: {reason, decided}` now records it as a VALUE, borrowing the `closed_with_discipline` shape deliberately: **a null recorded properly is an answer, not a gap.** Both companions are required and a reason must clear 20 characters, so `n/a` buys nothing. **It does not silence the row** — the id and its reason print every run, because the comment it replaced asked for exactly that visibility; only the exit status changes. Five negative controls: bare marker, token reason, undated reason, and a neighbour's exemption all still fail.*
