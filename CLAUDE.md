...
*Version 0.23.15 -- **Attribution label: lived-friction-triggered**. New Check 34: CLAUDE.md must contain at most one version entry. Mechanism-graduation of the discipline failure detected at v0.23.14 (deferred-entries-not-migrated, ×5 in one session). Detection: count lines matching `^\*Version [0-9]` in CLAUDE.md; fail if > 1. The latest entry stays; prior entries migrate to `docs/changelog.md`. Pairs with Check 30 (plugin.json#version tracks CLAUDE.md) as the doc-discipline-fast-failure family. This commit dogfoods the new check by migrating v0.23.14 out (move-the-old, not append-the-new). G-V12 note: no fixture test for this check — Check 33 and Check 31 (canvas-write Preflight) shipped without explicit fixtures because the bash-check logic is small enough to read as self-evident; same pattern applied here. If a future regression slips through this self-evidence threshold, ship a fixture then. PATCH per version-discipline: new validator check is observability-and-mechanism-strengthening for an existing discipline (CLAUDE.md format convention); no behavior change for downstream users (`tests/` doesn't ship via the plugin).*
