...
*Version 0.31.10 -- **Attribution label: hook-dual-path-hardening**. Critical-group audit fixes: (C1) `gate.sh` preflight resolution and (C2) `scope-gate.sh` helper resolution now prefer `${CLAUDE_PLUGIN_ROOT}` and fall back to legacy `.claude/`, mirroring `framework-guard.sh` — plugin installs have no `.claude/hooks` or `.claude/scripts`, so the hardcoded legacy paths silently no-op'd (preflight) or fail-closed deadlocked (scope-gate). (C3) `_manifest_lib.parse_manifest` now raises on indentation/structure drift instead of silently returning empty buckets, and `framework_guard.py` catches it to fail CLOSED — closing a fail-open hole where reindented `manifest.yml` would make every protected path writable. Regression tests added (`test_manifest_lib.py`). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: claudemd-dispatcher-refactor (v0.31.9), claudemd-size-ratchet (v0.31.8).*
