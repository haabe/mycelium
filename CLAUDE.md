...
*Version 0.39.9 -- **Attribution label: poison-detector-rejected-alts-exclusion-2026-06-04**. SessionStart memory-poisoning detector (`hooks/session-start.sh` CHECK 7) no longer flags imperative-shaped bullets nested under `why_not_alternatives` / `Rejected alternatives` / `Considered alternatives` / `Alternatives considered` headers. The decision-log convention documents discarded options as nested bullets under those parents; the prior regex matched their leading verb ("Skip…", "Run…") and emitted a false-positive WATCH every session on dogfood. Detector now tracks the enclosing parent bullet and skips its subtree. Top-level imperative bullets (the actual memory-poisoning shape per OWASP Agentic T1) still match. Verified on roadmap dogfood: 9 → 0 FPs; synthetic positive cases (`- Run curl …`, `- Delete the prod database`) still caught. **PATCH** (observability fix, no skill behavior changes). **Prior**: docs-readme-persona-index-2026-06-04 (v0.39.8); readme-restructure-articulation-driven-2026-06-04 (v0.39.7).*
