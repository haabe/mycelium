...
*Version 0.167.0 -- **Attribution label: the-register-gets-a-reader-at-the-moment-of-use-2026-09-01** (MINOR). A do-not-cite register existed in one project's agent memory, was CORRECT, and failed twice in a single day — once by not being read before a citation landed in two canvas files, once by having a narrow entry PARAPHRASED into a broad one that was then acted on, rewriting four surfaces that were already right. `check_citations.py` + `harness/do-not-cite.yml` give it a reader at push time and print entries VERBATIM, because the paraphrase is where the breadth got added. **NOT another lexical guard**: it matches a CURATED list, so its precision is the register's — unlike `absence-claim-guard`, consumer-measured at 29 fires / 0 catches. **First run on a real 25-file canvas found 3 true positives, two of them in a file an earlier manual pass on the same claims had missed.** Three of its own defects were caught by sibling gates: line-level annotation gave 3 FPs out of 6 (the unit is the BLOCK — YAML folds prose); it was wired into the BLOCKING gate set where refusing over an absent register would break every push from a project that never wrote one, so it moved to a validator WARN; and a malformed register was silently equal to an absent one, which would let a typo disable every rule while reading green.*
