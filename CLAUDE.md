...
*Version 0.139.1 -- **Attribution label: the-touch-log-was-checked-for-order-never-for-completeness-2026-08-27**. A dogfood task carries the line *"lost nine touches to prose-only logging"* — **and then lost a tenth the same way, inside the entry that documents the rule.** An inbound was written up as a field named `eighth_inbound_2026_08_26_...` and never entered `touch_log`, so `check_reply_owed` read the last inbound as five days old when it was one. **`check_touch_log_order` was green throughout, correctly: it checks that the entries which EXIST are ordered, and nothing asked whether the log was COMPLETE.** `check_contact_recorded_as_prose` treats a field NAME carrying both a date and a contact word as a claim that a contact happened, and reports it when no touch shares the date. **THE NARROWNESS IS THE CHECK, NOT A REFINEMENT OF IT**: measured on the live 26-task corpus, the naive "every dated field needs a touch" rule is 2 real hits against 47 false positives (`FOUNDER_RULING_2026_08_24`, `pre_registered_read_2026_08_21`, ...). Report-only, and it says so at the bottom of its own output: **some contacts genuinely have no known event date, and an entry dated by when it was REPORTED asserts an event date nobody has. **v0.139.1**: the check fired on a field written to EXPLAIN its own findings — `touch_dates_unknown_stated_2026_08_27` carries a contact word and a date, and that date is when the STATEMENT was made. Fields declaring the dates unknown are now skipped; `corrected` and `retracted` deliberately are NOT, because both live true positives are named that way.*
