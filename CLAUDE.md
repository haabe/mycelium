...
*Version 0.50.1 -- **Attribution label: receipt-render-killed-source-fix-2026-06-20** (lived-friction-triggered, dogfood). Fixes `/mycelium:receipt-render`'s "What got killed" section to read BOTH kill registries — `archived-solutions.yml` AND `cycle-history.yml` discard/kill outcomes. The v0.50.0 dogfood (roadmap L0 receipt) caught that reading only `archived-solutions.yml` under-reports work-avoided when a project records discards in `cycle-history.yml` (as the roadmap does), gutting the receipt's most load-bearing section. No new skill; corrects a just-shipped one. **PATCH**. **Priors**: v0.50.0 **receipt-render-l1-instrument-2026-06-20** (adds `/mycelium:receipt-render` + `docs/receipts/handoff.md`), v0.49.22 **cowork-dogfood-receipts-2026-06-19** (see `docs/changelog.md`).*
