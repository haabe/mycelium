...
*Version 0.40.4 -- **Attribution label: docs-skills-indexes-catchup-and-render-fleet-receipts-voice-revise-2026-06-08**. Docs-quality patch. Two coupled fixes: (1) `docs/skills/README.md` and `docs/skills/by-category.md` catch up to v0.40.x reality — the four render-fleet skills (`/diamond-render`, `/ost-render`, `/cycle-render`, `/render`) are added to both indexes under a new "Render & output" section; `by-category.md` also gains the five skills that were already missing before today (`/setup`, `/start`, `/migrate-from-legacy`, `/ping`, `/scaffold-cost-check`); "Last updated" bumped from 2026-05-08 to 2026-06-08 on both files. The hardcoded "54 skills" prose in `by-category.md` was the structural root cause — `sync_derived.py` only templated `README.md`'s skill-count token; `by-category.md` drifted silently. (2) `sync_derived.py` extended to include `docs/skills/by-category.md` in `SKILL_COUNT_FILES` so the drift cannot recur. (3) `docs/receipts/cases/2026-06-07-render-fleet-foundation.md` voice-revised per project-local `/voice-revise-framework-doc` skill (C-001 em-dash sweep ~17→1, opener lift with concrete-number triple, bookend close, parataxis-with-stinger at four compression moments, one Pratchett-mode ambush at F13 conclusion, banned AI-tells grep clean, substance fully preserved). **PATCH** (three docs/* files revised; one script extended; no skill, hook, engine, or schema behavior change). **Prior**: render-fleet-dispatcher-v0403-2026-06-08 (v0.40.3).*
