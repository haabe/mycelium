...
*Version 0.46.2 -- **Attribution label: readme-hero-positioning-2026-06-14** (lived-friction-triggered: peer-practitioner academic-language friction + desk-research adoption findings dogfooded in mycelium-roadmap). Reworked the README's first interaction for window-shoppers: (1) de-jargoned the hero's second paragraph (dropped the "two products / internal factory" abstraction for the direct lesson: the agent is fast, confident, and skips the deciding); (2) added a judgment-trustworthy line (the tool does not replace your judgment, it gives the agent enough feedback that the judgment that ships is still yours); (3) reordered so felt usefulness ("What it does" / "What it feels like") precedes the "Why this exists" manifesto. Grounded in adoption research (Rogers complexity/trialability; Hardgrave 2003 usefulness-not-belief; Kahneman & Klein high-validity-environment); positioning hypotheses to validate at observed-usage (ht-028). Core model names (Scales, Diamonds, L0–L5) and the agent-facing "earn the right to start" line KEPT; opp-005 in-product-vocabulary question stays evidence-gated. Doc-only human-surface change; no schema or skill-behavior change. **PATCH**. **Priors**: v0.46.1 **readme-jargon-reduction**, v0.46.0 **scenario-primitive-falsifiable-grounded** (see `docs/changelog.md`).*
