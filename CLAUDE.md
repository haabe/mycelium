...
*Version 0.60.2 -- **Attribution label: minilisp-showcase-2026-07-25** (docs/receipts; no runtime change). Adds the MiniLisp receipts case and rotates it onto the README highlights. A cold contact ran a Lisp interpreter end to end on **Vibe + Mistral with no Claude in the loop**, published the repo with the Mycelium dev files, and credited the framework unprompted; he consented to being named on 2026-06-24 and the showcase had gone unused for a month. Case carries the load-bearing finding: the **context-budget friction is two problems** -- window size, and rate-limit x file-operation-count on a rate-limited HPC -- and a bigger window fixes only the first. Honest boundary kept: educational project, so usable not adopted. Rotation closes the framework-health 4c staleness flag (newest case had been 36 days old) and adds a tell-me-what-broke invitation. **Priors**: v0.60.1 **verify-citations-matcher-2026-07-25** (see `docs/changelog.md`).*
