...
*Version 0.145.0 -- **Attribution label: two-purpose-checks-that-could-never-fire-2026-08-31** (MINOR, dogfood-found). **`check_purpose_stance.py` READ `why`/`how`/`what` AT TOP LEVEL ONLY, so a nested purpose hashed to one constant shared by every project in that shape and the drift branch could NEVER FIRE.** The checker now refuses to compare and says why. **THE SAME SHAPE ONE LEVEL UP: nothing anywhere checked whether `why` EXISTED.** `interview/SKILL.md` promised the user is *flagged for the deeper Phase-1 questions*; no script read `purpose["why"]` and no hook did either, so optional-at-entry was indistinguishable from permanently-empty. Now two mechanisms split by whether work was DERIVED from the purpose: a never-failing WARN at entry, and a `dependentSchemas` requirement that FAILS once `purpose_properties` exists. **`required` alone would not have worked** -- the template writes `why: ""`, which satisfies it; `minLength` carries the rule, verified by removing it and watching the test fail. **Emptiness now counts as absence in all three places**, after an `is not None` test in this release's own first fix passed `why: ""`. The empty-string case was found in the framework repo's `.claude/canvas/purpose.yml`, an untouched v0.1.0 template stub -- NOT evidence that Mycelium lacks a why, which is defined in the dogfood repo where product state lives; a first draft of this note claimed otherwise from one file.*
