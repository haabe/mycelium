...
*Version 0.138.0 -- **Attribution label: a-correct-decision-was-reported-as-a-permanent-defect-2026-08-27**. `check_instrument_contract` reported four dogfood instruments as INCOMPLETE for an empty `frozen_before` — **and all four are empty ON PURPOSE**, by a founder ruling written in each file: *"Leave empty, the silence is the finding."* Three are VOID agent-as-instrument tests that never self-logged, so there is no freeze event to name and inventing one is the exact failure the contract prevents. **The correct state could never satisfy the check**, so every run carried four entries meaning "working as intended" — and the next genuine INCOMPLETE would arrive in a list already 80% noise. Now: a sibling `<field>_absent_reason` key waives the field, **the field itself stays empty** (a sentinel VALUE would honour the ruling's spirit and violate its letter), the reason must be substantive so `n/a` buys nothing, and waivers are **PRINTED in their own section rather than suppressed** — an exemption nobody sees is how the next one gets granted without argument. **Fixing it exposed a latent parser bug**: `_frontmatter` read a folded `>-` block as the literal two-character value `">-"` and skipped every continuation line, so any long header field was silently truncated.*
