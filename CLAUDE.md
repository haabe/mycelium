...
*Version 0.162.0 -- **Attribution label: the-guards-watch-what-an-agent-asserts-not-what-it-reads-2026-09-01** (MINOR). From a consuming project's field report: an agent read a task's `next_moves`, named the wrong contact, and was corrected by the user — the task's own body said otherwise IN CAPITALS, in an entry dated NINETEEN DAYS AFTER the list. **No shipped guard could catch it**: nothing was inferred, absent or unsourced, so the absence-claim guard, read-before-research and the citation checks all pass. The agent trusted a SUMMARY over the detail that superseded it, in a file it had already read in full — and the canvas design produces this, because entries are append-only and dated while an instruction list is neither. New WARN compares dates only, reads no prose. **MEASURED BEFORE SHIPPING on two real projects**: 6 findings in the reporting one, **0 in the dogfood one**. Two false-positive classes were found by measuring and excluded — a list whose NAME declares it retired (`..._SUPERSEDED_..._KEPT_FOR_THE_RECORD`), and a record closed with the full discipline trio, reusing v0.160.0's rule rather than minting a second. A third was caught by its own test: `next_moves_updated` matched the pattern, so the check flagged the very field supplied to satisfy it. **The undated case is the common one** — two lists carried a date, five carried none, and a list that cannot be compared is how it goes stale unseen.*
