...
*Version 0.23.6 -- **Attribution label: mixed (research-while-here + lived-friction-triggered)**. Three coordinated security strengthenings. **(1)** New `docs/threat-model.md` — consolidated entry point for "what attacks does Mycelium worry about, and what does it do about them." Captures 7 Mycelium-specific threats (T-M1 Memory Poisoning, T-M2 Indirect Prompt Injection, T-M3 Secret Leakage, T-M4 Hook Tampering, T-M5 Sub-agent Escalation, T-M6 Audit-Trail Laundering, T-M7 Marketplace-Install Trust) structured against OWASP Top 10 for LLM Applications 2025 + OWASP Agentic AI Threats and Mitigations Feb 2025. Cross-references existing scattered pieces (harness/security-trust.md, docs/ai-system-card.md, anti-patterns.md cognitive cluster, context-management.md sister doc). Attribution: research-while-here. **(2)** New SessionStart hook Check 7 — memory-poisoning surveillance on recently-changed (<7 days) `.claude/memory/*.md` and `.claude/harness/decision-log.md`. Detects imperative-shaped bullet content (`Run`, `Execute`, `Delete`, `Send`, `Curl`, `Wget`, `Push`, `Force`, `Disable`, `Bypass`, `Skip`, `Ignore`, `Override`, `Fetch`, `Download`, `Install`, `Eval`, `Exec`) as PR-shipped-instruction signal. **Observability, not enforcement** — surfaces a warning, not a block. Conservative detection (low FP at cost of missed catches). Addresses OWASP Agentic T1 (Memory Poisoning) which was unmitigated at framework layer prior to this ship; the receipts/contributors-as-recognition GTM mechanism (landscape.yml#strategic_frame) deliberately creates an inbound contribution pathway for memory files — the structural defense follows the structural decision. Attribution: lived-friction-triggered (the 2026-05-11 BDSK-fabrication anti-pattern #7 instance + the receipts mechanism design are the load-bearing triggers). **(3)** Citation backfills in `harness/security-trust.md` Prompt-Injection Defense section — OWASP LLM Top 10 (2025) explicit (LLM01/02/05/06 named as load-bearing four for agentic dev-tool harness) + OWASP Agentic Threats (T1/T2/T15) added + Anthropic prompt-injection-defenses 2026 (substrate-layer RL refusal ~1% attack success on Opus 4.5) + Microsoft Spotlighting paper (arXiv 2403.14720, three techniques) as primary source. Tag-wrapping convention explicitly framed as folklore-grade alone, compounds with substrate defense. Attribution: research-while-here. PATCH per version-discipline (additive doc + new hook check + citation backfills; the memory-poisoning warning is observability not enforcement — no behavior change beyond visible surveillance). Connection to lived-friction: strong for item 2 (2026-05-11 anti-pattern #7 instance #9 in subagent-output-verification sub-class is the worked failure case the new threat model documents); medium for items 1+3 (research-driven from peer-product survey + canonical-framework gap analysis, no specific Mycelium failure forced the consolidation). Roadmap-side draft `mycelium-roadmap/.claude/drafts/security-strengthening-followups.md` tracks Tier 2 (6 candidates: secret-scan PreToolUse hook, hook-script SHA-pinning, capability manifest, Lasso claude-hooks PostToolUse convention, Validator Check 33 confidence-evidence coupling, framework_guard coverage 66%→85%+) and Tier 3 (5 deferred items).*
