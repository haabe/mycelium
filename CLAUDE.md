...
*Version 0.39.6 -- **Attribution label: sol-007a-technical-discovery-vocab-2026-06-03**. `/mycelium:diamond-progress` step 10 ("If blocked or needs evidence") extended with a **technical-discovery shape-detection branch** (sol-007a): when Evidence/Bias/Feasibility gates block AND canvas state shows an unverified external API/contract/schema/data-model, the verdict names "technical discovery" explicitly and routes to `/mycelium:assumption-test` with read-docs / pull-real-payload framing rather than `/mycelium:user-interview`. Closes opp-007 (Valiantsina Hryn 2026-06-01 inbound + roadmap brownfield-iteration eval 2026-06-03 sw-tech-discovery dogfood, which passed 6/7 with `decision_log_contains` failing — the framework was structurally gating correctly but didn't NAME or ROUTE the technical-discovery dimension). Behaviorally minimal: no new scale, no new diamond, no new gate, no new skill. The existing Evidence + Bias + AP#7 + Cagan-feasibility gates already do the blocking work; this version surfaces the vocabulary and corrects the routing. **PATCH** (vocabulary + routing branch in an existing skill). **Prior**: v0.39.4-validator-followup-2026-06-03 (v0.39.5); canvas-first-on-product-questions-2026-06-03 (v0.39.4).*
