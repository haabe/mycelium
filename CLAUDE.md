...
*Version 0.48.0 -- **Attribution label: framework-health-instrumentation-2026-06-15** (lived-friction-triggered: same `/framework-health` run; closes the two instrumentation gaps it flagged). (1) Cycle records gain **`gates_fired`** (which theory gates fired + pass/fail) and an in-cycle **`regressions`** block (phase step-backs during a cycle, distinct from post-delivery rework) — closes the Gate-effectiveness + Regression-rate dimensions that were unInstrumented in `cycle-history`; schema in `engine/cycle-learning.md`, populated by `/retrospective` from analysis it already performs, with absence recorded explicitly per AP#9. (2) **`/preflight`** gains a re-forecast trigger: audit-triggered / emergent work gets an explicit estimate and re-scopes when it crosses ~2× estimate or had none (the 2h→9h / session→14h balloon pattern). Schema + skill-behavior change. **MINOR**. **Priors**: v0.47.0 **framework-health-remediation**, v0.46.3 **amabile-creativity-bias-grounding** (see `docs/changelog.md`).*
