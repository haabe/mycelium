...
*Version 0.39.20 -- **Attribution label: faros-counter-receipts-rotation-2026-06-07**. Follow-on to v0.39.19 (Faros + Datadog integration) executing the framework-health recommendations: (a) new receipts case `docs/receipts/cases/2026-06-07-faros-whiplash-integration.md` captures the assessment-as-amendment + Level B distinction + scaffold-cost first dogfood baseline + premise-check honesty pattern. (b) README "How Mycelium got smarter" rotation: `2026-04-macos-fileviewer` → `2026-06-07-faros-whiplash-integration`. (c) `/mycelium:dora-check` Part 2b gains the Goodhart counter `apex.first_pass_success_rate` computed from `evals/pass-history.json` — pairs with `ai_rework_rate` + `hook_detection_rate` shipped v0.39.19, honoring the same-session scaffold-mistaken-for-instrumentation discipline (every new metric ships with its counter at the same maturity). Honest data-gap: pass-history currently shows 0 runs across active evals; the COMPUTE is real, the field reports `N/A` honestly until eval-runner is in regular use. **PATCH** (skill MANDATORY extension + docs receipts case + README rotation; no engine/hook behavior change). **Prior**: faros-whiplash-plus-datadog-soae-integration-2026-06-07 (v0.39.19).*
