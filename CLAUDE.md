...
*Version 0.118.1 -- **Attribution label: the-footer-nobody-typed-2026-08-21**. **ONE LINE, AND IT WAS TIED FOR THE MOST GENERATED TEXT IN THE FILE.** A blind forensic read scored the citation footer 8/10 machine-written on a single phrase: *"the intellectual foundations this system builds upon"*, which nobody types by hand. **Both substantive claims are kept** — not affiliated, no endorsement implied. **"For educational purposes" is dropped**: asserting fair use does not create it, so the phrase was belt-and-braces rather than load-bearing. The disclaimer is now shorter and says the same two things in words a person would use.*
