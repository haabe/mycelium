...
*Version 0.81.0 -- **Attribution label: harness-over-notes-2026-08-03**. Two mechanisms for a failure class that had three memory files and kept happening. In one session the agent hit documented shell and verification traps EIGHT times, and one produced a wrong answer to the operator: `which opencode | head -1; echo $?` reports `head`'s status, so a tool was declared missing when the binary was merely absent from PATH. Per the Lopopolo reframe this project runs on, a correction that recurs against an existing note needs a mechanism, not a better note — a note is read at session start and decays. **`hooks/shell-safety-guard.sh`** (PreToolUse on Bash) warns on `$?` after a pipeline (POSIX: it reports the pipeline's LAST command; remedy `${PIPESTATUS[0]}`, and the warning suppresses itself when that is already present), on backticks (ShellCheck SC2006, and because backticks inside a double-quoted string are command substitution — a markdown code span in a commit message gets executed), and on `grep` gating an `&&` chain (grep(1) exits 1 when nothing matched, which is often the desired answer). It WARNS and never denies: every pattern has legitimate uses, and a guard that blocks real work gets disabled. **`scripts/safe_replace.py`** fixes the ordering bug in this project's own scripted-edit rule. "Assert the replacement landed" produced a WORSE failure than ignoring it — a script patching two files with a shared anchor wrote the first, raised on the second, and left the tree half-edited, because the two "mirror" files were not verbatim copies. `write; assert` turns a bad assertion into a partial edit, which is worse than no assertion because the tree is now in a state nobody described. It validates EVERY anchor, reports EVERY failure at once, and writes nothing unless all match; expected occurrence counts are a precondition rather than an afterthought remembered wrongly. The agent-operating-contract mandates it for any scripted edit touching more than one file or anchor, and it made its own wiring edits. **Stated limit**: cwd persistence across Bash calls is the fourth trap in this family and is invisible to a PreToolUse hook, which sees one command; that needs cross-invocation state and is not pretended at.*
