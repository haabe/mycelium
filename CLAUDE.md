...
*Version 0.46.0 -- **Attribution label: scenario-primitive-falsifiable-grounded-2026-06-13** (lived-friction-triggered: peer-practitioner feedback on academic-language friction + the scenario primitive). Two upgrades make the Hoskins scenario an *advanced, falsifiable user story*: (1) `simulation.success_criteria` (observable + measure + threshold) as the `/eval-runner` assertion, kept alongside the qualitative `success_state`; (2) a `provenance.source_class` grounding rule — `internal_simulated` / `speculation` scenarios are envision-only and cannot drive design, confidence, or gates until a real `external_human` / `external_data` source grounds them. Authored in `/user-interview`, propagated to `/ost-builder`, enforced by two new `/canvas-health` Check 8b lints (falsifiable-success + grounding). Also: README "What it feels like" re-skeletoned from framework modes to user-situation scenarios (Hoskins) and de-jargoned, voice-revised. **MINOR**. **Priors**: v0.45.2 **chat-ux-4e-graduation**, v0.45.1 **gap-prevention-registry**, v0.45.0 **gap-analysis-mechanisms** (see `docs/changelog.md`).*
