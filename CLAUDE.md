...
*Version 0.37.0 -- **Attribution label: delegation-authority**. Adds `harness/delegation-authority.md`: a decision-authority layer that binds the existing BLOCK/REVIEW/NUDGE tiers to *execution* decisions (not just epistemic discipline), keyed to **consequence** = effective-reversibility × aggregate-blast-radius. Names the **no-standing** decisions that are absolutely human (which bet to make; security/privacy/regulatory/ethics tradeoffs; editing the authority map itself), with round-up-under-uncertainty and an escape-hatch override. Gives behavioral-contract **N9** its missing repo anchor and adds **N10**. Derived from an ADLC ("agents execute, humans govern") gap analysis — Mycelium was a superset of ADLC for Intent/Govern/Validate but lacked an explicit execution-authority model. Wires into `guardrails.md`, `diamond-rules.md` (the Develop→Deliver delegate seam), `theory-gates.md` (deploy authority is the consequence rule, *not* a new gate), the delivery domain (L4 continuous-validation = the fine-grained concurrency ADLC names), and a new adversarial eval (`deploy-delete-escalation.yml`). The risk-envelope canvas field is demand-pulled (added when a real L4 diamond needs one). **MINOR** (new doctrine + behavioral-contract rows; no schema change, outcome-neutral on product diamonds). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: ruff-cleanup (v0.36.3), framework-health-followthrough (v0.36.2).*
