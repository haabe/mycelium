...
*Version 0.54.0 -- **Attribution label: torres-selection-authority-2026-07-01** (theory-fidelity B4). Makes Torres's OST solution-selection mechanism REAL (not softened, not a nudge): solutions graduate OST→GIST by **assumption-test verdict**, not by ICE score. Fixes the audit's Torres finding — `leaf-lifecycle.md` Phase 5 required `ICE ≥ 100` (a scoring-for-selection gate Torres explicitly cautions against) and Phase 3 auto-killed leaves on `ICE < threshold`, while the file's own anti-pattern #5 said "never discard on ICE alone" (self-contradiction). Now: **Gate = the #1 riskiest assumption has a recorded test verdict of `validated`**; ICE demoted to advisory (sequences validated leaves + feeds calibration; never gates/kills). Discard only on an `invalidated` assumption; a `partial` verdict graduates neither (re-test/scope down). Teeth: `/canvas-health` step 8 upgraded from "source leaf exists" to "source leaf's riskiest assumption == `validated`", enforced by the **G-L2 REVIEW** gate (blocks) — the framework's semantic-enforcement surface (a CI validator was deliberately NOT built: the OST→GIST leaf mechanism has ZERO live instances in dogfood — all `solutions: []` — so hard CI enforcement would be speculative, JiT-gated on real leaves). Scenario-designed: happy (validated→graduate), sad (validated+low-ICE→deprioritize not kill), bad (high-ICE+untested→BLOCK), partial (limbo→re-test). **Blind-tested BEFORE commit** — adversary caught that the first pass left Phase 3's ICE kill-gate intact (self-contradiction), lacked any verdict check (a nudge), and missed the `partial` path + a straggler Hoskins-4-element bug in canvas-health; all fixed. Updated: leaf-lifecycle, guardrails(+index+delivery), anti-patterns #5, canvas-health, theories.md. **MINOR** (OST→GIST gate behaviour change). **Priors**: v0.53.1 **theory-fidelity-corrections-2026-07-01** (Track 1), v0.53.0 **outcome-discovery-loop-move1-2026-07-01** (see `docs/changelog.md`).*
