...
*Version 0.135.0 -- **Attribution label: three-dogfood-checks-were-proven-and-never-shipped-2026-08-26**. **Three checks had run in the dogfood repo for weeks, caught real defects there, and existed nowhere a consumer could reach.** Now upstream: `check_touch_log_order` (an out-of-order `touch_log` parses cleanly and silently answers "what happened last" wrong — it reported two sent replies as unsent), `check_upstream_candidates` (re-probes a surfaced-friction registry and reports LANDED/REGRESSED, because shipped work reading as open is what makes a candidate pile regrow), and `check_fail_open` (the anti-pattern #9 signature — an `except` whose handler returns a benign value and says nothing). **`check_fail_open` ships REPORT-ONLY and that is deliberate**: it finds 42 sites in this tree, 31 of them SILENT, and **no baseline was seeded** because `ruff.toml` forbids re-introducing tolerated debt. Each needs a written verdict before `--strict`. **A fourth, `check_evidence_links`, was NOT shipped** — 29 lint violations, 5 structural; it needs a real refactor, not a suppression list.*
