...
*Version 0.23.22 -- **Attribution label: lived-friction-triggered**. Two layered mechanisms graduated from the recurrence-#3 named-attribution-leak class (logged in `corrections.md` 2026-05-15): (1) Check 33 now context-aware — `$CI` / `$GITHUB_ACTIONS` set → fail-open as before (CI runners legitimately can't reach the private companion repo's registry); framework-self-host context (plugin manifest present AND CLAUDE.md starts `# Mycelium:`) with registry absent → WARN with explicit setup guidance (was INFO); other contexts unchanged. Catches the local-dev-without-registry case that was the v0.23.21 leak vector. (2) Pre-push hook (shipped v0.23.20 as `git-pre-push-example.sh`) extended to also run `tests/validate-template.sh` when present — adds Check 33 and the other structural-integrity checks as a push-time backstop. Downstream user projects don't ship `tests/`; they skip this branch gracefully. Both layers are mechanical replacements for the auto-memory cognitive gate that failed three times in two weeks. Out of scope for this PATCH: (a) cleanup of 3 pre-existing leaks on origin from v0.23.13 / v0.23.10 / v0.18.x — separate regenericize PATCH; (b) graduation of Check 33 leak detection from WARN to FAIL — depends on (a) clearing first. PATCH per version-discipline: validator behavior change + hook script extension; no schema change, no skill change, no behavior change for downstream users beyond the (correctly-shaped) push-time backstop.*
