...
*Version 0.23.16 -- **Attribution label: lived-friction-triggered**. Framework-on-framework exemption for `/mycelium:framework-health`. This repo dogfoods Mycelium on itself, so improvements ship as commits and graduate through `corrections.md` → mechanism rather than through OST leaves with predicted ICE → actual outcome. The skill's Step 1 was returning early on N=0 cycles, masking the framework's actual learning signal (21 corrections in memory, recent graduations into Checks 30–34). Detection added: project root contains `plugins/mycelium/.claude-plugin/plugin.json` AND `CLAUDE.md` begins with `# Mycelium:` → route to a corrections-graduation summary instead of cycle-count gate; still runs Steps 2b/4b/4c/4d. Exemption documented in `engine/cycle-learning.md` with reconsider-trigger (second product surface). PATCH per version-discipline: skill routing change + doc clarification; no schema change to `cycle-history.yml`, no new validator check, no behavior change for non-self-host downstream users.*
