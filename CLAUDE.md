...
*Version 0.45.0 -- **Attribution label: gap-analysis-mechanisms-2026-06-12** (lived-friction-triggered). The mechanism batch from the 2026-06-12 five-dimension gap analysis: **Check 44** (consistency-check-spec Rule 5 per-rule promotion, narrowed core — hooks*.json script existence + cross-surface registration parity; cluster bar 2/3); **diamonds state validation** (`validate_canvas.py` parse-checks `.claude/diamonds/*.yml` + validates active.yml against new `schemas/diamonds/active.schema.json` incl. the definition_of_done shape — live proof: dogfood active.yml sat committed-unparseable ≥3 days undetected); tier-1 canvas schemas (purpose/north-star/gist) + WARN-on-schema-less; parked-diamond resume surfacing actually implemented in `/diamond-assess` + `/feedback-review`; SessionStart CHECK 0 state-parse sentinel. **MINOR**. **Priors** (same day): v0.44.2 **gap-analysis-doc-backfill** (communication-rules 9/9, skills index 55/55, autonomous-mode/environment/uninstall pages), v0.44.1 **hooks-parity-evidence-guard** (guard registered on Codex+Cursor — was Claude-Code-only), v0.44.0 **write-narration-surface-expansion** (see `docs/changelog.md`).*
