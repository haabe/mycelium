...
*Version 0.31.2 -- **Attribution label: bluf-footnote-output-convention**. Second carry-forward from cohort-tester-2 ("brain fried from gigantic walls of text") closed. Deep cross-angle research (cognitive load theory, accessibility/WCAG-COGA, information architecture, need-to-know hierarchy, AI-agent UX) diagnosed the friction as a **layering problem, not a verbosity problem**: discipline-visibility metadata (citations, `verified|consistency_only|unverified` labels, why-not-alternatives, anti-pattern refs, recommended next skills) is currently emitted at the same fidelity, visual hierarchy, and emission as the actionable claim. **Solution**: new `G-C1` NUDGE-level guardrail in `harness/guardrails-core.md` + corresponding clause in `CLAUDE.md` Communication Rules — every emission with discipline-visibility metadata layers in three blocks: (1) BLUF (1-2 lines, plain register, actionable claim, no inline citations), (2) Rationale (scannable middle, no attribution metadata inline), (3) Discipline notes under `---` rule (citations, attribution labels, why-not-alts, recommended skills — load-bearing, below the fold). Theory-discipline visibility preserved by construction. Convention is a nudge not a limit; 3-line and 50-line emissions both satisfy it. Audience-tier sensitivity encouraged (junior/designer → plain register; theory-fluent → denser trailing block OK). Research basis: Sweller (CLT), Cowan 2001 (~4 chunks), Nielsen NN/g (F-pattern), Minto Pyramid, BLUF (military/business), W3C COGA + WCAG 3.0 cognitive accessibility working draft. **PATCH per version-discipline**: prose convention only, two files touched, no new files, additive. **Honesty caveat**: chain "wall-of-text → cohort attrition" is *consistency_only* at N=1 (cohort-tester-2); convention is research-informed, not research-validated for the Claude Code / Cursor / Codex surface specifically — a `/mycelium:prompt-optimizer` A/B is the right next step. L0-L2 discoverability hardening (third tester-2 carry-forward) remains deferred to post-surgery synthesis. v0.31.1 entry migrated to docs/changelog.md per Check 34.* **Prior**: post-build-silence-nudge (v0.31.1).
