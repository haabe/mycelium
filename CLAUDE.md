...
*Version 0.23.20 -- **Attribution label: lived-friction-triggered**. Canvas-validation integration capability shipped — `validate_canvas.py` already existed and was invocable, but its place in the pre-push pipeline was undocumented, so v0.23.19's failure mode recurred: two CI failures on 2026-05-15 (commits 43989ae, 483dd86) because the template validator passed locally but the canvas schema validator was not run before push. Three earlier discussion rounds (Option A doc-only / Option B PostToolUse hook / pre-push hook proposal) collapsed to the smaller correct shape after user pushback ("Won't the users set up their own pre-commit scripts for their project?"): Mycelium ships the *capability* (existing `validate_canvas.py` + a new reference hook script at `plugins/mycelium/scripts/git-pre-push-example.sh`) but does NOT install it for plugin users. Their `.git/hooks/` is per-clone state outside the plugin's official surface; they have or will have their own hook tooling. Documentation added to `docs/contributing/README.md` with the integration pattern (call `validate_canvas.py` from husky/lefthook/pre-commit/plain bash) plus opt-in install command for the reference hook. Mycelium-repo + roadmap-repo install the local pre-push hook now (dogfooding, per-clone, not in git). PATCH per version-discipline: new reference script + docs section; no schema change, no skill change, no behavior change for downstream users beyond the new capability discoverability.*
