...
*Version 0.32.0 -- **Attribution label: deferred-generators**. Closes two of the three generators deferred in v0.31.12. New `scripts/sync_derived.py` rewrites mechanically-derived tokens from their single source: the framework version (from CLAUDE.md's `*Version` line) into `plugin.json`, and the skill count (from `plugins/mycelium/skills/*/SKILL.md`) into every "N skills" token across CLAUDE.md, README.md, docs/skills/README.md, plugin.json, marketplace.json. `--check` reports drift without writing (CI/pre-push); the existing Checks 6/7/10/30 stay the enforcement net. CLAUDE.md prose and plugin descriptions stay hand-written — only derived tokens move. Manifest auto-generation is intentionally NOT built: its framework/project_state classification is a semantic judgement, not derivable from a tree walk — the right complement is an orphan-coverage validator, logged as the one remaining deferral. 7 unit tests. **MINOR** (new maintainer tooling; no agent-behavior, canvas, or schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: audit-medium-group (v0.31.12), audit-high-group (v0.31.11).*
