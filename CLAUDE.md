...
*Version 0.39.0 -- **Attribution label: framework-health-followup-2026-06-02**. /mycelium:framework-health run on the dogfood repo surfaced two structural gaps: (1) confidence-calibration permanently dark because cycles had no class field to distinguish OST-leaf delivery (ICE required) from framework-self-development (ICE not applicable); (2) `documented-rule-diverges-from-enforcement` cluster at 13 instances had no per-rule promotion path, forcing the spec→mechanism transition into all-or-nothing. Shipped: **Check 38** (cycle_class discipline — product-leaf cycles must carry non-zero ICE) wired into `/mycelium:ice-score`, `/mycelium:diamond-progress`, `/mycelium:retrospective`, with `cycle_class: product-leaf | meta-dogfood | observation` required field added to `engine/cycle-learning.md`. **Check 39** (Rule 4 of consistency-check-spec — rendering-spec docs must carry STRICT or illustrative marker) promotes one cluster rule independently per a new "per-rule promotion" path in the spec; cluster overall stays at spec status (1/3 rules implemented). Receipts highlights rotation: plugin-form-dogfood (2026-05-09, "subagent-simulation ≠ lived friction" graduation) replaces framework-self-correction on README. Both new checks ship G-V12 fixture tests (10 + 9 assertions, all green); Check 37 confirms full coverage at 31 declared checks. **MINOR** (two new validator gates + one new mandatory protocol + one new convention with structural impact in the consistency-check spec). **Prior**: architecture-discovery-narrowed-receipt (v0.38.4); mental-model-docs-page (v0.38.3); framework-health-remediation (v0.38.2); guardrail-count-sync (v0.38.1).*
