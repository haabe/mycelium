...
*Version 0.23.24 -- **Attribution label: external-validation-triggered**. Two-step verification arc on a Mycelium → opencode port (sst/opencode) graduates two delivery patterns and the framework's first real decision-log entries. Phase 0 (desk + binary inspection) estimated a ~1–3 day adapter and flagged three runtime unknowns; Phase 1 (headless test against opencode 1.15.1 + local Ollama) overturned all three: `tui.prompt.append` silent in headless mode, `tool.execute.after` fires success-only, Read-before-Edit not runtime-enforced. Adapter cost inverts to ~1–2 weeks; PR deferred indefinitely. Two patterns extracted into `patterns.md` — "don't infer runtime enforcement from schema strings" and "symmetric API names don't imply symmetric semantics" — both structurally adjacent to anti-pattern #7 (consistency-as-evidence). Two receipts under `docs/receipts/cases/`; decision-log gains its first two entries (initial decision + post-Phase-1 re-scope). PATCH per version-discipline: documentation + decision-log + memory additions only; no schema change, no skill change, no behavior change for downstream users.*
