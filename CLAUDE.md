...
*Version 0.133.0 -- **Attribution label: the-guard-fired-on-substrings-inside-longer-words-2026-08-24**. **`read_before_research_guard.py` matched terms with `low in line.lower()`, so a term fired anywhere inside a longer word: "Verna" matched every occurrence of "go-VERNA-nce".** Now word-boundary, via lookarounds rather than `\b` so quoted PHRASES with edge punctuation still match. **TUNED FROM THE LOG, WHICH IS WHAT v0.127.0 SHIPPED THE LOG FOR** -- that release predicted the guard would be noisy in a canvas naming hundreds of entities and committed to measuring rather than guessing. Six days later `.claude/state/read-before-research-log.jsonl` held **4 firings, 3 of them noise**, two from that one collision. **AND THE NOISE WAS NOT MERELY ADDITIVE**: in both collision firings the REAL entities returned zero canvas hits, while the false match consumed all three display slots -- so a true *nothing known* was presented to the agent as *the canvas already records this*, which inverts the guard's purpose. A second, separate class also came from the log: one firing returned three hits ALL matching `Product` inside schema comments like `# Product type: ai_tool`. Whole-word, so the boundary fix does not reach them -- job titles and canvas structural vocabulary are now stopped (`Product`, `Growth`, `Chief`, `Officer`, `Head`, `Panel`, `Event`). **The stop list grew from OBSERVED firings only**, per its own note that over-filtering on intuition reintroduces the gap.*
