...
*Version 0.39.22 -- **Attribution label: framework-health-temporal-independence-2026-06-07**. `/mycelium:framework-health` Step 4e gains an explicit temporal-independence rule: graduations require two assessments separated by independent observation windows (quarterly run, cycle-count-trigger run, or deliberate re-audit after skill-template edits), not same-day re-runs of the same flag set. Rule applies symmetrically to 4b (cluster graduation-readiness) and 4d (docs health) — re-flagging in the same session does not count as independent confirmation. Surfaced when a same-day re-run of `/mycelium:framework-health` would have mechanically graduated the morning run's 4e baseline (six skills flagged: canvas-update, ost-builder, ice-score, canvas-health, bvssh-check, dora-check) on the second pass, with no intervening skill edit between observations. **PATCH** (one skill doc tightened; no engine, hook, or schema behavior change). **Prior**: faros-whiplash-receipts-voice-revise-2026-06-07 (v0.39.21).*
