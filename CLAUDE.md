...
*Version 0.49.7 -- **Attribution label: legacy-path-rot-runtime-strings-2026-06-18** (follow-up to v0.49.6, refining its boundary: extended the sweep from docs into runtime-surfaced references and fixed one real plugin-form bug). Fixes: agent-facing messages in `scripts/framework_guard.py` + `hooks/stop-check.sh`; the `scripts/ingest_warnings.py` docstring + the `warnings-log.md` header it generates; the `scripts/validate_canvas.py` docstring; and the descriptive `engine/surfaces.yml` registry — all repointed off the moved `.claude/{engine,orchestration,schemas}/` dirs (surfaces.yml is descriptive metadata, not a dual-tree map, so it was in scope after all). Bug: `hooks/post-write-nudge.sh` schema-existence check resolved `$PROJECT_DIR/.claude/schemas/`, which never matches in plugin form — plugin users silently lost the schema-validation nudge; now resolves `${CLAUDE_PLUGIN_ROOT}` with a legacy fallback. Left + flagged for a maintainer call (semantic, not path rot): the dogfood-sync workflow guidance in `framework_guard.py` (`.claude/scripts/upgrade.sh`) couples to allowlist logic + the plugin-form sync model; `manifest.yml` + `provision-skills.sh` legitimately operate on the runtime tree. Scripts/hooks/config. **PATCH**. **Priors**: v0.49.6 **legacy-path-rot-sweep-2026-06-18**, v0.49.5 **docs-dead-link-sweep-2026-06-16** (see `docs/changelog.md`).*
