...
*Version 0.39.2 -- **Attribution label: rule-6-fp-measurement-2026-06-02**. /mycelium:framework-health (run #2 same day) followed up its own recommendation #2 to unblock Rule 6 (Test-driver drift) of the `documented-rule-diverges-from-enforcement` cluster. FP measurement executed against the cluster's instance corpus; surfaced two findings. **(1) New escape valve added** to `consistency-check-spec.md#escape-valves`: "framework-wide procedural writes" — writes to `harness/decision-log.md` (G-P4 rule) and `diamonds/active.yml` (theory-gates rule) are mandated framework-wide, not skill-specific, so SKILL.mds correctly don't repeat them. Roadmap-side `check_skill_prompt_drift.py` updated to honor the escape valve. **(2) Promotion-bar shape mismatch surfaced**: after escape valve, 21/28 task fns still flagged (75%, far above 5% bar). Residual flags are dominantly conditional/scenario-dependent writes by Phase-5 Option-C design — test-scaffold simplification, not drift. Rule 6 is a maintainer-review-surfacer (flag-and-decide), not a validator (flag-and-fix); the cluster's `<5% FP rate` criterion fits validator-shaped rules. Rule 6 status updated from "near-ready (FP-pending)" to "promotion-bar-shape-mismatch — needs Path A (revise bar to per-flag-decision-log) or Path B (graduate Rule 3 or 5 instead) before cluster bar can progress." Cluster bar stays at 1/3 implemented rules. AP#7 instance #13 (conversational-reasoning sub-class) also logged this session in roadmap memory; sub-graduation candidate sharpened to Read-before-Recommend when topic has any extant `opportunities.yml` entry. **PATCH** (doc-only spec edit; no skill/gate/hook change). **Prior**: readme-language-revision-2026-06-02 (v0.39.1); framework-health-followup-2026-06-02 (v0.39.0).*
