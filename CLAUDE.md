...
*Version 0.23.1 -- CLAUDE.md restructure: version history moved to `docs/changelog.md` (was 67k chars / 262 lines bloating every session load). Current-version summary kept here as agent priming + Check 26 version-bump trigger; full per-version detail lives in the changelog. PATCH per version-discipline (doc-only restructure; no new mechanisms; behavioral content unchanged).*
