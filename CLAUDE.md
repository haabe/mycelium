...
*Version 0.174.0 -- **Attribution label: the-horizon-exemption-never-travelled-to-the-sibling-subcheck-2026-09-03** (PATCH-shaped MINOR). A `/mycelium:canvas-health` dogfood run flagged **11 stale human-tasks and all 11 had a horizon in the FUTURE** — a 100% false-positive rate, against this skill's own written standard that *"a check that fires on 80% of a corpus trains its reader to skip it."* **The repair already existed one sub-check away.** v0.90.0 added the future-horizon exemption to sub-check **(b)** for the identical stated reason — *"the check demanded a reason that was already written in the field beside it"* — and it was never carried across to **(a)**. (a) was edited again at v0.132.0 to skip `status: watching`, so it has been touched since, and the exemption still did not travel. **One rule, two adjacent sub-checks, one repaired** — the fourth instance of that shape found in a single day, after the reply-owed rule, the `gates.sh` waiver parser, and Check 7's file list. (a) now skips a future horizon, and **ELEVATES** a passed one: a task untouched past its own stated deadline is the strongest form of this finding, not an exempt one.*
