...
*Version 0.107.0 -- **Attribution label: the-empty-quadrant-a-closed-task-whose-evidence-never-landed-2026-08-08**. **New advisory `check_evidence_landed.py`.** canvas-health flags an OPEN task whose evidence exists (8c(b)) and evidence with NO task (8c(d)). **A CLOSED task whose findings never reached the canvas it declared had no check, and `canvas_refs` -- the field where a task states where its findings belong -- was read by NOTHING**: grep across scripts/ returned zero matches. Documentation of an intention with no mechanism behind it. DOGFOOD 2026-08-08, and the timing is the argument: that morning's /bvssh-check rated Measurement AMBER for 'measurements generated and never landing where they are read'. THREE HOURS LATER ht-055 was scored, closed, and written only into human-tasks.yml while its own canvas_refs pointed at purpose.yml. **It surfaced because a human asked 'all logged and verified?', which is not a mechanism.** **THE FIRST RUN FOUND SEVEN MORE, all predating that day** -- which is the case for building the check rather than repairing the instance: fixing ht-055 alone would have left seven stranded. WHY THIS DIRECTION IS THE EXPENSIVE ONE: an open task with stranded evidence still sits in the pending list and something trips over it. A CLOSED task is gone from every open-work surface, so its findings are UNREACHABLE, not merely unrouted -- the next session reads the canvas, sees nothing, and concludes the question was never asked. `abandoned`/`cancelled` are exempt (the point of abandoning is that nothing came of it) and an explicit `no_evidence_produced:` is exempt, because 'we asked and learned nothing' is a finding that belongs in the task. 10 tests, regression-proven against the real pre-fix commit. Wired to session-start.*
