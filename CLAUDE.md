...
*Version 0.49.1 -- **Attribution label: opencode-clone-onboarding-fix-2026-06-15** (lived-friction: dry-run prep caught that `provision-skills.sh` defaulted `CLAUDE_PLUGIN_ROOT` to the Claude-Code plugin-cache path, so a pure-opencode git-clone user would hit "cannot find plugin"). The script now self-locates the plugin root from its own path (clone case, no env var needed); `docs/integrations/opencode.md` onboarding uses the self-locating invocation and adds Apple-Silicon model-size guidance (16GB → `qwen2.5-coder:14b`, 32GB+ → `:32b`). Doc + script fix. **PATCH**. **Priors**: v0.49.0 **opencode-starter-scaffold**, v0.48.2 **dora-firstpass-battery-locations** (see `docs/changelog.md`).*
