...
*Version 0.39.17 -- **Attribution label: agents-md-plugin-cache-path-fix-2026-06-05**. AGENTS.md Minimal Path Step 5 referenced `plugins/mycelium/engine/canvas-guidance.yml` (in-repo plugin) and `.claude/engine/canvas-guidance.yml` (legacy) — neither resolved in post-plugin-migration projects where the engine lives in `~/.claude/plugins/cache/haabe-mycelium/mycelium/<version>/engine/`. Surfaced by the 2026-06-05 /mycelium:eval-runner re-run of agents-md-router-discipline.yml (blind subagent found the convention anyway but flagged the drift as a NUDGE). AGENTS.md Step 5 now lists all three install forms with the recommended fallback search order. Single-line edit; no convention or skill-behavior change. **PATCH** (docs only). **Prior**: read-before-recommend-graduation-stage-1-2026-06-05 (v0.39.16); system-card-substantive-refresh-plus-coupling-sharpening-2026-06-05 (v0.39.15).*
