...
*Version 0.23.21 -- **Attribution label: cohort-log-triggered (ht-012)**. Three friction points from the first cautious-learner cohort log (ht-012 partial_findings 2026-05-10/12) shipped as targeted skill-prompt fixes: f4 (Q1 single-sentence constraint mis-read as rhetorical politeness) → format spec now mandates bolded mechanical constraint prefix rather than prose-prefix; f5 (expectation drift about time-budget questioning) → `/interview` intro line now explicitly states no time-budget question at brief stage; f9 (internal phase-index vocabulary leaks to user) → narration-discipline rule added to both `/interview` and `/mycelium:diamond-assess` SKILL.md ("Phase-N is internal skill structure, not user-facing; L0-L5 is framework-external and ARE narratable"). Also: f2 (per-file permission prompts during /mycelium:start with no upfront preview list) graduated to opp-010 with `evidence_type: data-supported` provenance (schema enum; underlying class is behavior-validated per the cohort-log). Implementation deferred pending wider cohort triangulation. Distinct epistemic class from opp-008/009 (framework-on-framework anecdotal): opp-010 is real user research, satisfies Torres CDH "from research, not brainstorming" rule cleanly. **Privacy correction layered into this PATCH**: initial draft of this version, plus the v0.23.21 changelog and SKILL.md discipline-note attribution lines, named the cohort participant explicitly in the public framework repo — a recurrence of the v0.23.13 named-attribution-leak class. Mechanism existed (Check 33 attribution registry) but fail-opens in CI when registry absent. User caught pre-push; regenericized across 6 files (5 working-tree + opp-009 cleanup of already-pushed leak from commit 43989ae). Generic-only attribution discipline holds in the framework repo per the cohort-attribution-boundary rule; named attribution stays in the private roadmap repo where disclosure-ack covers generic-framed friction flow only. Correction logged. PATCH per version-discipline.*
