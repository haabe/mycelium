...
*Version 0.36.3 -- **Attribution label: ruff-cleanup**. Clears the 8 ruff findings the v0.36.2 pre-push surfaced as a WARN (Check 17), with zero behavior change (161 pytest tests unchanged; `sync_derived.py --check` no drift; `check_doc_references.py` links resolve): `check_doc_references.py` — SIM103 direct-return, UP034 paren drop ×2, EXE001 executable bit; `sync_derived.py` — `_compute_drift` extraction for C901, two E501 `argparse` help-line wraps, EXE001 executable bit. **PATCH** (lint only; the bump exists solely to satisfy Check 26, since `plugins/mycelium/scripts` is a material path). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: framework-health-followthrough (v0.36.2), ux-axioms-bridge-remediation (v0.36.1).*
