...
*Version 0.23.18 -- **Attribution label: lived-friction-triggered**. Read-before-Write rule sharpened to differentiate `Edit` (cheap `Read(limit:1)` satisfies check at ~50 tokens) from `Write` (full Read required for safety). Second-order cost surfaced 2026-05-14: the v0.23.x Preflight discipline correctly prevented the surface-confusion failure (Bash `head` ≠ Read tool, anti-pattern #7 instance #5) but the agent over-applied "Read before Write" as "full Read before every Edit." With `purpose.yml` at 800+ lines (~20k tokens), this dominated Pro-tier session cost. Verified experimentally 2026-05-14: Claude Code's state-tracking is per-file, not per-byte — `Read(limit:1)` registers the file and unblocks `Edit` calls anywhere in it. Sharpened block deployed to CLAUDE.md and to all 22 canvas-writing SKILL.md Preflight sections (byte-identical via script — no per-skill drift). Estimated savings 10–50k tokens per session, ongoing, every user. Check 31 unchanged (matches on heading; rule body sharpened underneath). PATCH per version-discipline: rule clarification + token-cost optimization; no schema change, no behavior change for the discipline (still Read-before-Write, just with a documented cheap path). Pairs with the framework-on-framework exemption (v0.23.16) and external-validation citations (v0.23.17) as the third entry in the 2026-05-14 framework-health cycle.*
...
Two graduations: the surface-confusion failure (anti-pattern #7 instance #5, 2026-05-09 — agent conflated Bash `head` with the Read tool, lost ~14k tokens to a Write-fail → remedial-full-Read → re-Write loop) was solved at v0.23.x by the Preflight blocks. The second-order cost — agent *correctly* following the rule but full-Reading every time — was solved at v0.23.18 by adding the `limit:1` discipline. Per-session full-Reads on 800+ line canvas files were dominating token cost on Pro-tier sessions (estimated 10–50k tokens/session saved). This applies to every canvas-writing skill — see the Preflight blocks. Validator Check 31 enforces presence; the rule's content stays in sync via the canonical block. Detected Juniors.dev pre-run 2026-05-06; structural layer 2026-05-09; cost-tier sharpening 2026-05-14.
