...
*Version 0.150.0 -- **Attribution label: the-pre-push-hook-was-doing-cis-job-2026-08-31** (MINOR). With `--cov` the local test step took 192s and the hook 366s, and that duration caused TWO failures in one day, neither of which looked like a slow hook: **every push died with exit 141 AFTER all gates passed** (git opens the remote connection BEFORE the hook runs, so GitHub closed the idle SSH connection mid-run -- misdiagnosed as a sandbox problem six times until the founder's terminal showed `Connection to github.com closed by remote host`), and **an agent reached for `--no-verify`**, skipped the fail-open gate it did not know was there, and put a red commit on main. Coverage instrumentation and `check_coverage_floor.py` move to CI; the SUITE, canvas validation, template integrity and the 17 declared gates stay local. Same suite runs ~78s without `--cov`. NOTHING becomes unenforced -- CI runs both coverage gates on every push. The divergence is DECLARED via `!waived` in `local-gate-set.txt`, so parity reports 17 declared + 1 waived with the reason, rather than repeating the 2026-08-09 silent drift. **Consumers must re-install the hook**; its self-drift warning will say so.*
