...
*Version 0.159.0 -- **Attribution label: no-canvas-is-parse-checked-only-any-more-2026-09-01** (MINOR). The last three of seven: `privacy-assessment`, `trust-signals`, `value-stream`. **25 canvases, 25 schemas, 0 schema-less** — and that is now a RATCHET, not a changelog claim: a template added without a schema fails a test, which is the only moment anyone would notice. Every enum again came from the templates' own YAML COMMENTS (`pass|partial|fail|not-assessed`, `at_rest|in_transit|both|none`, `explicit|implicit|legitimate_interest`, `improved|declined|stable`). **Cavoukian's seven principles are pinned CLOSED** — an eighth, or the plausible rename `privacy_by_default`, is a theory-fidelity break rather than a customisation, and nothing else could catch it. **One constraint is flagged as the schema author's own, not doctrine**: a `dpia_required: false` must carry a rationale. Basis is GDPR Art. 5(2) accountability; `engine/theory-gates.md` requires a DPIA only for HIGH-RISK processing and says nothing about justifying a false, so the rule is labelled in the schema, in its test and here, and can be struck deliberately instead of hardening into folklore. `value-stream` is dormant-by-design and its schema guards a first population that may never come — which is the bet the whole series makes.*
