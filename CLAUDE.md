...
*Version 0.144.1 -- **Attribution label: the-release-could-not-create-its-own-tag-2026-08-31** (PATCH, lived-friction-triggered). **`gh release create --target <sha>` ASKS THE API TO DO TWO THINGS -- CREATE A TAG REF AND CREATE A RELEASE -- AND ONLY THE SECOND WORKED.** Every release whose tag did not already exist failed; a release created against an EXISTING tag succeeded on the first try. **THE ERRORS NAMED NONE OF THAT**: `gh` reported 403 *"workflow scope may be required"* (its own hedged guess), a direct API call returned **404** (GitHub's stand-in for a denied write), a 422 came first because the body was invalid, and a repo `PATCH` with the same token succeeded -- four signals, none pointing at the missing capability. Two wrong fixes were pursued on those signals before the mechanism was isolated by splitting the operations. **The tag is now pushed over git**, which carries the checkout credential rather than the REST path and is the operation that demonstrably works, and the release is created against it. **The split also makes the next failure legible**: a tag with no release is a visibly half-finished release, where a silent 404 was not. A tag push retriggers nothing -- both workflows filter on `push: branches`, and a tag is not a branch.*
