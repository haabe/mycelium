...
*Version 0.108.0 -- **Attribution label: the-checks-were-shaped-like-the-repo-that-wrote-them-2026-08-08**. A census ran every shipped check against a real plugin CONSUMER. Nothing had degraded; two guards were BORN upstream-shaped and had never matched an installation -- `check_doc_references` and `check_legacy_paths` scanned **29 of 212 markdown files**, leaving 158 links unexamined, **three of them dead for 88 days** and pointing at directories the consumer's plugin migration removed, which is precisely what `check_legacy_paths` exists to catch. Both now scan the consumer tree, scoped to files that ROUTE (a first cut over the whole tree gave a 13:1 noise ratio against the append-only record). `check_theory_fidelity` auto-detect used `parents[3]` and resolved to the marketplace cache in plugin form -- now finds its root by ARTIFACT and says N/A honestly. **NEW `tests/python/test_consumer_shape_coverage.py`**: plants each check's own defect in a consumer-shaped tree and requires it to be FOUND -- the reduced-input case `check_empty_input_honesty` names as its own limit. Verified to bite against pre-fix code. A TEST, not a 25th check, because the parked refactor names check-count>=27 as an un-park trigger. Also `scripts/check_source_class_fidelity.py` + 12 tests: five founder-sourced canvas entries were classed `external_human`, three of them beside prose that already disagreed.*
