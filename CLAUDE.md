...
*Version 0.108.2 -- **Attribution label: a-guard-that-fires-on-writing-about-it-gets-muted-2026-08-08**. Closes an inconsistency introduced by v0.108.0, which widened both doc guards to `.claude/` but gave only `check_legacy_paths` the append-only-record exclusion. Within hours a decision-log entry describing that fix quoted a relative link shape and `check_doc_references` correctly read it as a dead link -- the **third time in one session** that documenting a defect created an instance of it. A decision log, a corrections file and an eval report legitimately QUOTE link shapes while narrating the work; their job is to remember, a README's job is to route. **READMEs stay in scope even inside those trees** -- `.claude/evals/README.md` routing a reader to `results/` is navigation and must keep being checked -- so the distinction is the file's JOB, not its parent directory. 3 tests, verified to bite: without the exclusion both quoting cases fail, and a README under `memory/` still gets flagged.*
