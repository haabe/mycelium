...
*Version 0.38.4 -- **Attribution label: architecture-discovery-narrowed-receipt**. New self-dogfood receipts case `docs/receipts/cases/2026-06-01-architecture-discovery-narrowed.md`: an external practitioner asked whether the evidence-gate could extend from product to technical discovery; instead of building for the adjacency, the hypothesized gap was captured as a failing-first scenario and dogfooded via Claude Code subagent. Disk-verified result: existing gates (Evidence + Bias + AP#7 + Cagan feasibility) do the blocking work; the gap is narrower than the original framing — vocabulary + routing nudge, not a new scale. Generic-framed per attribution discipline; nothing shipped to skills/hooks. **PATCH** (one new receipts case; no behavior change). **Prior**: mental-model-docs-page (v0.38.3); framework-health-remediation (v0.38.2); guardrail-count-sync (v0.38.1); mid-build-idea-intake (v0.38.0).*
