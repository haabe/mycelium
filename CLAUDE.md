...
*Version 0.49.20 -- **Attribution label: release-backlog-convention-2026-06-19** (maintenance-housekeeping). Adds the **GitHub-release cutting checklist** to `engine/version-discipline.md`: every version bump now also cuts an annotated tag + `gh release` whose notes are the matching `docs/changelog.md` section, building a gap-free, browsable version **backlog** that mirrors `plugin.json#version`. Convention-tier — no new gate (cutting a release touches no tracked file, so Check 26 cannot see it); rides the existing coordinated-commit trigger. Doc-only. **PATCH**. **Priors**: v0.49.19 **theory-fidelity-anti-drift-guard-2026-06-18**, v0.49.18 **theory-fidelity-skill-2026-06-18** (see `docs/changelog.md`).*
