...
*Version 0.126.0 -- **Attribution label: counting-the-population-hid-the-seven-2026-08-24**. **`check_leaf_lifecycle.py` gains a second half: FOUR RISKS PAST A DECISION POINT.** The 2026-08-23 rule census counted **19 of 54** solution leaves carrying a `four_risks` block and read it as a sprawling backlog. **Recounted by STATUS it is three different things**: 24 still `candidate` (pre-decision, where the rule says SHOULD and a filled block on a leaf that may never be pursued is the filler trap), 4 closed (nothing was invested, nothing to assess), and **7 that passed a decision — shipped or validated — with no risk evaluation at all.** Those 7 are the finding, and **counting the population is what hid them.** **EXTENDS RATHER THAN DUPLICATES**: that script already audits leaves at a decision point for a missing selection artifact (ICE on shipped), with an `ice_exempt:` escape hatch; the new half mirrors it with `four_risks_exempt:`. **The two halves use DIFFERENT populations on purpose** — ICE keys on SHIPPED because it is the Check 38 precondition, four-risks on shipped **or VALIDATED** because the rule is about passing a decision; widening the ICE half under cover of adding a new one was considered and refused. **AN EMPTY `four_risks:` COUNTS AS UNASSESSED**, because a key with nothing under it is the exact shape the census exists to catch. **AND IT DOES NOT ASK FOR A BACKFILL** (founder ruling): the rule is *risk evaluation FIRST*, so a block written today cannot restore that sequence — it only makes a past decision look compliant. **The flag is the honest state**, advisory via `session-start`, which now reports the two halves separately so a four-risks-only finding cannot render as "0 leaves carry no ICE".*
