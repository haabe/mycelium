...
*Version 0.170.0 -- **Attribution label: assessment-skills-never-wrote-their-canvas-2026-09-03** (MINOR). A consumer measured three assessment skills whose findings never reached the canvas file built to hold them. `service-check`, `privacy-check` and `threat-model` named no canvas destination at all — `threat-model` named no output surface whatsoever — while `canvas-update`'s routing table already mapped all three. **The cost, measured:** a full 15-principle Downe assessment ran on that project 2026-05-23 and `services.yml` held every principle at `not-assessed` for **103 days**; `privacy-assessment.yml` carried `last_assessed: 2026-05-04` over seven unassessed principles — a date asserting an assessment that never landed; `threat-model.yml` held zero threats. **All three were kept looking fresh by `xai-check`**, one of the skills that does write canvas, which stamped their `_meta` while filling none of their content. The Service Quality, Privacy and Security theory gates all read those files and are all Required at L4. Each skill now carries the `bvssh-check` convention verbatim — **Canvas (MANDATORY — the source of truth, do this FIRST)**, the decision log demoted to provenance — plus a Verify-After-Write postflight. `check_assessment_landed.py` ships with them and fires on the discriminator that makes this a defect rather than a backlog: **an empty file that declares itself empty in `_meta.applicability` is a decision; an empty file carrying a freshness stamp is a defect.** Report-only unless `--strict`, UNKNOWN (exit 2) on a missing, empty or unparseable canvas, and seven tests prove it bites on the real shapes and stays silent on the three legitimately-empty canvases.*
