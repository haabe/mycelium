...
*Version 0.81.1 -- **Attribution label: pipestatus-portability-2026-08-03**. The shell-safety guard shipped in v0.81.0 gave advice that does not work in the shell this project runs. It recommended `${PIPESTATUS[0]}` as the remedy for `$?` after a pipeline; that is bash, and this project's shell is zsh, where the array is lowercase `$pipestatus` AND 1-indexed, so `${PIPESTATUS[0]}` expands to the empty string. **The worse of the two bugs was the suppression**: the regex testing for the remedy was case-sensitive, so anyone who had ALREADY applied the correct zsh form was still warned — a false positive aimed squarely at the people doing it right, which is how a guard gets disabled. Both forms are named now, with the index difference stated, and the suppression is case-insensitive. Caught by the author's own command printing `EXIT=` while testing something unrelated. Advice that fails quietly is worse than none, because it reads as handled — the same shape as every green-over-nothing defect the v0.77.0-v0.80.1 run removed, in a guard four hours old.*
