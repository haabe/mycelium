...
*Version 0.166.0 -- **Attribution label: two-theory-rows-that-had-to-be-earned-first-2026-09-01** (MINOR). Peak-End and Chunking join Tier 2 — but ONLY because v0.165.0 gave them a mechanism first. `docs/theories.md` requires every theory to be mechanism-mapped and `check_theory_fidelity` rule E enforces it on every push, so these rows were impossible to add honestly two hours earlier and are possible now; the order was mechanism, then row, not the reverse. Cognitive Load and Postel's Law have no artifact and stay OUT. The Chunking row carries **'the mechanism, never the number 7'** in its own text, so Miller's revised limit travels with the citation instead of being re-litigated. **Also corrected a row that had gone false**: Team Topologies still claimed `team-shape.yml` 'has no schema and nothing else consumes it' — untrue since v0.158.0 this morning. Now cites the schema and the 2nd edition, and KEEPS the honest half: nothing yet ACTS on the assessment, so it stays advisory-only. 21 → 23 rows.*
