...
*Version 0.132.0 -- **Attribution label: sent-and-waiting-is-not-in-progress-2026-08-24**. **`pending` carried two incompatible meanings -- NOT STARTED and STARTED-BUT-WAITING-ON-SOMEONE-ELSE -- and an agent grepping the field could not tell them apart.** Surfaced 2026-08-20 after the misreading cost a wrong recommendation to a founder; it sat as an upstream candidate until the evidence arrived. **MEASURED 2026-08-24, once a list split made the distribution visible: of 25 open tasks, 16 were `in_progress` and 13 of those carried a touch_log** -- sent, replied to, ball elsewhere, with horizons that are reply-SCORING dates rather than work deadlines. **The overload had MOVED rather than resolved**: `pending` used to mean not-started-or-waiting, and `in_progress` had come to mean being-worked-or-waiting, with the second reading the majority at 13 of 16. **TWO values ship, not one, because they differ in what a reader should DO**: a `waiting` task CAN go stale -- a reply that never came is a finding -- while a `watching` task cannot, since nothing is owed. Three dogfood tasks were self-labelled *'WATCH ONLY. NOTHING IS OWED'* in prose, and the project had already hand-exempted its watches from a standing staleness rule, which is the evidence the states differ. **BOTH READERS SHIP IN THE SAME COMMIT, which is now the standing rule here**: canvas-health 8c(a) skips `watching` outright, and 8c(b) takes an explicit status as the recorded reason a task stays open **instead of inferring 'legitimately waiting' from a future `horizon` date** -- a proxy the author can now simply declare. The v0.130.0 list-agreement check treats both as open.*
