...
*Version 0.221.0 -- **The runtimes moved** (MINOR). Six changes from a sweep of what Claude Code, Codex and opencode shipped between mid-August and mid-September 2026. opencode 2.x does not load 1.x plugins, so a second enforcement plugin ships beside the first (`integrations/opencode/plugins-v2/`), and on 2.x the reflexion prompt on a failed tool call works, which 1.x never allowed. The Codex integration doc now says that a configured hook is not a running hook: Codex skips every hook until the user trusts it, again after each upgrade. `stop-check.sh` serialises its last hand-built JSON object, and a new test runs every command in `hooks.json` and checks that stdout is empty, plain text or JSON that parses, because Claude Code 2.1.248 turned near-JSON into a hook error. `/security-review` and `/threat-model` gain the OWASP Agentic Skills Top 10, applied only when the product ships skills, plugins or MCP servers. A three-case `claude plugin eval` suite (`plugins/mycelium/evals/`) lets the runtime score the plugin against a no-plugin baseline.*
