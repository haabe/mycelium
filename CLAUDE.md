...
*Version 0.39.8 -- **Attribution label: docs-readme-persona-index-2026-06-04**. `docs/README.md` gains a "Where to start" persona-index above the Contents list: three task-anchored entry paths (install and try / understand before adopting / evaluate fit) routing to the relevant existing pages. Routing fork placed at docs landing rather than main README hero per Core Model methodology (the main README stays single-funnel; routing forks belong where multiple deep paths have room without duplication pressure). **PATCH** (docs only, no skill behavior changes). **Prior**: readme-restructure-articulation-driven-2026-06-04 (v0.39.7); sol-007a-technical-discovery-vocab-2026-06-03 (v0.39.6).*
