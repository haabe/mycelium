...
*Version 0.24.0 -- **Attribution label: lived-friction-triggered + self-audit-driven**. Saturday session that started as a /bvssh-check on l0-purpose and surfaced two clusters of discipline gaps the framework was carrying. **Three feature surfaces shipped**: (1) `Gated by:` Communication Rule added to CLAUDE.md — every deferral/threshold/date-based recommendation must name its unblock event explicitly, including agent pushback statements. Closes the implicit-causal-link sub-class of anti-pattern #7 (sub-class g, NEW) that surfaced when a BVSSH "defer to ≥05-31" recommendation conflated capacity-gating with evidence-gating. Convention accepts three equivalent forms (`Gated by:`, canvas `ON HOLD (pending X)`, natural prose) so existing well-formed gate-naming doesn't fail retroactively. (2) `tests/bash/` fixture-test convention — sourcing guard in `validate-template.sh` so check_* functions can be invoked in isolation, plus `_assert.sh`, `run.sh`, 8 worked-example test files covering all 7 post-G-V12 Bash checks (26, 28, 29, 30, 31, 32, 33, 34). Closes G-V12 promotion-bar gap for the Bash-check surface; wired into Check 17 alongside pytest. (3) Anti-pattern #7 entry extended with full sub-class catalog (a-g) and graduation philosophy ("make interpolation visible, don't eliminate it" — Grice/Sperber-Wilson). **Three new clusters added to framework `cluster-instances.md`**: `consistency-as-evidence` (8+ instances, ported generic-framed from roadmap), `subagent-simulation-misses-lived-friction` (2 instances), `bash-check-without-fixture-test` (7 instances, Phase 1 complete same day at ~1.5h actual vs 4.5-6.5h estimated — convention reuse was the lever). Plus `documented-rule-diverges-from-enforcement` extended to 13 instances with two new sub-shape candidates (hook-vs-canvas-distribution, canvas-state-vs-reality-drift). **MINOR per version-discipline**: new feature surfaces (testing convention + Communication Rule + script stub `check_gated_by.py`), backward-compatible. v0.23.43 entry migrated to docs/changelog.md per Check 34.*
