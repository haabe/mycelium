...
*Version 0.23.4 -- Seven primary-source citations from Laws of Software Engineering gap analysis (mycelium-roadmap dogfood 2026-05-10, same playbook as v0.23.2 CBI graduations). All 7 land on existing surfaces; no new mechanism. **Hyrum's Law** (Wright et al. *Software Engineering at Google* 2020 ch. 22) added to `engine/version-discipline.md` Theory grounding — the version-bump discipline IS Hyrum's Law applied to plugin-form install. **Postel's Law / Robustness Principle** (RFC 793, Postel 1981) added to `engine/consistency-check-spec.md` Theory grounding — retroactive citation for the 0.16.2 schema fix accepting both `source_class` (singular) and `source_classes` (plural). **Leaky Abstractions** (Spolsky 2002) added to `engine/version-discipline.md` Theory grounding alongside Hyrum — directly cites the bare-path sweeps (v0.20.7 + v0.23.3) as instances of the principle. **Tesler's Law of Conservation of Complexity** added to `jit-tooling/detector.md` Theory grounding — articulates why JiT detect-and-generate beats pre-shipped catalogs (complexity moves to the agent at moment-of-need, not eliminated). **Gall's Law** (Gall *Systemantics* 1975) added to `engine/framework-reflexion.md` Theory Citations — the mechanism-graduation discipline (≥3 instances → pattern → mechanism) IS Gall's Law as method. **Cunningham's Law** added as Technique 6 in `/devils-advocate` — justifies dogfood-first / publish-rough-then-iterate over wait-for-certainty. **Brooks's Law** (Brooks *Mythical Man-Month* 1975) added to `domains/delivery/CLAUDE.md` Theory of Constraints section with AI-assisted variant — adding another agent to a stuck task usually doesn't help; fix the constraint instead. PATCH per version-discipline (citation additions on existing surfaces — no new mechanism, no behavior change beyond the named-and-cited principles). Three citations (Hyrum, Postel, Leaky Abstractions) connect directly to lived-friction from the same week (install-model migration, schema discipline fix, bare-path sweep). User-override of draft promotion bar logged in `mycelium-roadmap/.claude/drafts/lawsose-gap-followups.md` — Tier 1 graduated now; 15 Tier 2 candidates tracked for trigger-driven graduation.*
