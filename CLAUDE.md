...
*Version 0.31.3 -- **Attribution label: human-task-reconciliation**. Closes the canvas-drift class surfaced this session (corrections.md 2026-05-28): human-task `status`, evidence-capture, and the consent registry are decoupled — a fact lives in 2+ homes and only the salient one gets updated, so the canvas silently drifts from reality (tasks left `in_progress` after their evidence was already logged; cohort consent in auto-memory but not the canonical `attribution-registry.yml`; cold outreach never closed because abandonment is a non-event with no trigger). Two fixes ship the **detection layer**: (1) session-start hook `CHECK 5` now counts OPEN human-tasks by status (excludes `completed`/`abandoned`/`stalled`) and flags items with no activity in 14+ days, replacing the raw `len(pending_tasks)` count that surfaced noise not signal (dogfood: 16 entries → 4 open, all stale); (2) new `/canvas-health` sub-check **8c** (human-task reconciliation): status-vs-activity staleness (21d), evidence-exists-but-task-still-open, and consent-registry-vs-auto-memory mismatch. **PATCH per version-discipline**: two framework files touched (hook + canvas-health SKILL), no new files, additive + bug-fix (same tier-shape as v0.31.2). **Attribution class**: lived-friction-triggered — the drift was user-caught this session ("why weren't these updated when the agent got the data?"). **Honesty caveat / deferred**: the `/log-evidence` close-the-loop (auto-close the source task + sync registry on evidence write — the third prevention item in the corrections entry) is NOT in this bump; this ships detection, not auto-close. v0.31.2 entry in docs/changelog.md per Check 34.* **Prior**: bluf-footnote-output-convention (v0.31.2).
