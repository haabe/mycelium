...
*Version 0.39.16 -- **Attribution label: read-before-recommend-graduation-stage-1-2026-06-05**. Anti-pattern #7 (Consistency-as-Evidence) graduation Stage 1: the conversational + gate-narration sub-shapes graduate to mechanism after 5 same-day instances 2026-06-02 (cluster-instances.md #13-#17) and 30+ days as a named-but-ungraduated candidate. New CLAUDE.md Communication Rule "Always read canvas state before recommending or narrating gate-status on a topic with a known canvas entry" requires canvas-file + field-path citation for recommendations, gate-narration, blocker/hold claims on canvas-tracked topics; adjacent-surface inference must be tagged explicitly. New "## Preflight: Read-before-Recommend" block added to diamond-assess + diamond-progress (the two skills founder named explicitly per corrections.md L52 — the recursive instance #17 fired during diamond-assess running an un-mechanized version of its own diagnosis). New Check 41 in tests/validate-template.sh + fixture test enforces preamble presence on the surface skills (parallel to Check 31's Read-before-Write on canvas-writing skills). Sub-shapes deferred to next graduation cycle: cross-repo state checks, consent-state-change reconciliation, cross-file completeness verification. Plus: /canvas-health 9b additions — changelog exemption from page-length cap (log files where line count IS the value); stable-cohort INFO signal when ≥3 docs share the same Last-updated date approaching 180d. Per /mycelium:framework-health 2026-06-05 dashboard findings #1, #4, #5. **PATCH** (Communication Rule + skill preambles + validator check + canvas-health 9b additions; no engine/hook behavior change). **Prior**: system-card-substantive-refresh-plus-coupling-sharpening-2026-06-05 (v0.39.15); check-40-sync-derived-pre-push-gate-2026-06-05 (v0.39.14).*
