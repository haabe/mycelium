...
*Version 0.160.0 -- **Attribution label: two-thirds-of-the-warnings-were-the-check-being-wrong-2026-09-01** (MINOR). Nine `evidence target` warnings stood open for a day. **Six of them were the check, not the canvas.** THREE WERE FALSE POSITIVES: `has_pointer` looked in exactly two places — top-level keys and `touch_log[]` — while the convention in live use has three, and ht-037/038/059 record the pointer in `partial_findings[]`. They were reported as gaps their own data did not have. The FPs named the missing convention: the rule is 'a pointer in the task record', so it now walks the structure. THREE MORE were tasks closed with the full `closure_discipline` trio (reason + basis + reopen_trigger) — which IS the project's mechanism for recording that nothing came of something; demanding a pointer as well asks them to point at evidence they are recording the absence of. All three companions are required, so a bare `closure_reason` still warns and silence cannot pass as an answer. **Only three were real**, and each got a traced pointer: ht-073→comp-014, ht-033→opp-014, ht-074→its own forage field, with the option to promote that to comp-039 left to the founder rather than taken. The roadmap canvas now runs 0 evidence-target warnings.*
