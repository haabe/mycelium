...
*Version 0.56.1 -- **Attribution label: canvas-health-enum-2026-07-03** (roadmap dogfood /canvas-health finding — one doc patch). **/canvas-health step-5 evidence_type rubric aligned to the shipped schema**: the skill listed a seven-value "valid set" (`interview`/`survey`/…/`mocked_persona`) that had drifted from `schemas/canvas/_common.schema.json#$defs/evidence_type` (`speculation`/`anecdotal`/`data-supported`/`test-validated`/`launch-validated`, Gilad's ladder) — one audit run mislabeled ~120 legitimate canvas fields as unknown. Step 5 now cites the schema enum directly, carves out `_meta` structural markers (flag only outside `_meta`), and rewords the mocked-persona honesty check in ladder terms (simulated evidence = `speculation` regardless of vividness). Documented-rule-diverges-from-enforcement class. **Priors**: v0.56.0 **discovery-gate-2026-07-02** (auto-dogfood utterance-mode findings — discovery gate hook + /start AGENTS.md deferral + ost-render fail-loud), v0.55.3 **scenarios-schema-2026-07-01** (theory-fidelity B6/B7 program close), v0.55.2 **cynefin-rigor-correction-2026-07-01** (see `docs/changelog.md`).*
