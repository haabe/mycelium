...
*Version 0.163.0 -- **Attribution label: undated-is-not-stale-it-is-uncheckable-2026-09-01** (MINOR). Eleven landscape entries written over eleven days shipped with no `captured_at`. They were not STALE — they were UNCHECKABLE, which is worse: a stale entry eventually trips a decay threshold and asks to be revisited, an undated one never does. `canvas-health` step 7 compares the dates it FINDS and silently skips absent ones, so the entries were invisible to the exact mechanism meant to catch them. New WARN reports ONE coverage ratio, not one line per entry: measured 98 of 346 blocks undated on a real canvas, 93 in a single file, and a per-entry warning would fire ninety-odd times and teach its reader to skip the class — the failure canvas-health already recorded when a rule fired on 80% of a corpus. Deliberately NOT a schema requirement, because `provenance` is shared by every canvas through `_common`.*
