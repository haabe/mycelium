...
*Version 0.168.0 -- **Attribution label: a-match-token-must-be-as-narrow-as-its-ruling-2026-09-01** (MINOR). Extending a do-not-cite register from 6 entries to 26 produced a **6-of-6 FALSE POSITIVE run**, and both causes were real. (1) BARE NAMES: `Ulwick`, `Jansen`, `Standish` matched LEGITIMATE citations — Ulwick is a correctly-cited theorist and the Standish ruling is about its success RATES while the canvas uses its cause-RANKING. **A token broader than its ruling recreates, inside the register, the narrow-true-to-over-broad error the register exists to prevent** — the third instance of that error in one day. (2) The annotation vocabulary missed `DISPUTED`, `contested`, `unverified`: a line that already discloses its own weakness was flagged anyway, which teaches authors that disclosure is punished. Both fixed, the narrowness rule is stated in the register itself where the next person adding an entry will read it, and detection is proven still live — two uncaveated violations caught in the same run that suppressed a caveated one.*
