...
*Version 0.23.17 -- **Attribution label: external-validation-triggered**. Two converging signals dated 2026-05-13: Torres ships AI-generated OSTs via Vistaly (producttalk.org), generating from interview transcripts — reinforcing this framework's "OSTs from research, never brainstorming" rule. And "harness engineering" is named as the fourth paradigm of AI engineering (TechTimes piece citing Hashimoto's Feb 2026 principle and Lopopolo's Feb 11 2026 definition), formalizing the vocabulary Mycelium has used since v0.1. Two citation edits to shipped framework files: `engine/cycle-learning.md` adds the Hashimoto citation as canonical source for the corrections-to-mechanism graduation discipline (the framework-on-framework exemption added in v0.23.16 sits in this section); `skills/ost-builder/SKILL.md` adds the Torres-Vistaly citation under existing Theory Citations, surfaces the OST-update-with-change-set gap as L3 backlog, and reinforces the never-from-brainstorming rule with canonical-source convergence. PATCH per version-discipline: citation additions to shipped docs, no schema/behavior change. Canvas evidence entries (landscape + purpose) for these signals land in the roadmap-repo canvas, not here — per the framework-on-framework exemption shipped at v0.23.16.*
