...
*Version 0.39.3 -- **Attribution label: cross-repo-stale-state-arm-2026-06-02**. SessionStart CHECK 8 added: observability nudge surfacing recent commits in sibling repos (`MYCELIUM_CROSS_REPO_WATCH` env var, colon-separated paths) whose messages reference canvas IDs (opp/sol/comp/ht/cyc/sce-XXX). Targets the cross-repo arm of anti-pattern #8 (Stale State Read) — sibling instance of AP#7 #13 logged earlier same day, but a different surface (cross-repo state instead of same-repo canvas state). Fail-open, NUDGE tier; opt-in via env var. Worked example same day (2026-06-02): roadmap dogfood session touched opp-005 to log a Bård evidence source while unaware that upstream commit a1cef04 had already acted on the marketing-surface arm of the same friction; CHECK 8 would have surfaced the upstream activity. **PATCH** (single observability check, no behavior gate, opt-in). PreToolUse cross-repo check NOT shipped this version — overlaps with Read-before-Recommend graduation candidate pending at next `/corrections-audit` (folded into that work to avoid two codepaths for sibling instances of the same root pattern). **Prior**: rule-6-fp-measurement-2026-06-02 (v0.39.2); readme-language-revision-2026-06-02 (v0.39.1).*
