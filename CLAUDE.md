...
*Version 0.49.2 -- **Attribution label: runtime-verification-before-ship-2026-06-15** (lived-friction: the opencode scaffold was authored from dev-branch source analysis, tagged "source-verified", and would have shipped broken — 3 ship-blocking bugs surfaced only when a user-prompted runtime test ran it; the safety came from a prompt, not a gate). Tightens the framework so runnable artifacts can't ship on source-analysis alone: new **G-V13** (a runnable-artifact works/ships claim needs a runtime existence-proof `Ran: <cmd> → <result>`, or an explicit "source-verified, untested" downgrade), the verification-surface rule gains a runnable-artifact tier (`communication-rules.md`), and anti-pattern #7 gains sub-class **(i) source-analysis-narrated-as-runtime-verification**. Sharpening of existing mechanisms (verification-surface rule + existence-proof correction + DoD), NUDGE-weight, no new validator Check (detection-mechanization deferred). **PATCH**. **Priors**: v0.49.1 **opencode-clone-onboarding-fix**, v0.49.0 **opencode-starter-scaffold** (see `docs/changelog.md`).*
