...
*Version 0.23.42 -- **Attribution label: scheduled-discipline**. `/mycelium:framework-health` + `/mycelium:corrections-audit` cycle 2026-05-23 produced four mechanical actions: (1) `engine/consistency-check-spec.md` updated — added 10th cluster instance (`test-driver-vs-source-of-truth` subclass from today's deep dive) + Rule 6 candidate (drift-checker reference) moving the cluster closer to its ≥3-detection-rules promotion bar. (2) `.claude/memory/corrections.md` — 25 entries backfilled with `Detection_origin` field per audit Step 4b discipline (distribution: evaluator 9, agent_self 7, external_review 4, user 3, hook 1, eval_runner 1 — harness coverage healthy, not user-detection-gap). (3) `.claude/memory/cluster-instances.md` — instance count 9 → 10 with full instance-log entry for the auto-dogfood prompts-bypass-SKILL.md finding. (4) README.md "How Mycelium got smarter" highlights — rotated 2026-05-20 Edith-Mari first-non-developer-user case onto the list (replacing macos-can-i-open). Also: `agents-md-router-discipline.yml` regression scenario re-run (Step 2b deferred design-verification) PASSED — improved over 2026-05-03 baseline (the decision-authority-gap complaint is closed by the action_flags convention shipped between baseline and re-run). Subagent surfaced one new meta-finding: timeout_handling case where named-date passed without evidence-resolution warrants `/canvas-health` route. PATCH per version-discipline: atomic-commit rule applied; no schema, skill, or behaviour change.*
