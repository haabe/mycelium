...
*Version 0.31.11 -- **Attribution label: audit-high-group**. Second audit fix batch (High). (H1) `gate.sh`/`preflight.sh` preflight stamp moves from the world-predictable `/tmp/mycelium-preflight-stamp` to a per-user + per-project path under `$TMPDIR` written `umask 077` (closes a symlink-clobber / cross-user-collision surface). (H2) `scope_check.py` replaces `fnmatch` (where `*` eats `/`) with segment-aware globbing — `*` stays within a path segment, `**` spans depth — so an in_scope pattern can no longer silently leak into subdirectories. (H3) new **Check 37** mechanically enforces G-V12 (every declared check ships a `tests/bash/test_check_<N>.sh`); backfills the previously-missing `test_check_16.sh` + `test_check_17.sh` and adds `test_check_37.sh` (self-applying). Regression tests added across all three. **PATCH** (new CI Check + tests, robustness/security fixes; no behavioral-contract or canvas/schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: hook-dual-path-hardening (v0.31.10), claudemd-dispatcher-refactor (v0.31.9).*
