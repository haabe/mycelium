...
*Version 0.49.0 -- **Attribution label: opencode-starter-scaffold-2026-06-15** (dogfood-triggered: a cross-repo opencode discovery arc re-verified the runtime gaps and produced a runnable starter). Ships opencode as a provisionable agent target: a starter scaffold under `plugins/mycelium/integrations/opencode/` (config + enforcement-plugin skeleton with the two clean hooks + example command + `provision-skills.sh`), and `/mycelium:setup` Step 5 now optionally provisions it — guarded, opt-in, skips silently on Claude-Code-only installs. Provisioning also vendors the skills + their engine/harness reference files into the project's `.claude/` and rewrites `${CLAUDE_PLUGIN_ROOT}` paths to project-relative ones (opencode does no variable substitution; 36/55 skills need it; env-var alternative tested + ruled out). Vendored copies are a snapshot — re-run setup after an upgrade. `docs/integrations/opencode.md` refreshed: of the three previously-hard runtime gaps only reflexion-on-tool-failure (#27900) remains genuinely hard; headless context injection (#27899) is effectively closed via `chat.message`, read-before-edit (#27901) is plugin-solvable. The scaffold is RUNTIME-VERIFIED on opencode 1.17.7 (plugin loads, chat.message injection + read-before-edit guard fire live on llama3.1:8b, clean EXIT 0) — the verification caught and fixed 3 ship-blocking defects (invalid JSON comment keys, missing provider models map, a crashing chat.message part shape). Maintained-core support (TS build + test toolchain + publish) stays paced behind real adoption. New capability, no breaking change. **MINOR**. **Priors**: v0.48.2 **dora-firstpass-battery-locations**, v0.48.1 **dora-firstpass-live-instrument** (see `docs/changelog.md`).*
