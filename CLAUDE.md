...
*Version 0.80.0 -- **Attribution label: evidence-breadth-2026-08-03**. Two named guardrails required evidence BREADTH, both declared `NUDGE`, and nothing enforced either for the whole life of the framework. G-D2 ("triangulate: at least 2 independent evidence types; single-source evidence is anecdotal, 0.3 on Gilad's meter") and G-D4 ("each opportunity in the OST must have evidence from at least 2 sources") existed in three markdown files and zero scripts. **The vocabulary collision is why they were unenforceable.** G-D2 asks for "2 independent evidence types" and means METHOD — its own examples are interviews plus behavioral data — but `evidence_type` in the schema is Gilad's confidence LADDER. The rule named a field carrying a different concept, and the field that does carry method, `source_class`, is referenced by neither guardrail. `scripts/check_source_independence.py` binds the rule to the field that can answer it. **Prompted by a peer practitioner** arguing that evidence strength is a coverage problem rather than a weighting one: four sources blind to the same thing are weaker than three blind to different things, and two interviews are two sources but one method. **What it measures is stated narrowly**, because "independent" promises more than distinct `source_class` delivers: it cannot see two snapshots from one vendor or five interviews recruited from one channel. **Two false starts are recorded in the code rather than hidden.** Trusting the unclassified-defaults-to-`internal_desk` rule reported 40 monoculture findings on the dogfood canvas, every one an artefact; scoping it beyond discovery canvases produced 8 more, all competitor entries neither guardrail is about. Measured in scope: 47 provenance objects, 0 violations, and `source_classes` present on 4. The check ships green and prints that denominator every run, because a verdict over 4 of 47 wearing the clothes of the whole canvas is the failure v0.77.0-v0.79.0 were spent removing.*
