...
*Version 0.110.2 -- **Attribution label: the-id-scan-reported-the-wrong-maximum-2026-08-13**. The ID-scan preflight, carried by 22 SKILL.md files, told the agent to run `grep "^  - id: <prefix>-" ... | sort -u` before assigning a new canvas ID. **That sort is LEXICAL, and on a canvas with inconsistent zero-padding it reports the wrong maximum.** Verified live on the dogfood repo: for human-tasks it returns `ht-1` as the highest ID when the actual highest is `ht-080`, so an agent following the instruction exactly would assign `ht-2` and collide with `ht-002`. `ht` is the prefix `/handoff` and `/log-evidence` write most. Replaced in all 22 of them with a numeric sort on the ID's number field, and switched to `grep -o` so IDs referenced in prose or cross-references are counted too and a promised-but-undefined ID is not handed out twice. The rule was never the problem; the command implementing it was.*
