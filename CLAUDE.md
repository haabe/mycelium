...
*Version 0.124.0 -- **Attribution label: the-promise-closed-by-the-doc-ceasing-to-promise-2026-08-23**. **`engine/mutation-log.md` is RETIRED, not built.** It fully specified `canvas/mutation_log.jsonl` and claimed replay, audit, regression support and drift detection; **nothing wrote it and nothing read it, in any project, ever.** The analysis inverted the question rather than answering it: **three of the four claims were already served** -- replay and regression by git, audit by the decision log in prose -- and the fourth, drift detection, is better reached from the dependency direction than from a log. **AND THE EXPERIMENT HAD ALREADY BEEN RUN**: the dogfood project's `change-log.jsonl` held **5,802 entries over four months with exactly ONE reader**, a skill that must be remembered to be invoked -- the framework's own named contradiction of `mechanically-enforced`. **A richer-fielded version of an unread log is still an unread log.** Asked what he actually wanted, the builder described something else entirely: *"if there are fields that depend on other fields, I expect them to auto-update and maybe with a notification."* **That is a dependency graph with propagation, and the difference decides the question** -- a mutation log would have faithfully recorded a parent diamond's re-specification and changed nothing about its child carrying a stale link for eight days. **P5 closes by the doc ceasing to promise**, which this registry has always permitted; the successor idea is recorded as a design question with an owner and deliberately NOT as a new registry row, since inventing a promise while closing one is the defect the registry exists to catch.*
