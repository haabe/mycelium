...
*Version 0.42.0 -- **Attribution label: autonomous-evidence-guard-2026-06-11** (lived-friction-triggered). Ships the enforcement guardrail the Stage A refutation (v0.41.7) demanded: **`autonomous-evidence-guard`**, a PreToolUse hook (`hooks/autonomous-evidence-guard.sh` + `scripts/autonomous_evidence_guard.py`) that — only in a DECLARED autonomous run — hard-blocks any Write/Edit/MultiEdit or MCP-filesystem write introducing `source_class: external_human|external_data`, `validated: true`, or `evidence_type` above `speculation` into `.claude/canvas/*.yml`, `.claude/diamonds/*.yml`, or their `mycelium-state/` mirror. Turns the evidence-integrity boundary from prose ("hope the model holds it" — which Haiku didn't) into mechanism that would have blocked the Haiku fabrication at write-time. Strict no-op in every interactive session (a present human is never autonomous → zero added friction). New hook-visible declaration surface `MYCELIUM_AUTONOMOUS_RUN=1` (survives the headless `.claude/` write-block). Defense-in-depth: the tier restriction still stands — known gaps are Bash-heredoc writes (V8 evasion vector), in-conversation prose fabrication, and non-canonical paths. 8-test coverage proof (`tests/bash/test_autonomous_evidence_guard.sh`). opp-011 confidence 0.60 → 0.65. **MINOR** (new enforcement hook + declaration surface). **Prior** (v0.41.7): **autonomous-mode-cross-model-refuted-2026-06-11** (lived-friction-triggered).*
