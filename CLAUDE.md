...
*Version 0.57.2 -- **Attribution label: opencode-upstream-closure-2026-07-19** (maintenance-housekeeping). **Upstream-tracking patch**: recorded the procedural closure of two `anomalyco/opencode` contributions — issue #27899 (headless prompt-mutation hook) auto-closed `NOT_PLANNED` by the 60-day stale bot (harmless — the capability is already covered structurally via the `chat.message` path), and PR #32542 (implements `tool.execute.error`, the fix for the #27900 reflexion gap) auto-closed unmerged by "Automated PR Cleanup" (age + <2 reactions, not a review rejection; reopenable). Issues #27900 (the one genuine hard gap) and #27901 (read-before-edit) remain OPEN. Fixed the now-stale "a PR draft exists" claim in `docs/integrations/opencode.md`; no adapter-posture or gate change. **Priors**: v0.57.1 **docs-drift-skillcount-2026-07-18** (documentation drift sweep — skill counts → **58**, opencode reference re-measured to 33 of 58), v0.57.0 **harms-check-gate-2026-07-11** (new `/harms-check` design-time safety gate — dark/deceptive patterns + foreseeable misuse, the BVSSH-**Happier** front-end), v0.56.3 **canvas-health-reopened-at-2026-07-05** (`reopened_at` counts as task activity) (see `docs/changelog.md`).*
