...
*Version 0.110.6 -- **Attribution label: two-checks-whose-message-did-not-match-their-condition-2026-08-16**. `shell-safety-guard`'s backtick rule was a bare character search with no exclusion for the QUOTED HEREDOC its own message recommends, so it fired on commands already following its advice — ~13 times in one session, correctly ignored every one, against a sibling guard that fired 4 times and changed the text 4 times. Separately, the test gate reported "tests failing" when the true condition was "pytest is not importable by the interpreter that has the deps": a Homebrew pytest was first on PATH, the project's jsonschema was elsewhere, 75 tests died on ModuleNotFoundError and 906 passed under a working runner. Both now resolve a runner that can actually run, prefer `uv` with the pinned requirements and no global install, and warn-and-skip rather than fail when nothing can run. Running ruff for real for the first time surfaced one live FURB167. Fixing the runner then exposed two structural checks that had inherited a tool's absence as a silent pass, leaving main unable to pass its own gate: the lint-POLICY check was nested inside the linter-AVAILABILITY check, and gates.sh marked pytest and ruff MISSING whenever they were unreachable from the first interpreter on PATH. Both fixed. One defect this release INTRODUCED was caught by its own pre-existing test within the hour: resolving ruff AT the pin made the installed-vs-pinned divergence check vacuous, so the comparison now uses the AMBIENT ruff and reports "not checked" rather than "matches" when there is none.*
