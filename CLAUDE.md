...
*Version 0.56.2 -- **Attribution label: install-command-drift-2026-07-04** (roadmap Show-HN readiness review — doc patch + graduation). **Install-command drift fixed and made enforceable**: 53 skill `framework_dependency_note` frontmatter notes used the slash marketplace ref (`mycelium@haabe/mycelium`) instead of the dash-form name `haabe-mycelium` from `.claude-plugin/marketplace.json`, so the ref resolved to a nonexistent marketplace and failed on paste; README/get-started/install-paths were already correct. All 53 corrected. A blind cold-install readiness test then surfaced further Show-HN friction (slash-command blocks fenced as shell, a missing prerequisite line, a get-started↔install-paths legacy-npx contradiction, a stale 55/57 skill count) — all fixed. New **Check 46** (install-command marketplace ref is canonical) graduates the recurring documented-rule-diverges-from-enforcement / doc-drift class to CI teeth. **Priors**: v0.56.1 **canvas-health-enum-2026-07-03** (/canvas-health rubric aligned to schema), v0.56.0 **discovery-gate-2026-07-02** (auto-dogfood utterance-mode findings), v0.55.3 **scenarios-schema-2026-07-01** (theory-fidelity B6/B7 close) (see `docs/changelog.md`).*
