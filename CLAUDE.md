...
*Version 0.52.0 -- **Attribution label: non-software-routing-fix-2026-06-29** (lived-friction-triggered). Fixes non-software-object DRIFT (dogfood: a career-discovery project where the agent reinvented skills as freeform `outputs/*.md` instead of routing). Two-part fix: **(A)** made 4 software-shaped skill descriptions object-agnostic — `wardley-map`, `user-needs-map`, `jtbd-map`, `user-interview` — so they route on non-software objects (copy the `handoff`/`log-evidence` template: name the ACTIVITY, not the object); **(B)** added an always-on Communication Rule — route through skills, canvas is source of truth, human docs are DERIVED not invented in parallel, with an ESCAPE VALVE for tasks no skill/emitter covers (the prohibition is on REINVENTING a skill's output, not on producing artifacts the system can't yet emit). Blind-tested BEFORE commit: routing 4/4 on a non-software object; a rule-adversary pass caught + fixed an over-absolute "canvas is the deliverable" framing and the missing escape valve. Crux refinement: "object=software incidental" holds at the CONFIG layer but routing/descriptions were still software-shaped. **MINOR** (routing-affecting skill-description changes + new always-on convention). Deferred follow-ons (gated, not authored): a non-product object-type + software-fallback fix; a canvas→discovery-deliverable emitter; a skill-duplication drift-detector. **Priors**: v0.51.0 **output-discipline-interface-load-2026-06-28** (interface-load/problem-load output discipline), v0.50.1 **receipt-render-killed-source-fix-2026-06-20** (see `docs/changelog.md`).*
