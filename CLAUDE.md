...
*Version 0.142.0 -- **Attribution label: the-guard-was-wrong-more-often-than-not-2026-08-30**. **THE `$?`-AFTER-A-PIPELINE GUARD WAS WRONG 53-60% OF THE TIME, AND NOBODY KNEW BECAUSE THE LEDGER COUNTED RE-FIRES INSTEAD OF CORRECTNESS.** A re-fire rate says how often a signature recurred; it cannot tell a warning ignored from a warning that was WRONG. Measured over **12,260 real Bash commands** from dogfood session transcripts, scored with the guard's own regexes: 223 fires, **135 of them effective false positives** -- five to six times outside Tricorder's <10% advisory bar (ICSE 2015). **TWO CAUSES, BOTH NARROWED.** The `|` was frequently inside a quoted string or quoted heredoc, so no pipeline existed at all -- `grep "a\|b"`, `jq '.a | .b'`; and the `$?` usually belonged to a different command using a REDIRECT, `cmd > log 2>&1; echo "rc=$?"`, which is the recommended idiom and was being warned about. Pipes are now found only outside quotes and heredocs, and the `$?` must sit in the segment immediately following the piped one -- the only shape where `$?` reports a pipeline. **Validated against the corpus: 88 of 88 true positives kept, 135 of 135 false positives dropped.** **A CHECK THAT IS WRONG MORE OFTEN THAN RIGHT TEACHES THE READER TO SCROLL PAST THE LINE WHERE A REAL DEFECT WOULD APPEAR.***
