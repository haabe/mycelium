...
*Version 0.23.5 -- **Attribution label: research-while-here** (per the new Work-Mode Mix discipline shipped in this same bump — first version-line to wear its own attribution label). Three coordinated changes: **(1)** New harness doc `harness/context-management.md` codifying Mycelium's context-rot defense layer — names the 6 mechanisms (lost-in-the-middle, attention dilution, instruction drift, NIAH degradation, compaction loss, prompt-cache bloat), maps each to existing Mycelium defenses (phase-scoped loading, JiT detection, two-memory system, Read-before-Write, canvas as SSOT, externalize-everything), explicitly names the blind subagent pattern, calls out Claude-specific findings (especially sensitive to irrelevant context per Chroma study), lists known gaps without graduating speculative defenses. Primary sources cited: Liu et al. 2023, Chroma "Context Rot" 2025, NVIDIA RULER, Anthropic Engineering blog, Anthropic April-23-2026 Claude Code postmortem. **(2)** Citation backfills: Recency Bias in Context (`harness/cognitive-biases.md`) cites Liu 2023 + cross-references Knowledge Reconstruction Tax + the new context-management doc; Cognitive Offloading Loop + Knowledge Reconstruction Tax (`harness/anti-patterns.md`) cite Chroma 2025 + cross-reference each other and the new doc — symptomatic-bias and structural-anti-pattern now sit beside each other in the agent's mental model. **(3)** Per-graduation attribution discipline: new section in `engine/version-discipline.md` requires every Version line to include a dominant attribution label (`lived-friction-triggered` / `research-while-here` / `maintenance-housekeeping` / `scheduled-discipline`); new Work-Mode Mix section in `engine/feedback-loops.md` defines the four modes + when each is fine vs risky. Surfaced 2026-05-10 in-session: the discipline closes a meta-instance of anti-pattern #7 at the graduation-velocity layer (one genuine trigger extending into a streak via consistency rather than per-graduation attribution). Logged as new corrections.md instance + cluster-instances.md entry in mycelium-roadmap. PATCH per version-discipline (citation additions + new doc on existing-surface concerns + audit-trail discipline tweak; no new mechanism beyond the attribution-label requirement, which is observability not enforcement). Connection to lived-friction: medium — today's CLAUDE.md restructure (51k → 17.9k) was implicit attention-budget management; today's 4-bump session is exactly the multi-turn agent-loop scenario the research describes; meta-instance of anti-pattern #7 surfaced the graduation-velocity failure surface concretely. Research-while-here is the honest dominant label.*
