...
*Version 0.109.0 -- **Attribution label: three-counters-three-answers-2026-08-09**. Three shipped artifacts counted the entries in `corrections.md` and returned **100, 103 and 141** for the same file on the same day. The banner in `preflight.sh` -- the number printed into every session's context before any work starts -- was the worst of the three: `^### ` missed `##` and `####` entries, missed the entire bullet form most recent entries use, and counted section headings like `### Prevention rule` as corrections. Wrong in both directions at once, and degrading with use. **This was the same defect's third instance**: fixed in `check_correction_attribution` in v0.80.1 on 2026-08-03, then `check_cluster_reconcile` shipped in v0.99.0 on 2026-08-06 -- three days later -- carrying the identical blind spot, because the fix lived in one file's regex and nothing named the format itself. So the remedy is not a fourth correct regex: `_corrections_lib.py` holds one definition, and `tests/fixtures/corrections/mixed.md` is a fixture every counter is tested against. Bash cannot import Python, so the hook's agreement is held by a test that runs the real hook against the real fixture and compares its banner. 11 tests, verified to bite in both directions.*
