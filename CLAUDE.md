...
*Version 0.137.0 -- **Attribution label: every-fail-open-site-got-a-written-verdict-2026-08-26**. v0.135.0 shipped `check_fail_open` REPORT-ONLY with no baseline seeded, because `ruff.toml` forbids re-introducing tolerated debt and each site needed a judgement first. **All 43 now carry one, and writing them found three handlers that produced a GREEN by not being able to look.** `check_reply_owed` returned `[]` on an unparseable `human-tasks.yml` and printed *"OK: no reply owed across 0 task(s)"* — while the branch immediately above it handled a MISSING file honestly (*"No task file is not a clean pass; it is nothing to check. Say which."*), so the file contained both the failure and its own remedy. `check_bvssh_reconcile` had it in **two of three** failure combinations: an unreadable decision log reconciled cleanly against the canvas, and both unreadable reported "nothing to reconcile". `check_instrument_contract` conflated a MALFORMED `score_by` with an absent one, reporting an instrument that carries `score_by: 2026-13-45` as having no scoring date. **CI is now `--strict`**: it fails only on an UNREVIEWED handler, so a new default-on-exception site costs one judgement. **A pre-existing test had pinned one of the defects as the contract** — its stated intent (does not raise) is kept, its assertion amended.*
