...
*Version 0.53.1 -- **Attribution label: theory-fidelity-corrections-2026-07-01** (theory-fidelity audit → correct). Track 1 of the theory-fidelity remediation: factual corrections surfaced by a full audit of `docs/theories.md` (Tier 1/2/3, hunting the v0.53.0-shape "claimed-wired, actually prose/manual" gaps). Fixes — all corrections to TRUTH, not softening: (1) **Hoskins scenarios** — dropped the fabricated 4th element "Means" (Hoskins's model is THREE: Motivation/Persona/Simulation) and a fabricated "SAP-talk" citation; restored the O'Reilly book (`/user-interview` + `theories.md`); (2) **OWASP LLM Top 10** — `/security-review` listed the stale 2023 taxonomy under a "2025" label; swapped for the real v2025.1 list (LLM01–10), aligned with `/threat-model`; (3) **Tier-3 attribution drift** — corrected CALMS (→`bvssh-check`, not retrospectives), TPS 7-Wastes (→delivery/retrospective, not `value-stream.yml`), Bansal (→`xai-check` adaptive-explanation, not cognitive-forcing), Selbst&Barocas (→recourse-as-substance in `xai-check`, not "disparate impact in regulatory-review"), Norman (dropped an unattributed example); REMOVED the decorative "Halland CORE" citation (nowhere operationalized). The REAL-MECHANISM builds (build-to-learn/earn gate, Cynefin domain-gate, ICE Check 38, Torres selection-authority, Sinek Why-first, schema cluster) follow as separate gated releases — each scenario-designed (happy/sad/bad paths) + blind-tested. **PATCH** (fidelity corrections; no new mechanism). **Priors**: v0.53.0 **outcome-discovery-loop-move1-2026-07-01** (wires the outcome→discovery loop), v0.52.0 **non-software-routing-fix-2026-06-29** (see `docs/changelog.md`).*
