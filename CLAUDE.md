...
*Version 0.123.0 -- **Attribution label: the-block-tier-gate-could-not-see-the-artifact-it-guarded-2026-08-23**. **`purpose-stance` was specified for diamonds from the day it shipped and implemented only for solutions.** The wiring spec named three diamond firing points -- 4 and 5 at the Define->Develop and Develop->Deliver transitions (`BLOCK`), 7 in the validator (`WARN`, never fail) -- and `diamond-progress` was ALREADY invoking `check_purpose_stance.py --strict` at exactly those transitions. **The script it called never opened `active.yml`.** It read the solutions, found them clean, returned green: a `BLOCK`-tier gate that runs, passes, and cannot see the artifact whose transition it guards -- **this framework's own blind-green defect, sitting inside the checker built to catch it.** Measured on the dogfood canvas: **0 of 4 diamonds carried a stance**, and none were exempt, because the 53-entry grandfathering list is solution IDs only. Surfaced by a founder asking out loud whether an L1 outcome re-specified EIGHT DAYS BEFORE the current why/what still fits it -- **nothing could answer him.** Now the script reads both files. The `BLOCK` tier is scoped with `--diamond-id` so a transition is never failed by a DIFFERENT diamond's silence, parked diamonds never block, a Discover diamond is nagged only once it has a `definition_of_done` to drift, and firing point 6 lands in `/mycelium:define-done` -- WARN, at the one moment the builder has just said what done means.*
