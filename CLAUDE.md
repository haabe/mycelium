...
*Version 0.31.6 -- **Attribution label: learning-target-coupling**. Closes the "we asked for feedback but didn't ask what we needed to learn" gap: a canonical `learning_target_coupling` convention in `engine/canvas-guidance.yml` (seed ≥1 feedback question per open canvas gap — ON HOLD / RE-GATED flags, in-progress human-tasks naming a MISSING SIGNAL, low-confidence un-validated assumptions), an inline `[target → <file>#<anchor>]` question tag for grep-detectable close-the-loop routing, steps added to `/user-interview` (Pre-Interview) and `/assumption-test` (Step 1), a `/log-evidence` close-the-loop note, and a canvas-health NUDGE check (8d) flagging open feedback tasks that target zero open gaps. Graduated from a lived instance: cohort first-runs produced rich friction logs while the canvas carried open positioning/hypothesis gaps the feedback never targeted — so we learned what we happened to observe, not what we needed to learn. **PATCH per version-discipline**: prose-only across one engine file + four skill files, single NUDGE-tier check added, no BLOCK/REVIEW gate, no mechanical validator change (matches the v0.31.3 human-task-reconciliation precedent — agent-run prose check, no G-V12 test obligation; a mechanical validate_canvas.py Check is flagged as a future graduation candidate). **Attribution class**: lived-instance-triggered + maintainer-directed ("capture and ship now"). Rationale in `harness/decision-log.md` 2026-05-29; changelog entry per Check 34.* **Prior**: loop-extremity-streetlight (v0.31.5).
