...
*Version 0.55.0 -- **Attribution label: build-mode-gate-2026-07-01** (theory-fidelity B1). Makes the "earn the right to start" thesis REAL (not softened, not a nudge). The audit's worst over-claim: `theories.md` said build-to-learn-vs-build-to-earn was "enforced via G-M2 + define-done", but G-M2 is an off-target NUDGE (experiment audience-scope), `/define-done` had ZERO build-mode awareness, and `philosophy.md`'s "that refusal is the gate" pointed at no gate. Now `/define-done` carries a **build-mode gate**: a build-to-LEARN diamond (L0–L3) may not set an earn-shaped DoD `outcome`, where earn-shaped = **the ship IS the done-bar, or scope is all-users/production** (NOT merely mentioning "ship" — fake-door / opt-in prototypes are ship-to-LEARN and pass; the discriminator is the done-bar). Backstopped by `/canvas-health` **step 8c**: an unconditional earn-verb lint on every L0–L3 diamond's DoD that WARNs + routes the semantic call back (converts a birth-only agent-adjudicated gate into a tripwire that fires regardless). **Blind-tested BEFORE commit** — adversary caught (1) the bare verb-list false-positived on legitimate ship-to-learn (fixed with the done-bar/scope discriminator), (2) no machine backstop = birth-only (added step 8c), (3) philosophy.md frames the line by PHASE while the skill keys by SCALE (reconciled: DoD keys to the diamond's terminal altitude; mode still shifts across a diamond's phases — an L4's early spike is build-to-learn), (4) "enforced" over-stated a prose rule (relabelled agent-adjudicated-at-birth + lint backstop). Updated: define-done, canvas-health, philosophy.md, theories.md. **MINOR** (new gate on the live define-done path). **Priors**: v0.54.0 **torres-selection-authority-2026-07-01** (B4), v0.53.1 **theory-fidelity-corrections-2026-07-01** (Track 1; see `docs/changelog.md`).*
