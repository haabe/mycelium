...
*Version 0.23.40 -- **Attribution label: lived-friction-triggered**. Continuation of v0.23.39 `/interview` cold-start gap fix. Verification re-run of `onboarding-solo-cold-start.yml` post-v0.23.39 surfaced a second gap: `decision_log_entries: None` after the brief — the brief flow's Step 2 wrote canvas files but NOT a decision-log entry. The existing decision-log instruction lived in the "After the Interview: What Happens Next" section, reachable only when the user takes specific depth-menu paths. Users who stop after the brief get no decision-log entry. The interview IS the most foundational decision in the project lifecycle; it should write its own log entry at the point of writing the canvas, not in a downstream section that may not be reached. Fix: Step 2 of brief flow now appends a minimal decision-log entry alongside the canvas writes (purpose.yml + jobs-to-be-done.yml + active.yml + **decision-log.md**). The "After the Interview" section's deeper entry can extend/replace the minimal one when reached. PATCH per version-discipline: atomic-commit rule applied.*
