...
*Version 0.39.7 -- **Attribution label: readme-restructure-articulation-driven-2026-06-04**. README restructured per Core Model methodology + voice-revise-framework-doc discipline. Driving findings: non-founder purpose-articulation test (6 returns) plus brownfield-iteration eval surfaced structural symptoms (density at top-of-page with 12 H2 + multiple Diátaxis quadrants, missing docs-handoff signal above the fold, absence of the load-bearing two-products / factory external framing). Hero rewritten with factory framing in the lead paragraph and canonical purpose merged in; sections reordered to target IA (16 H2 → 11 H2); "What it feels like" converted from a three-mode table to story-shaped prose triggering curiosity / safety / clarity beats; centralized-CRUD scope bullet added to "Who it's not for"; receipts case titles humanized; Quick start + Upgrading collapsed to a single "Resuming work" beat plus new `docs/install-paths.md` consolidating install variants, migration, upgrading, and self-hosted runtimes. Voice discipline: em-dashes removed across the README (C-001), AI-tells absent. **PATCH** (docs only, no skill behavior changes). **Prior**: sol-007a-technical-discovery-vocab-2026-06-03 (v0.39.6); v0.39.4-validator-followup-2026-06-03 (v0.39.5).*
