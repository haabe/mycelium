...
*Version 0.39.10 -- **Attribution label: log-evidence-untracked-channel-backfill-2026-06-05**. `/mycelium:log-evidence` step 1 now handles the no-matching-task case explicitly: when user notes describe an exchange with a contributor not covered by any `pending_tasks`, the skill stops, surfaces the symmetric-to-`8c(b)` drift, and offers two paths — (a) backfill an `ht-XXX` inline with a `backfill_note`, or (b) pause to `/mycelium:handoff`. Free-form capture with no `ht` reference is no longer the happy path. Paired with new `/canvas-health` sub-check `8c(d)` (untracked-channel evidence): scans 30-day window for `external_human` entries naming a contributor with no matching task and flags for backfill. Catches the symmetric inverse of `8c(b)` (evidence-with-no-task). Dogfood trigger: roadmap `ht-022` Dean Peters channel — DM reply landed without a pre-registered task, drift was only visible because `/log-evidence` happened to run. **PATCH** (skill behavior refinement, no engine/hook changes). **Prior**: poison-detector-rejected-alts-exclusion-2026-06-04 (v0.39.9); docs-readme-persona-index-2026-06-04 (v0.39.8).*
