...
*Version 0.23.2 -- Five bias-mitigation citations from cognitivebiasindex.com gap analysis (mycelium-roadmap dogfood 2026-05-10). Citations land on existing surfaces with clean primary-source references: **Illusion of Validity** (Kahneman 2011 ch. 20) added to anti-pattern #7 *Consistency-as-Evidence* as the established academic name; **Curse of Knowledge** (Camerer/Loewenstein/Weber 1989) added to L2 cognitive-biases.md as a row in the Opportunity-stage table (founder-vocabulary risk in user interviews); **Hindsight Bias** (Fischhoff 1975) added to `/retrospective` as a new check section (counters "we should have seen X coming" rewriting uncertainty); **Hyperbolic Discounting** (Laibson 1997) added to `/ice-score` Step 5 bias check (near-term Impact overweighting); **Decoy Effect** (Huber/Payne/Puto 1982) added to `/ost-builder` Rules (no decoy solutions). PATCH per version-discipline (additive citations + 1 new table row + 1 new mitigation section in existing skill; no new mechanism; user-override of the draft's "wait for real failure" promotion bar logged in `mycelium-roadmap/.claude/drafts/cbi-gap-followups.md`). Notable non-additions documented in the same draft: 1 candidate (Dunning-Kruger) was already present in L1+L4 — caught during pre-ship Read; CBI's 14 "tools" already covered by existing Mycelium mechanisms (decision-log, /devils-advocate, /fan-out, /assumption-test, etc.).*
