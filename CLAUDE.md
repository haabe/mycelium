...
*Version 0.59.0 -- **Attribution label: bvssh-assessment-reconcile-2026-07-25** (dogfood-surfaced defect; every consumer affected). `/bvssh-check` MANDATED a decision-log append and never mentioned `.claude/canvas/bvssh-health.yml`, while `hooks/session-start.sh` computes the "BVSSH overdue" reminder FROM that canvas -- the skill wrote to A, the detector read B, nothing connected them, so `last_assessed` went stale permanently and the overdue nag fired forever. Adds the missing **mandatory Canvas step** to the skill (append `assessment_history`, update both `last_assessed` fields, CALMS status/evidence, `_meta`, and the stale-prone top-of-file snapshot blocks, then verify-after-write), a **`check_bvssh_reconcile.py`** guard (dated decision-log headings must match `assessment_history` entries; absent-input discipline both ways per AP#9), a **SessionStart orphan reminder** that flags when the overdue count itself is unreliable, and **10 G-V12 coverage tests** fixtured on the three real orphans found (2026-05-20 framework; 2026-06-20 + 2026-07-11 dogfood). The 06-20 orphan was previously "fixed" with a prose rule in a notes field; 07-11 is that rule failing -- this ships the step, not more prose. **Priors**: v0.58.1 **consumer-dogfood-test-and-architecture-doc-2026-07-25**, v0.58.0 **plugin-form-operating-contract-injection-2026-07-25** (see `docs/changelog.md`).*
