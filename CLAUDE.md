...
*Version 0.36.2 -- **Attribution label: framework-health-followthrough**. Acts on the three `/framework-health` (2026-05-30) recommendations: (1) **gate-defers** the documented-rule-diverges-from-enforcement cluster's mechanism graduation with a *named, logged* gate (≥3 spec Rules implemented + run against the 15-instance corpus at measured FP <5%) instead of silently leaving it pending or faking the validation — recorded in `cluster-instances.md`; (2) **rotates the README receipts** (alex-cohort-first-run in, drew-hoskins-takehome out — stays in `docs/receipts/cases/`) and corrects the now-stale "one outside user" count to "outside users"; (3) **fixes the two step-4e Von-Restorff flags** (regulatory-review + diamond-assess output templates now render the gate **Fail** case distinctly) and **codifies the gate-rendering convention class-level** in `design-principles.md`. Broader checklist-skill sweep deferred (G-C1 "lead with verdict" territory, not blocker-pop); mechanical 4e check deferred (first flag, two-strike rule). **PATCH** (follow-through edits; no schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: ux-axioms-bridge-remediation (v0.36.1), ux-axioms-bridge (v0.36.0).*
