...
*Version 0.49.11 -- **Attribution label: python-test-coverage-62-to-90-2026-06-18** (test-coverage gap surfaced by a dogfood audit: 4 scripts at 0%, the load-bearing validators under-covered, hooks untested — and crucially, coverage was reported but NOT gated). Adds ~109 tests: `check_legacy_paths` 0→91% (locks the v0.49.7/9 false-positive regressions), `validate_canvas` 58→91% (trace-resolution + cycle-detection + CLI), `framework_guard` 64→99% (deny-message builders + bash-deny path, locking the v0.49.8/10 message fixes), and first-ever python coverage for `validate_mermaid` (87%), `autonomous_evidence_guard` (93%), `check_gated_by` (83%), `check-tool-calling` (86%). New bash smoke-tests for the `post-write-nudge` + `stop-check` hooks (the untested runtime surface that let the v0.49.7 schema-nudge bug ship). CI now gates coverage: `--cov-fail-under=85` + `integrations/` pulled into scope. Total scripts coverage 62%→**90%** (270 python tests pass). Tests + CI. **PATCH**. **Priors**: v0.49.10 **check-legacy-paths-doc-quote-false-positive-2026-06-18**, v0.49.9 **legacy-path-rot-receipts-case-2026-06-18** (see `docs/changelog.md`).*
