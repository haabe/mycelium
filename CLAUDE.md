...
*Version 0.51.0 -- **Attribution label: output-discipline-interface-load-2026-06-28** (lived-friction-triggered). Adds an "interface-load vs problem-load" output discipline to `engine/status-translations.md`, applied to ALL user-facing output via the always-on communication rule in `CLAUDE.md` (broadened from diamond-state translation) plus explicit references from `log-evidence` and `devils-advocate` (highest-traffic prose surfaces by usage proxy; `diamond-assess`/`diamond-progress` already load the doc): a three-move per-sentence test — cut framework-facing words (taxonomy, mechanism jargon), keep problem-facing substance, rewrite/relocate sentences that carry a latent function (credibility, brownfield-safety) — grounded in Krug's third law ("get rid of half the words"). Rewrites the `/mycelium:start` welcome as the worked example: drops the framework-acronym wall (JTBD/OST/Wardley/…) and mechanism jargon, keeps the credibility signal and the brief's contents. Optimizes user-facing output without cutting substance; strictness and gate enforcement unchanged (verbosity is a presentation knob, not a discipline knob). Surfaces the verbose/walls-of-text friction (Bentes "verbose/strict", Bård "OST bounce"). **MINOR** (new convention with framework-wide output impact). **Priors**: v0.50.1 **receipt-render-killed-source-fix-2026-06-20** (PATCH), v0.50.0 **receipt-render-l1-instrument-2026-06-20** (adds `/mycelium:receipt-render` + `docs/receipts/handoff.md`) (see `docs/changelog.md`).*
