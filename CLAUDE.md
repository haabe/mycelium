...
*Version 0.33.0 -- **Attribution label: card-integrity**. Closes two latent gaps in the AI System Card. (1) The Mitchell-et-al. *template* that `/xai-check` Stage 4 reads for its `Required` markings was referenced in four places (the skill, `engine/xai-canvas-threading.md`, the card footer, `manifest.yml`'s `.claude/templates/` entry) but never created — the skill pointed at a missing file. Now shipped at `plugins/mycelium/templates/ai-system-card.md` (installed to `.claude/templates/`). (2) `docs/ai-system-card.md` had drifted to v0.15.1 / "45+ skills" while the framework shipped 0.32.0 / 49 — a live honesty problem for a published disclosure artifact, not just untidy. Both closed going forward: `scripts/sync_derived.py` now also sweeps the card's `**Version:**` and `N skills` tokens (`--check` on CI/pre-push), and new `tests/python/test_ai_system_card.py` fails if the template or any Required section disappears again. The card's *mechanical* tokens are refreshed; its *substantive* audit content stays dated to the last `/xai-check` (2026-05-04) — a token refresh is not a re-audit. 8 sync tests + 5 card tests. **MINOR** (new template artifact + generator coverage + CI guard; no agent-behavior or schema change). Rationale in `harness/decision-log.md` 2026-05-30. **Prior**: deferred-generators (v0.32.0), audit-medium-group (v0.31.12).*
