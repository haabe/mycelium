...
*Version 0.49.10 -- **Attribution label: check-legacy-paths-doc-quote-false-positive-2026-06-18** (the v0.49.6 guard flagged documentation that QUOTES the moved paths to describe them — v0.49.7 went red on the CLAUDE.md version line, v0.49.9 on the receipts case; both passed local `validate-template.sh` because the guard runs only in `validate.yml`, not the local validator). Fix: `check_legacy_paths.py` now skips the CLAUDE.md `*Version` line (an embedded changelog record, same rationale as the allowlisted `changelog.md`) and allowlists the legacy-path-rot receipts case. The guard's own message already prescribed the allowlist path for intentional documentation. Process note: CI gates beyond `validate-template.sh` (`check_legacy_paths.py`, `check_doc_references.py`) must be run locally before pushing framework changes. Scripts. **PATCH**. **Priors**: v0.49.9 **legacy-path-rot-receipts-case-2026-06-18**, v0.49.8 **framework-guard-sync-message-harness-neutral-2026-06-18** (see `docs/changelog.md`).*
