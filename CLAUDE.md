...
*Version 0.49.4 -- **Attribution label: opencode-e2e-friction-fixes-2026-06-16** (the end-to-end test on opencode + llama3.1:8b PASSED — workflow runs, vendored-ref resolution + read-before-edit guard confirmed live — and surfaced three friction fixes). Plugin: `MYCELIUM_PREFLIGHT=off` opt-out for the `chat.message` preflight injection (verified: ON injects / OFF skips), since it can distract weak local models from tool-calling. Docs: `OLLAMA_CONTEXT_LENGTH=32768` must be set on the SERVE PROCESS not the shell (the #1 setup gotcha — GUI/brew launchd `ollama serve` doesn't inherit a shell export). `provision-skills.sh`: residual-`${CLAUDE_PLUGIN_ROOT}` warning now flags only path-shaped refs (whitelists prose mentions — no more false positives; verified clean). Doc + scaffold + small plugin opt-out. **PATCH**. **Priors**: v0.49.3 **opencode-model-toolcalling-guidance**, v0.49.2 **runtime-verification-before-ship** (see `docs/changelog.md`).*
