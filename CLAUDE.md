...
*Version 0.114.0 -- **Attribution label: a-review-date-promises-a-decision-not-data-2026-08-20**. A prediction gated on an EVENT cannot honestly carry a scoring date, because the data may never exist — so seven of them carried no date at all and "waiting" was indistinguishable from "forgotten". Founder ruling: event-gated instruments carry a `review_by`, the date by which you decide whether to KEEP WAITING, and a decision is always available even when data is not. Optional `review_on` + `review_on_fingerprint` fire the review on a trace of the event instead of the calendar. THE TRIGGER'S LIMIT IS DOCUMENTED WHERE IT IS IMPLEMENTED, because the founder asked the right question of it — "how can we know whether an event has happened if we don't track them?": it does NOT track events, it detects change at a nominated anchor, so it inherits the discipline it backstops. Point it at an event LOG (touch_log entries, a new task id, git history), never at a prose summary that only updates once someone has already noticed.*
