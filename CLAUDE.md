...
*Version 0.78.0 -- **Attribution label: correction-attribution-2026-08-03**. The correction loop's headline number — who caught the mistake — was the least automated thing in the loop. `/corrections-audit` computed it by hand, and it had been produced exactly twice, six weeks apart, each time because someone remembered to look. It is the one number that says whether the answer is more harness or more context. `scripts/check_correction_attribution.py` computes it as defect-escape analysis — of the defects that occurred, what fraction escaped the automated net — and prints its denominator every time: 14 of 72 entries name a catcher, so a rate quoted bare would be a claim about 19% of the corpus wearing the clothes of the whole, which is the exact failure v0.77.0 spent itself removing. `NO RATE AVAILABLE` is not 0%. It exits 0 always; a gate on this rate would reward logging fewer user-caught mistakes. **The gate feeding it was miscounting.** `reflexion-gate.sh` fired on any non-zero exit, but grep(1), diff(1) and test(1) all document non-zero as a RESULT — no line selected, differences found, expression false — so this was a category error, not a judgement call. 39 firings had accumulated, 23 of them in one session, and the five most recent were all greps that matched nothing; the counter announced "30 outstanding learnings" at every session start until it was ignored, which is how a guard dies. Documented non-failures are now classified and still logged WITH their reason — dropping them silently would make the classifier unauditable, which is the failure one level up — and `reconcile_reflexions.py` excludes them from learning debt while stating the suppressed count even when nothing remains to reconcile. Check 51 also gains the `rc==3` PyYAML-skip arm that Checks 50/52/53 received in v0.77.0; without it the skip fell through to `else` and hard-failed, blocking pushes from any machine without PyYAML.*
