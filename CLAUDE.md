...
*Version 0.145.2 -- **Attribution label: the-changelog-promised-a-release-the-gate-had-refused-2026-08-31** (PATCH). `release_gaps.py --check` failed on main: the changelog documented v0.145.0 and no such release existed. **Cause: that commit was pushed with `--no-verify`**, skipping the pre-push `check_fail_open.py --strict` gate; CI went red on exactly that check and auto-release's green-validator step refused to publish -- the v0.144.0 gate working on its first real exercise. Fixed by folding v0.145.0's changelog section into v0.145.1 rather than releasing a commit CI rejected. NOT caused by the v0.144.1 tag split: all four tags from that work are present and released. Also recorded: **Check 26 compares COMMITTED changes**, so it passes before `git commit` and fails at pre-push on the same tree.*
