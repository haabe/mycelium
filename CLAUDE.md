...
*Version 0.57.3 -- **Attribution label: diamond-evidence-type-enum-2026-07-19** (roadmap/i-productified dogfood — validator-correctness fix). **Schema-enforcement patch**: the diamond-record `evidence_type` field in `schemas/diamonds/active.schema.json` was typed `{"type": "string"}` instead of `$ref`-ing the `evidence_type` enum, so a `source_class` value (`internal_stakeholder`) written into that field survived ~3 weeks of `validate_canvas.py` PASSes in a downstream project — a wrong evidence-ladder rung reading as a valid claim (load-bearing for confidence discipline + every gate that reads it). Caught only by a human-invoked `/canvas-health` grep. Fixed to `$ref` the enum (the enforcement canvas provenance blocks already carry); added two regression tests in `test_validate_canvas.py`. A cross-schema scan confirmed this was the ONLY named `evidence_type`/`source_class` property typed bare-string framework-wide. Sub-shape of documented-rule-diverges-from-enforcement (enforced on the canvas surface, omitted on the diamonds schema retrofitted 2026-06-12). **Priors**: v0.57.2 **opencode-upstream-closure-2026-07-19** (recorded bot-closure of opencode #27899 + PR #32542), v0.57.1 **docs-drift-skillcount-2026-07-18** (documentation drift sweep — skill counts → **58**), v0.57.0 **harms-check-gate-2026-07-11** (new `/harms-check` design-time safety gate) (see `docs/changelog.md`).*
