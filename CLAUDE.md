...
*Version 0.115.1 -- **Attribution label: the-first-guard-that-ships-red-2026-08-20**. **WIRING VERIFIED 2026-08-21** from the installed plugin: the guard fired on a live Bash call, which also surfaced a limitation now documented in the script — on Bash it reads the COMMAND TEXT, so a command that only mentions the path warns without writing. Fail-safe direction, and not fixable at PreToolUse. `correction-attribution-guard` fires at the moment a new `corrections.md` entry is written without naming who caught the mistake. The rule has been HARD in the operating contract since 2026-08-03; compliance since is **15 of 91 entries (16%), LOWER than the 29% it scored while still advisory**. The pattern is bimodal, not declining — 9/11 on the day the attribution script was written, 8/9 a week later, ~0 on every other day including 0/5 in the session that measured it. So the failure is TIMING, not comprehension: the contract is read at session start and the write happens hundreds of turns later. **It also matches Bash, and that is load-bearing** — corrections.md is appended with a heredoc, so a Write|Edit-only guard would have shipped green against the exact corpus that motivated it. **This is the first guard here to ship RED**: it fires on 77% of the existing corpus and 84% of entries written since the rule was hardened, which is the inverse of the near-zero-action-rate problem every recent guard has carried. It warns and never denies; the escalation criterion is stated in the script rather than left to appetite.*
