...
*Version 0.148.0 -- **Attribution label: content-in-key-position-ruled-a-real-problem-2026-08-31** (MINOR). Founder ruling: keys carrying dates or entity ids in their NAMES defeat every field-level mechanism and the content belongs in VALUES. **A date in a value can be compared and can go overdue; a date in a key can only be grepped** -- which is why `horizon_set_2026_08_28` never fires while `horizon: 2026-08-28` does. `check_key_shape.py` ships REPORT-ONLY (523 such keys exist on the dogfood canvas; failing them all on day one is a root canal for a floss problem), with a `--write-baseline` ratchet so `--strict` costs only NEW writing. **The interview corrected its own premise**: asked what he intended by these fields, the founder answered that AN AGENT WROTE THEM ALL and he has no knowledge of them -- agent-invented fields no human ever ruled on. **The gate's own regex was wrong and its own test caught it** (`\b` never matches before `_status`), under-counting the population at 507.*
