...
*Version 0.103.1 -- **Attribution label: a-same-day-reply-is-still-a-reply-2026-08-07**. **REPLY-OWED false positives: a same-day reply read as unanswered.** `last_contact()` compared dates with STRICTLY-GREATER, so on an equal date the first entry seen won -- and inbound is logged before the reply to it. 'They wrote, you answered the same day' therefore scored as owed. Dogfood 2026-08-07: BOTH live flags on the consumer canvas were this bug (ht-060, reply sent same day in founder-final text; ht-003, a 2026-08-01 inbound answered on 2026-08-01). **A false REPLY OWED is not harmless -- it sends the operator to contact someone they already answered, which is the same real-person cost v0.101.0's obligation guard exists to prevent, pointing the other way.** Tiebreak is LOG POSITION, not direction: preferring outbound on a tie would silence the honest case where they wrote BACK the same day, and that case now has its own test. Does NOT weaken the existing out-of-order guardpost, which concerns DIFFERENT dates where the date still wins; position is consulted only when dates are equal and carry no information. 13 bash tests (2 new), inserted ABOVE the runner's `report` call after v0.102.0's built-not-wired lesson.*
