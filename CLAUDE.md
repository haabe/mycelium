...
*Version 0.60.1 -- **Attribution label: verify-citations-matcher-2026-07-25** (guard correctness). `verify_citations.py` required a colon in `(per: X)`. Across 19 captured sessions that form occurs ZERO times -- every real citation is `(per X)` -- so the checker **matched 0% of live citations for ~2.5 months while reporting "no problems found"**: a guard that cannot match its target, AP#9, same class as the legacy-path-rot green audit. Colon now OPTIONAL (not removed; the documented convention uses it). Also fixes `looks_like_file_path()` reporting PROSE as unverified file paths -- every "unverified" verdict in the corpus was a slash inside prose ("Checks 26/30", "doc-reference/legacy-path"); detection is now per-token and requires an extension or slash+dot, erring toward UNVERIFIABLE over false UNVERIFIED. Measured effect on the real corpus: unverified 3 -> 0, 27 citations correctly concept-shaped. 6 regression tests. **Priors**: v0.60.0 **opencode-consumer-path-2026-07-25** (see `docs/changelog.md`).*
