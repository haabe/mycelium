...
*Version 0.144.0 -- **Attribution label: the-validator-now-gates-the-release-2026-08-31**. **v0.143.0 PUBLISHED AS LATEST WHILE ITS OWN VALIDATOR WAS RED ON THE SAME COMMIT.** `auto-release` and `validate` both fire on `push: main` and neither could see the other, so a failing validation and a published release were independent events -- the two-mechanisms-one-gates-nothing shape this tree audits everywhere, sitting on the DISTRIBUTION path. A precondition step now waits for `Validate Template Integrity` to conclude on the release commit and blocks on anything but success. **BLOCKING IS LICENSED BY THE ERROR RATE, NOT CHOSEN**: Tricorder (ICSE 2015) requires an effective-FP rate of essentially zero to block, and this workflow measures **39 success / 1 failure over its last 40 runs, with that one failure a TRUE positive**. A flaky gate would get bypassed and then it is not a gate. **THE DELIBERATE OVERRIDE IS `workflow_dispatch`**, which skips the step by design and is recorded in the Actions log -- the *unskippable by the agent, deliberately overridable by the person* half of this project's own contract, which this path previously satisfied in neither direction. Verified against real history before shipping: the gate returns `failure` for 614ab27 and `success` for 3a5d440.*
