...
*Version 0.79.0 -- **Attribution label: fixes-to-the-fixes-2026-08-03**. A max-effort review of the v0.77.0/v0.78.0 fix batch returned 15 confirmed defects, every one reproduced by execution — defects in the fixes, in the releases written to eliminate green-over-nothing. **Check 50 decided suppression by sniffing prose.** A six-word list gated whether quoted spans were blanked, and `correct` fires on "correctly" and "incorrect"; theories.md paragraphs are one line each, up to 1368 characters, so one unrelated word blanked every quoted citation in the paragraph. Measured: 9 lines blanked, 8 by ordinary prose. It failed the other way too — a note reading `(Fixed v0.80.0: this read "...")` matched nothing, so CI went red because the author wrote "Fixed" instead of "Corrected". No linter sniffs prose to decide whether to suppress itself; they take an explicit marker (`# noqa`, `eslint-disable`, `# type: ignore`). Replaced by `<!-- validator:historical-citation -->`, and the vocabulary heuristic is deleted. **Check 51 declared `PARSE_ERRORS` and never read it** — Check 52's otherwise identical heredoc has the `sys.exit(1)` block and this one did not, so an unparseable canvas produced "nothing to reconcile", exit 0. The state most likely to hold the drift it hunts was the one state guaranteed to pass. **Check 50's N/A guard used `and`**, refusing only when BOTH populations were empty, so a citation regex that stopped matching still printed green off the back of 9 verified paths. **`validate_canvas` had two ways to exit 0 having validated nothing:** a missing schema directory over a POPULATED canvas printed "silently passing" (reachable whenever CLAUDE_PLUGIN_ROOT is stale), and the empty-canvas N/A also covered a canvas whose glob had STOPPED MATCHING — rename to `.yaml` and CI goes green over a canvas nobody read. Empty-by-birth still exits 0, because the shipped pre-push hook would otherwise block every first push; empty-by-breakage now fails. The first of those also made `check_empty_input_honesty`'s exemption for the script FALSE, and it was fixed by fixing the script rather than rewording the reason. **Two tests could not fail on the bug they were written for**, both proven by reverting the fix and watching them stay green.*
