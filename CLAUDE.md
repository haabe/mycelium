...
*Version 0.101.2 -- **Attribution label: the-repair-path-could-not-repair-2026-08-07**. **`auto-release.yml` advertised a manual gap-repair since v0.87.0 and it never worked: detection was sound, repair was decorative.** On `workflow_dispatch` `github.event.before` is empty, so `--introduced` degrades to HEAD-only and returns the version already at the tip, which by definition already has a Release. Found in anger when a GitHub outage left v0.100.0 and v0.101.0 committed, changelogged and unreleased; the backstop named both correctly and the advertised repair would have re-offered v0.101.1 and fixed neither. **`release_gaps.py --repair` now returns every documented version with no Release**, each anchored to the commit it first appeared in, in the same shape the release step already consumes; the workflow branches on `github.event_name` so a push asks *what did this introduce* and a dispatch asks *what is missing*. A version whose originating commit cannot be found is NAMED and exits non-zero, because a repair that silently emits a shorter list than the gap it was handed is the same fail-open one layer in. 4 new tests, 30 total, the first a NEGATIVE CONTROL asserting the old path returns only the tip on a dispatch-shaped call -- so if `--repair` ever becomes redundant the test says so.*

*Prior version 0.101.1 -- **Attribution label: the-critic-accepted-the-fact-it-could-not-look-up-2026-08-07**. **Docs only, no behaviour change: `docs/design/critic-claim-checkability.md` records a PROPOSED, not-built design and the dogfood evidence that refuted the mechanism it set out to borrow.** An external practitioner reported that same-context critics rationalise, and that a plan carrying bad reasoning gets accepted at face value; his fix is a forked-context Critic, and Mycelium had the exposed configuration (grep of `skills/` + `engine/` for `context: fork` / `blind subagent` returns zero; `/reflexion` Step 3 self-critiques in the builder's own context). A five-run, three-tier test built to justify the borrow refuted it. **CLAIM TYPE PREDICTS AUDITING, NOT CONTEXT AND NOT AUTHORSHIP**: a false-but-checkable external fact planted in a build plan was audited 3/3 at opus and **0/3 at sonnet and 0/3 at haiku**, while artifact-verifiable claims were audited well at every tier. **Detection never dropped** -- the defect the false fact defended was flagged 3/3 everywhere, so a found/not-found metric sees none of it. Forking is rejected as unsupported (the blind arms never saw the premise either, so no arm isolates hiding it); feeding more rationale is also rejected as an opus-only conclusion. Proposed instead: oblige load-bearing factual claims to carry a citation or an explicit `UNVERIFIED` tag. **The process lesson cost the most: validate at sonnet and haiku, never at opus alone** -- the convention already existed and this investigation broke it silently by passing no model parameter, then argued the uncertainty was harmless. Three pre-registered predictions failed, all assuming a motivational failure; the real one is epistemic.*

*Prior version 0.101.0 -- **Attribution label: an-obligation-rots-like-an-absence-2026-08-06**. **The absence-claim guard now fires on OBLIGATIONS and ROLE CLAIMS, because both rot the same way and an obligation rots more expensively -- acting on a stale one contacts a real person.** Both patterns were added after the agent made exactly these two errors in one session. It recommended writing to a contributor because an auto-memory line read `OWED: founder reply re: crediting`, when four replies had been sent and the task said in its own text \"Nothing is owed to him\" -- a REPEAT of a correction logged two days earlier about the same person, whose prose prevention rule did not hold. And it recorded a named practitioner as \"a senior product coach\" across six surfaces, inferred from marketplace profiles a search returned rather than from his own site, which says he ships software fast and lists coaching as one link among a dozen. **BOTH NEW PATTERNS MISSED THEIR OWN MOTIVATING SENTENCE ON THE FIRST CUT** -- a colon broke the obligation regex (`OWED:` not `owed `), an interposed adjective broke the identity one (\"senior PRODUCT coach\") -- which is the verify_citations failure exactly: a pattern that does not match live text. Caught by testing against the real lines rather than invented ones, and every positive fixture is now a verbatim sentence that shipped. 7 new tests, 83 total on this guard.*

*Superseded label (v0.100.0): the-leaf-side-half-of-check-38-2026-08-06. **Check 38 requires non-zero ICE on a product-leaf CYCLE RECORD. Nothing required it on the LEAF.** So a leaf could ship with the selection gate bypassed and nothing noticed, because in a tree whose opportunities all roll up to the framework, no product-leaf cycle could open to trip Check 38 anyway. **The structural zero MASKS the wiring zero**, and that is the whole finding: if the leaf lifecycle does not fire for framework leaves today it will not fire for product leaves tomorrow, when it starts eating the ICE-calibration dimension product-leaf cycles exist to feed. Dogfood measured 7 of 7 shipped leaves unscored, four of them documenting their own drift in their own status text (`SHIPPED-BEFORE-SCORING`, \"open for two months after shipping\", \"verified in code, not inferred from the canvas\"). `check_leaf_lifecycle.py` is ADVISORY via session-start; `ice_exempt` with a reason satisfies it, because a leaf may ship unscored but not SILENTLY. **It was first written as validate-template Check 54 and that was wrong** -- that runs in the FRAMEWORK repo, whose canvas has no shipped leaves, so it would have reported nothing-to-audit forever and read green: the built-not-wired class committed inside the fix for a wiring failure, caught by running it. N/A is reported distinctly from OK so \"nothing shipped yet\" and \"everything shipped is scored\" never print the same thing. 12 new tests. Found by the founder refusing a summary that had glued three unrelated zeros together.*

*Superseded label (v0.99.1): the-registry-whose-own-guard-caught-it-2026-08-06. **`right-content-wrong-surface` graduates to a general mechanism**: `engine/surface-registry.yml` declares, per recurring artifact class, which file is AUTHORITATIVE and which mechanism READS it, and `check_surface_registry.py` verifies each declared reader genuinely references its declared surface. The cluster's own criterion was that a fourth class must be a ROW and not a fourth point-check; `correction-to-cluster-instance` was added as a row the same day, which is the criterion satisfying itself. `upstream-candidate` ships as an OPEN row with a null surface and no reader, because a registry that could only express solved classes would hide its most important entry. **Then the new script was caught by an existing guard**: `check_empty_input_honesty.py` (v0.79.0) found that both scripts added today exit 0 over an empty tree, reporting SKIP -- a check that verified nothing and reads green forever, which is anti-pattern #9 inside the mechanisms built to close its sibling cluster. Both now fail the precondition (exit 2) and both gained `--root` so the guard can aim at them at all; coverage went 10 scripts to 12. **And the AP#9 cluster record was three days stale**: its cross-cutting mechanism shipped in v0.79.0 while the catalogue still read NOT BUILT, and a `/framework-health` run read that and repeated it into a dashboard the same morning. Corrected, and logged as a stale-state read on the catalogue itself. 28 new tests.*
