...
*Version 0.143.1 -- **Attribution label: the-new-handler-needed-its-verdict-2026-08-31**. **v0.143.0 SHIPPED RED AND CI CAUGHT IT.** The Bash half of the read log added one default-on-exception site -- a token-to-path resolution that skips a token which is not a resolvable project file -- and `check_fail_open --strict` reported it as NEW and unreviewed. **That step is CI-ONLY**: it is not in `validate-template.sh` and not in the pre-push hook, so the full local suite went green and the release went out failing. The judgement is now recorded as the check asks: **ACCEPTED**, because `continue` there means *this token is not a path*, which is the common case rather than an error, and making it speak would emit noise on nearly every Bash call -- the muted-guard failure this tree has removed from four other checks. **It cannot launder an absence into a pass**, which is what anti-pattern #9 is about: a missing row means not-observed, never confirmed-unread, and `verify_citations` treats an unmatched citation as a signal rather than a verdict. All 17 CI python gates were then enumerated from the workflows and run locally.*
