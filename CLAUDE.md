...
*Version 0.42.1 -- **Attribution label: dod-design-doc-2026-06-11** (lived-friction-triggered). Doc-only: adds `docs/design/definition-of-done.md` — the design for an outcome-based Definition-of-Done forcing function (pin a measurable, outcome-based done-criterion at diamond BIRTH + retrofit when missing), grounded in two adversarially-verified deep-research passes. Captures adoptable rules (contribution-not-summation laddering; lead-low/lag-high indicators + OMTM; state+date pre-committed kill-criteria via pre-mortem; provisional teaching prompts → `/prompt-optimizer` A/B), the canvas field, the `/define-done` Socratic sequence, per-scale done table, birth + detect-and-prompt retrofit mechanism, and a phased build plan. Design only — NOT built. Origin: roadmap dogfood (L0 reached Deliver with an implicit success bar). **PATCH** (design doc; no skill/engine/hook/schema change). **Prior** (v0.42.0): **autonomous-evidence-guard-2026-06-11** (lived-friction-triggered).*
