...
**G-D1: Never skip discovery for Complex-domain problems** `REVIEW`
...
**G-D2: Never treat a single interview as sufficient evidence** `NUDGE`
...
**G-D3: Never ask hypothetical or leading questions in research** `NUDGE`
...
**G-D4: Never validate opportunities using only one evidence type** `NUDGE`
...
**G-D5: Always run bias checklist before conducting research** `NUDGE`
...
**G-D6: Always map emotional and social dimensions, not just functional** `NUDGE`
...
**G-S1: Never store, log, or transmit user secrets in plaintext** `BLOCK`
...
**G-S2: Never skip threat modeling when a solution handles user data or requires system permissions** `REVIEW`
...
**G-S3: Never add a data collection point without Privacy by Design assessment** `REVIEW`
...
**G-S4: Never ship without input validation on all external inputs** `REVIEW`
...
**G-S5: Always apply data minimization** `NUDGE`
...
**G-S6: Always design for secure defaults** `NUDGE`
...
**G-S7: Always disclose AI nature in user-facing systems** `REVIEW`
...
**G-S8: Always assess EU AI Act risk classification for AI-powered products** `NUDGE`
...
**G-V5: Always prefer the simplest working solution** `NUDGE`
...
**G-V6: Always maintain clean separation between layers** `NUDGE`
...
**G-V9: Always design error states** `REVIEW`
...
**G-V10: Always check usability heuristics for user-facing interfaces** `REVIEW`
...
**G-P1: Never progress a diamond without updating the canvas** `REVIEW`
...
**G-P2: Never ignore BVSSH dimensions** `NUDGE`
...
**G-P3: Always cite which theory informed a decision** `NUDGE`
...
**G-P4: Always log decisions in the decision log** `REVIEW`
...
**G-P5: Always read corrections.md before implementation tasks** `BLOCK`
...
**G-P6: Always play devil's advocate before major transitions** `NUDGE`
