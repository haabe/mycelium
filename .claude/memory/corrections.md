...
- **JTBD schema lacks per-dimension backing fields (Christensen tripartite)**: `jobs-to-be-done.schema.json` provenance has aggregate `confidence` only; Christensen's discipline requires functional + emotional + social to be backed independently, but the schema can't express that natively. Workaround: per-dimension validation block at job-entry level (additionalProperties: true). **6th-instance graduation candidate** of the "documented rule diverges from enforcement" cluster — Check 26 graduated at instance 5, next graduation per Check 26 commit message is "framework changes ship with their own consistency checks" guardrail at REVIEW tier.
