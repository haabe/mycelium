...
**Total instances:** 15 (as of 2026-05-28 — instance 15 GRADUATED instance-13's `canvas-state-vs-reality-drift` carry-forwards to mechanism via v0.31.3; instance 14 added late-session during validate_canvas.py refactor investigation; new sub-shape `validator-tolerance-vs-parser-strictness` graduated to mechanism same session via fail-loud refactor + positional argv fix + G-V12 fixture test)

**First instance:** pre-2026-04-28 (referenced in the Check 26 graduation commit `d6c4e9d`)

**Most recent:** 2026-05-28 (instance 15 — `canvas-state-vs-reality-drift` recurred across ht-012/ht-014 + cold ht-009/ht-010 + Alex consent registry-lag; **graduated to mechanism via v0.31.3**, closing instance 13's two carry-forward candidates)

**Log-divergence note (reconciled 2026-05-28):** this framework `cluster-instances.md` itself lagged the roadmap dogfood log by ~4 days (framework file 24 May; roadmap had 2026-05-26/28 instances). That lag is itself a `documented-rule-diverges-from-enforcement` instance — the cluster log is a fact-home, and it drifted from the shipped reality (v0.31.3 graduated a carry-forward the log didn't record). Surfaced by `/framework-health` 2026-05-28 and reconciled in this edit. The framework log is the curated/published set; the roadmap log (`mycelium-roadmap/.claude/memory/cluster-instances.md`) carries the full live instance stream including AP#7 instance #11 (agent-presented-tool-output-as-fact, this session).
...
| 15 | 2026-05-28 | `canvas-state-vs-reality-drift` RECURRED (instance 13's sub-shape): ht-012 (Frida) left `in_progress` after its evidence (partial_findings f1-f10 + purpose.yml entries) was already logged; ht-014 (Alex) findings reached purpose.yml as "cohort tester-2" but were never linked back to the task; cold outreach (ht-009 Simon, ht-010 Daniel) stayed open because abandonment is a non-event; and consent reached auto-memory but not the canonical attribution-registry (Alex, stale 2 days). User-detected during backlog triage: "why weren't these updated when the agent got the data?" Recurrence confirms instance 13 was not a one-off. | canvas-state-vs-reality-drift (recurrence) | **GRADUATED TO MECHANISM (v0.31.3).** Both of instance 13's carry-forward candidates shipped: (a) `/canvas-health` Step 8c reconciliation (status-vs-activity staleness 21d, evidence-exists-but-task-open, consent-vs-registry mismatch); (b) session-start hook `CHECK 5` now counts OPEN tasks by status + flags 14d staleness, replacing raw `len(pending_tasks)` (dogfood: 16 entries → 4 open, all stale). Deferred (honest): `/log-evidence` close-the-loop (generating-side auto-close + registry sync). Framework `corrections.md` 2026-05-28 + roadmap `corrections.md` 2026-05-28 (paired with AP#7-on-own-tooling). Validator passed 34/0/1. |
...
**Mechanism-graduation GATE-DEFER (2026-05-30, surfaced by `/framework-health` step 4b):** the cluster is at 15 instances, well past the initial graduation candidate threshold, but the *mechanism* promotion bar (≥3 detection rules from the spec validated against the instance corpus at <5% FP) is **explicitly gate-deferred, not silently pending**. `Gated by: ≥3 of the spec's 6 Rules implemented in this repo AND run against the 15-instance corpus with measured FP <5% — interventional`. Why deferred rather than forced now: Rules 1-5 are not yet implemented in the framework repo (Rule 6 exists only in roadmap as `check_skill_prompt_drift.py`); FP measurement requires running candidate detectors against a labelled corpus, which is research-shaped work, not a same-session edit. Faking the validation (asserting <5% FP without measuring) would itself be an AP#7 *consistency-only* instance. The honest state: spec is the current mechanism; full graduation waits on the named gate. Re-checked every `/framework-health` run.
...
**Graduation status:** `anti-pattern` (graduated 2026-05-09 to `harness/anti-patterns.md` as #7) + `behavioral-enforcement-canvas-writes` (v0.22.0 shipped Read-before-Write, Validator Check 31, /diamond-assess Step 3 Read-before-claim hard rule) + `communication-rule-conventions` (conversational + implicit-causal-link sub-classes shipped to CLAUDE.md Communication Rules — see below).

**Sub-graduations:**
- **Conversational sub-class** — **GRADUATED**. Shipped to CLAUDE.md Communication Rules as the "name the verification surface" convention (`Verified: ran [tool]` / `Cited: [source:line]` / `Per [speaker/tool]: [claim]` / `Unverified`) — requires the agent to name + cite the evidence source before propagating a claim it did not directly observe. This is the structural enforcement the threshold-met candidate called for. NUDGE-tier (visibility convention, not a hard block) — the correct mechanism class for this cluster per the graduation philosophy below. Also covers sub-class e (trust-without-verification).
- **Implicit-causal-link sub-class** (sub-class g, surfaced 2026-05-23) — **GRADUATED**. Shipped to CLAUDE.md Communication Rules as the "name the gate before a deferral" convention (`Gated by: [event] — interventional|observational`, plus the `ON HOLD (pending X)` and natural-prose forms) — makes the implicit date/threshold causal link inspectable in user-facing output. Closes the gap Pre-Ship #9 left (its attribution pass operates on explicit chains, not implicit date/threshold causality). NUDGE-tier.
...
**Total instances:** 18+ catalogued across both repos (framework public corrections.md + roadmap private corrections.md). **Stage 1 graduation shipped v0.39.16 (2026-06-05)**: Read-before-Recommend Communication Rule + Preflight blocks on diamond-assess + diamond-progress + Validator Check 41. Covers sub-shapes 1 (conversational recommendations on topics with extant canvas entries) and 5 (gate-status narration in diamond-assess/diamond-progress). Stage 2 backlog: sub-shape 2 (cross-repo state checks — needs hooks or registry diffs), sub-shape 3 (consent-state-change reconciliation), sub-shape 4 (cross-file completeness verification), sub-shape 6 (write-narration-verification, NEW from instance #18 — agent narrates "updated canvas" without verifying value-fields actually changed; symmetric to Check 31's Read-before-Write). Instance counts: 5 same-day instances 2026-06-02 (#13-#17) across all five Stage 1+2 sub-shapes; 1 same-day-same-graduation-day instance 2026-06-05 (#18, the recursive write-narration case that surfaced sub-shape 6). Convention-layer graduations (Verify-before-propagate, Gated-by) shipped 2026-05-23 remain in force.
...
**Most recent (framework-visible):** 2026-06-05 (instance #18, NEW sub-shape 6 *write-narration-verification*: agent narrated "✅ canvas updated" after a `/dora-check` invocation when only `_meta.last_validated` was changed, not the value-field set the skill MANDATORY specified. Operator surfaced via Torres-shape question. Symmetric to Check 31's Read-before-Write: the verify-after-write half. **Recursive case**: fired in the SAME session as Stage 1 graduation v0.39.16 Read-before-Recommend, within hours of the ship — Stage 1 covered conversational + gate-narration; #18's sub-shape was outside that scope. Sub-shape (6) added to Stage 2 backlog. Recovery: roadmap commit `9ddea8f` refreshed dora-metrics.yml value fields properly; corrections.md + cluster-instances.md entries logged. Prevention mechanism candidate: Stop-hook that diffs canvas post-state against the running-skill's update-spec, OR new skill-internal "## Postflight: Verify-After-Write" preamble parallel to "## Preflight: Read-before-Write".)

**Prior framework-visible:** 2026-05-23 (capacity-vs-evidence-gating conflation in BVSSH-on-l0-purpose recommendation; Pre-Ship #9 ran and did not catch the implicit causal link between deferral date and unblock event)
...
| 2026-06-02 (single working day) | canvas-state skip + cross-repo state + consent-state-change + cross-file completeness + gate-narration confabulation | 5 (#13-#17) | Heaviest single-day contribution to the cluster ever. Five sibling sub-shapes from one root pattern (System-1 verification-skip across surfaces). Recursive case: #17 fired during `/diamond-assess` running an un-mechanized version of the failure the same skill named as queued graduation candidate. Drove Stage 1 graduation v0.39.16 (Read-before-Recommend Communication Rule + diamond-assess/diamond-progress preambles + Validator Check 41). |
| 2026-06-05 (same-day-same-graduation-day) | write-narration-verification (NEW sub-shape 6, Stage 2 backlog) | 1 (#18) | Agent narrated "✅ canvas updated" after a `/dora-check` invocation when only `_meta.last_validated` was changed, not the value-field set the skill MANDATORY specified. Operator surfaced via Torres-shape question. Recursive case: fired within hours of Stage 1 ship — Stage 1 covered conversational + gate-narration; #18's surface was outside that scope. Symmetric to Check 31's Read-before-Write: the verify-after-write half. Recovery: roadmap commit `9ddea8f`. Mechanism candidate: Stop-hook diffing canvas post-state against running-skill update-spec, OR skill-internal `## Postflight: Verify-After-Write` preamble. Sub-shape (6) added to Stage 2. |
