...
**Total instances:** 13+ catalogued across both repos (framework public corrections.md + roadmap private corrections.md). 5 new instances added 2026-05-23/24 session: sub-class g implicit-causal-link (BVSSH-deferral conflation, 2026-05-23) + sub-class g variant temporal-binarization (same-turn recurrence, 2026-05-23) + sub-class e new surface validator-output-verification (EXE001 "pre-existing" claim, 2026-05-23) + sub-class c self-application (P6 Edith-Mari evidence dismissed within service-check, 2026-05-23) + sub-class g instance (cohort-attribution on clone-spike days without version-ship correlation, /metrics-pull #36 draft, 2026-05-24 — user-prompted catch via clones/unique observation). Sub-class distribution skews to conversational + self-application + trust-without-verification + implicit-causal-link; canvas-write enforcement layer (v0.22.0) reduced canvas-write recurrence but did not eliminate adjacent surfaces. Sub-class e broadened from subagent-output-verification → trust-without-verification 2026-05-23 to cover any tool/wrapper/dialog claim the agent didn't independently verify. `Verify-before-propagate:` convention shipped 2026-05-23 to CLAUDE.md Communication Rules as next mechanism layer for sub-classes b + e. **Convention's first observable effect 2026-05-24**: when user prompted on clones/unique angle, agent CHECKED via version-ship correlation rather than defending original cohort-attribution framing. Convention caught mid-stream after first-draft slippage — consistent with graduation philosophy ("make interpolation visible, not prevent it").
...
**Most recent (framework-visible):** 2026-05-28 (AP#7 surfaced on the agent's OWN tooling output — a NEW surface: the `Verify-before-propagate` convention was being applied to subagent/external claims but not to the agent's own `awk`/`grep`. Three instances this session: (1) a buggy-`awk` human-task id→objective map presented as ground truth, which the user then acted on — would have killed the Frida observation instead of the cold Luca outreach if not caught; (2) "Alex's feedback isn't captured" asserted without first grepping purpose.yml (it was captured, as "cohort tester-2"); (3) "project name appears nowhere (grep-verified)" written into the registry on a bad grep pattern. Logged roadmap cluster #11 + framework `corrections.md` 2026-05-28. Prevention: extend the verification-surface convention + ID-scan discipline to the agent's own tool output, not just external claims.)
