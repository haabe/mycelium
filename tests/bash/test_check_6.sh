#!/usr/bin/env bash
# G-V12 coverage proof for Check 6: skill count in docs/skills/README.md vs disk.
set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/_assert.sh"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
FIXTURES_DIR="$SCRIPT_DIR/fixtures/check_6"
...
capture() {
    ...
    local out
    out=$(check_skill_count_readme 2>&1)
...
test_passes_on_match() {
    ...
    assert_contains "$output" "PASS" "passes when doc count matches disk"
...
test_flags_mismatch() {
    ...
    assert_contains "$output" "5" "names doc count"
}
test_handles_stub() {
    local output; output=$(capture "stub")
    assert_contains "$output" "Phase 1 stub" "recognizes stub"
}

echo "=== test_check_6: Check 6 (skill count in docs/skills/README.md) ==="
